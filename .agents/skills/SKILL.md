---
name: cubing-pro-base
description: 项目基础要求说明，美学要求。创建具有高设计品质、可交付生产的前端界面。当用户要求构建 Web 组件、页面、海报或应用时使用本技能。产出有创意、打磨到位且避免「AI 通用审美」的代码与界面设计。
---

## 基础要求
- 所有组件都要求多端自适应，兼容PC、窄屏、手机、平板。
- 主题颜色为淡蓝色、青色、海蓝色。
- 不改动业务逻辑，但UI界面等可进行适当调整。
    - 布局等可进行重构。但业务逻辑不要修改。
- 参考旧版本代码位置于 /Users/guojia/worker/code/cube/cubingPro/cubing.pro，按功能一比一迁移，组件可以依据需要替换为现有框架组件。
