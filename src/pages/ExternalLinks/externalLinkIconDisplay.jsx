...
import { OTHER_LINK_ANTD_ICON_MAP, OTHER_LINK_FC_ICON_MAP, OTHER_LINK_GI_ICON_MAP, } from './externalLinkIconRegistry';
import { DEFAULT_LINK_ICON, parseAntdIconKey, parseCubeIconKey, parseFcIconKey, parseGiIconKey, parseLetterIconKey, resolveIconSrc, } from './utils';
...
const fieldWrap = (node, size) => size === 'field' ? (<div className="external-link-icon-field__previewBox">{node}</div>) : (node);
/** 与 {@link ExternalLinkCard} 中图标逻辑一致，供表单预览等复用 */
export const ExternalLinkIconMedia = ({ link, size = 'card' }) => {
    const src = resolveIconSrc(link);
    const iconRaw = link.icon?.trim();
    if (src) {
        return fieldWrap(<img src={src} alt="" className="external-link-card__icon" onError={(e) => {
    ...
    if (cube) {
        return fieldWrap(<span className={cardCubeWrapClass}>
    ...
    return fieldWrap(<img src={DEFAULT_LINK_ICON} alt="" className="external-link-card__icon"/>, size);
};
//# sourceMappingURL=externalLinkIconDisplay.jsx.map