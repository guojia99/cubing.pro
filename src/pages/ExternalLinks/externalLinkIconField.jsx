...
import { ANTD_ICON_PREFIX, CUBE_ICON_PREFIX, FC_ICON_PREFIX, GI_ICON_PREFIX, LETTER_ICON_PREFIX, parsePrefixedIconKey, } from './utils';
function isLibraryIconValue(v) {
    const t = v?.trim();
    if (!t)
        return false;
    return (parsePrefixedIconKey(ANTD_ICON_PREFIX, t) !== null ||
...
export const ExternalLinkIconField = ({ value, onChange, iconUrl, }) => {
    ...
    return (<Space direction="vertical" style={{ width: '100%' }} size="middle">
      ...
      <ExternalLinkIconPicker open={pickerOpen} value={value} onCancel={() => setPickerOpen(false)} onConfirm={(next) => {
            onChange?.(next);
            setPickerOpen(false);
        }}/>

      <Input placeholder="或手动输入路径 / 完整 URL（写入 icon；与图标库二选一）" value={manualValue} onChange={(e) => onChange?.(e.target.value)}/>
    </Space>);
};
//# sourceMappingURL=externalLinkIconField.jsx.map