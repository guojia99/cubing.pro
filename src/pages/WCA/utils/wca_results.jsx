 ...
 */
export function secondTimeFormat(seconds, mbf) {
    // 处理负数？根据需求决定是否需要
    if (seconds < 0) {
        // 可选：抛出错误或处理负数逻辑
        // 这里假设 seconds >= 0
 ...
 * @param inValue
 */
export function get333MBFResult(inValue) {
 ...
 */
export function get333MBOResult(inValue) {
 ...
 * @returns 格式化后的字符串
 */
export function resultsTimeFormat(value, event, isAvg) {
    if (value === 0) {
        return '';
    }
    // 特殊值处理
...
export const formatAttempts = (attempts, eventId, best_index, worst_index, wrapLayout = false) => {
    ...
    const itemElements = attempts.map((time, i) => {
        ...
        return (<span key={i} title={formatted} style={{
    ...
    if (wrapLayout) {
        ...
        return (<div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px 8px' }}>
        {itemElements}
      </div>);
    }
    // 个人主页：固定每行 5 个成绩
    ...
    for (let i = 0; i < itemElements.length; i += 5) {
        ...
        rows.push(<div key={i} style={{ display: 'flex', gap: 8 }}>
        {group}
      </div>);
    }
    return <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{rows}</div>;
};
//# sourceMappingURL=wca_results.jsx.map