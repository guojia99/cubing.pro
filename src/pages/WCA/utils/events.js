...
export function getCompsEvents(wcaResults) {
    const competitionEventMap = new Map();
    ...
    wcaResults.forEach((result) => {
        ...
        if (!tempMap.has(competition_id)) {
            tempMap.set(competition_id, new Set());
        }
...
export const getRecordColor = (type) => {
    if (type.includes('WR'))
        return 'red';
    if (type.includes('NR'))
        return 'green';
    if (type.includes('AsR'))
        return 'orange';
    return 'gold';
};
