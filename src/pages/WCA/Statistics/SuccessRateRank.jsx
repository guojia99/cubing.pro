...
const SuccessRateRank = () => {
    ...
    const [eventId, setEventId] = useState(SUCCESS_RATE_EVENTS[0]);
    ...
    const [minAttemptedInput, setMinAttemptedInput] = useState(3);
    const [loading, setLoading] = useState(false);
    const commitMinAttempted = (v) => {
    ...
    const rankedData = useMemo(() => assignRanks(data, (page - 1) * pageSize), [data, page, pageSize]);
    const columns = [
        {
    ...
    return (<div className="success-rate-rank">
      <Card size="small" bordered className="stats-rank-filter-card">
        ...
        <div className="filter-item filter-item-min-attempted">
          ...
          <InputNumber size="small" min={3} max={500} value={minAttemptedInput} onChange={(v) => setMinAttemptedInput(v)} onBlur={() => commitMinAttempted(minAttemptedInput ?? minAttempted)} onPressEnter={() => commitMinAttempted(minAttemptedInput ?? minAttempted)} className="filter-input-number" style={{ width: 72 }}/>
          <Space size={4} className="filter-presets">
            {MIN_ATTEMPTED_PRESETS.map((n) => (<Button key={n} type={minAttempted === n ? 'primary' : 'default'} size="small" onClick={() => {
                setMinAttempted(n);
      ...
      <Card size="small" bordered className="stats-rank-table-card">
        <Spin spinning={loading}>
          <div className="table-wrapper">
            <Table dataSource={rankedData} columns={columns} rowKey="wcaId" size="small" tableLayout="fixed" scroll={{ x: 800 }} pagination={{
            size: 'small',
            current: page,
            pageSize,
            total,
...
//# sourceMappingURL=SuccessRateRank.jsx.map