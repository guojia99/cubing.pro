...
import { Select, Table, Spin, Switch, Space, Tag, Card } from 'antd';
 ...
 */
function parseCompletedEvents(record) {
    if (record.doneEventList?.length)
        return record.doneEventList;
    try {
        const parsed = JSON.parse(record.doneEventJSON || '[]');
 ...
 */
function getMissingEvents(completedEvents) {
...
function assignRanks(items, offset) {
    const result = [];
...
/** 仅分配序号（不做排行，用于缺少项目模式） */
function assignIndex(items, offset) {
    return items.map((item, i) => ({ ...item, displayRank: offset + i + 1 }));
}
const AllEventsAchievementRank = () => {
    const intl = useIntl();
    const [country, setCountry] = useState(WORLD_KEY);
    const [isFullAchievement, setIsFullAchievement] = useState(true);
    const [lackNum, setLackNum] = useState(1);
    ...
    const rankedData = useMemo(() => isFullAchievement
        ? assignRanks(data, (page - 1) * pageSize)
        : assignIndex(data, (page - 1) * pageSize), [data, page, pageSize, isFullAchievement]);
    const columns = [
        {
        ...
        },
        {
            ...
            width: 130,
            ellipsis: true,
            render: (_, record) => WCALinkWithCnName(record.wcaId, record.name),
        },
        ...(isFullAchievement
            ? [
                {
            ...
            ? [
                {
                    ...
                    width: 180,
                    ellipsis: true,
                    render: (_, record) => record.allEventCompName ? (<Tag className="competition-tag">{record.allEventCompName}</Tag>) : ('-'),
                },
                {
            ...
            ? [
                {
                    ...
                    render: (_, record) => {
                        ...
                        return (<div className="missing-events-cell">
                  {missing.map((ev) => (<Space key={ev} size={4} className="missing-event-tag">
    ...
    return (<div className="all-events-achievement-rank">
      <Card size="small" bordered className="stats-rank-filter-card">
        ...
        <div className="filter-item filter-item-switch">
          ...
          <Switch checked={isFullAchievement} onChange={(checked) => {
            setIsFullAchievement(checked);
            setPage(1);
        }} checkedChildren={intl.formatMessage({ id: 'wca.allEventsAchievement.fullAchievement' })} unCheckedChildren={intl.formatMessage({ id: 'wca.allEventsAchievement.missingEventsMode' })}/>
        </div>
        {!isFullAchievement && (<div className="filter-item">
            <span className="filter-label">{intl.formatMessage({ id: 'wca.allEventsAchievement.lackNum' })}:</span>
            <Select size="small" value={lackNum} onChange={(v) => {
                setLackNum(v);
                setPage(1);
      ...
      <Card size="small" bordered className="stats-rank-table-card">
        <Spin spinning={loading}>
          <div className="table-wrapper">
            <Table dataSource={rankedData} columns={columns} rowKey="wcaId" size="small" tableLayout="fixed" scroll={{ x: 900 }} pagination={{
            size: 'small',
            current: page,
            pageSize,
            total,
...
//# sourceMappingURL=AllEventsAchievementRank.jsx.map