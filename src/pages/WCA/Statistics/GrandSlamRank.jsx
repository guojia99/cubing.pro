...
function getTagBgColor(rank, type) {
...
function getMedalIcon(rank) {
    ...
    if (rank === 1)
        return <span style={{ ...sizeStyle, color: '#FFD700' }}>🥇</span>;
    if (rank === 2)
        return <span style={{ ...sizeStyle, color: '#C0C0C0' }}>🥈</span>;
    if (rank === 3)
        return <span style={{ ...sizeStyle, color: '#CD7F32' }}>🥉</span>;
    return null;
}
/** 渲染锦标赛名称+奖牌 */
function renderChampionshipWithMedal(name, rank, type) {
    const icon = getMedalIcon(rank);
    const bgColor = getTagBgColor(rank, type);
    return (<Tag className="championship-tag" style={{ backgroundColor: bgColor, borderColor: 'transparent' }}>
...
const GrandSlamRank = () => {
    ...
    const [filterGrandSlamOnly, setFilterGrandSlamOnly] = useState(true);
    const [filterHasWR, setFilterHasWR] = useState(true);
    const [countryFilter, setCountryFilter] = useState(WORLD_KEY);
    ...
    const filteredData = useMemo(() => {
        ...
        if (filterGrandSlamOnly) {
            result = result.filter((r) => r.worldChampionshipRank === 1 &&
    ...
    const table1Columns = [
        ...
        },
        {
            ...
            key: 'wcaID',
            width: 100,
            render: (_, record) => WCALinkWithCnName(record.wcaID, record.wcaID),
        },
        {
            ...
            width: 100,
            ellipsis: true,
            render: (_, record) => WCALinkWithCnName(record.wcaID, record.wcaName),
        },
        {
        ...
        },
        {
            ...
            key: 'best',
            width: 90,
            render: (_, record) => resultsTimeFormat(record.best, record.eventID, false),
        },
        {
            title: intl.formatMessage({ id: 'wca.resultTable.average' }),
            key: 'average',
            width: 90,
            render: (_, record) => record.eventID === '333mbf' ? '—' : resultsTimeFormat(record.average, record.eventID, true),
        },
        {
            title: 'WR',
            key: 'hasWR',
            width: 56,
            render: (_, record) => record.hasWR ? (<Tag style={{ backgroundColor: 'rgba(139, 0, 0, 0.25)', borderColor: '#8B0000', color: '#8B0000' }}>
            WR
          </Tag>) : ('—'),
        },
        {
            ...
            key: 'world',
            minWidth: 180,
            render: (_, record) => renderChampionshipWithMedal(record.worldChampionshipName, record.worldChampionshipRank, 'world'),
        },
        {
            title: intl.formatMessage({ id: 'wca.grandSlam.continentChampionship' }),
            key: 'continent',
            minWidth: 180,
            render: (_, record) => renderChampionshipWithMedal(record.continentChampionshipName, record.continentChampionshipRank, 'continent'),
        },
        {
            title: intl.formatMessage({ id: 'wca.grandSlam.countryChampionship' }),
            key: 'countryChamp',
            minWidth: 180,
            render: (_, record) => renderChampionshipWithMedal(record.countryChampionshipName, record.countryChampionshipRank, 'country'),
        },
        {
            ...
            key: 'playerCountry',
            width: 120,
            render: (val) => getWcaCountryLabel(val, countries),
        },
    ];
    const table2Columns = [
        {
            ...
            width: 80,
            fixed: 'left',
            render: (_, record) => (<Button type="link" size="small" icon={<RightOutlined />} onClick={() => {
        ...
        },
        {
            ...
            key: 'count',
            width: 120,
            render: (val) => (<span style={{ fontWeight: 600 }}>
    ...
    return (<div className="grand-slam-rank">
      ...
      <div className="filter-row">
        <div className="filter-item filter-item-switch">
          ...
          <Switch checked={showTable1} onChange={setShowTable1} checkedChildren={intl.formatMessage({ id: 'wca.grandSlam.viewResults' })} unCheckedChildren={intl.formatMessage({ id: 'wca.grandSlam.viewRank' })}/>
        </div>
        {showTable1 && (<>
            <div className="filter-item filter-item-switch">
              <span className="filter-label">{intl.formatMessage({ id: 'wca.grandSlam.filterGrandSlamOnly' })}:</span>
              <Switch size="small" checked={filterGrandSlamOnly} onChange={setFilterGrandSlamOnly} checkedChildren={intl.formatMessage({ id: 'wca.grandSlam.filterGrandSlamOnlyOn' })} unCheckedChildren={intl.formatMessage({ id: 'wca.grandSlam.filterGrandSlamOnlyOff' })}/>
            </div>
            <div className="filter-item filter-item-switch">
              <span className="filter-label">{intl.formatMessage({ id: 'wca.grandSlam.filterHasWR' })}:</span>
              <Switch size="small" checked={filterHasWR} onChange={setFilterHasWR} checkedChildren={intl.formatMessage({ id: 'wca.grandSlam.filterHasWROn' })} unCheckedChildren={intl.formatMessage({ id: 'wca.grandSlam.filterHasWROff' })}/>
            </div>
            <div className="filter-item">
              <span className="filter-label">{intl.formatMessage({ id: 'wca.players.country' })}:</span>
              <Select size="small" value={countryFilter} onChange={setCountryFilter} options={countryOptions} className="filter-select" showSearch optionFilterProp="label" filterOption={(input, opt) => opt?.label?.toLowerCase().includes(input.toLowerCase())} style={{ minWidth: 140 }}/>
            </div>
            <div className="filter-item">
              <span className="filter-label">{intl.formatMessage({ id: 'wca.grandSlam.event' })}:</span>
              <Select size="small" value={eventFilter} onChange={setEventFilter} options={eventOptions} className="filter-select" style={{ minWidth: 140 }}/>
            </div>
            <div className="filter-item">
              <span className="filter-label">{intl.formatMessage({ id: 'wca.players.name' })}:</span>
              <Input size="small" placeholder={intl.formatMessage({ id: 'wca.grandSlam.namePlaceholder' })} value={nameFilter} onChange={(e) => setNameFilter(e.target.value)} allowClear className="filter-input" style={{ width: 140 }}/>
            </div>
          </>)}
      </div>
      ...
      <div className="table-section">
        <Spin spinning={loading}>
          <div className="table-wrapper">
            {showTable1 ? (<Table dataSource={filteredData} columns={table1Columns} rowKey={(r) => `${r.wcaID}-${r.eventID}`} size="small" tableLayout="auto" scroll={{ x: 'max-content' }} pagination={false}/>) : (<Table dataSource={playerSummaryList} columns={table2Columns} rowKey="wcaID" size="small" tableLayout="fixed" scroll={{ x: 600 }} pagination={false}/>)}
          </div>
        </Spin>
      </div>

      <Modal title={modalRecord
            ? `${modalRecord.wcaName} (${modalRecord.wcaID}) - ${intl.formatMessage({ id: 'wca.grandSlam.viewDetail' })}`
            : ''} open={modalOpen} onCancel={() => {
            setModalOpen(false);
            setModalRecord(null);
        }} footer={null} width={800}>
        {modalRecord && (<Table dataSource={modalRecord.items} columns={table1Columns} rowKey={(r) => `${r.wcaID}-${r.eventID}`} size="small" tableLayout="auto" scroll={{ x: 'max-content' }} pagination={false} showHeader/>)}
      </Modal>
    </div>);
};
export default GrandSlamRank;
//# sourceMappingURL=GrandSlamRank.jsx.map