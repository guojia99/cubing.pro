...
const DiyEventsRank = () => {
    ...
    const [appliedEvents, setAppliedEvents] = useState(() => [...eventOrder]);
        ...
        if (!eventColsCollapsed && columnEventIds.length > 0) {
            for (const eventId of columnEventIds) {
                base.push({
                    ...
                    title: (<span className="diy-event-col-hdr" title={eventId}>
              {CubeIcon(eventId, `diy_hdr_${eventId}`, {})}
            </span>),
                    render: (_, record) => {
                        const entry = record.rankEntries?.find((x) => x.eventId === eventId);
                        if (!entry)
                            return '–';
    ...
    return (<div className="diy-events-rank">
      <Card size="small" bordered className="stats-rank-filter-card">
        <div className="filter-row">
          ...
          <div className="filter-item filter-item-switch">
            ...
            <Switch checked={notPodiumOnly} onChange={(checked) => {
            setNotPodiumOnly(checked);
            if (!checked)
                setFourthPlaceKing(false);
            setPage(1);
        }}/>
          </div>
          {notPodiumOnly && (<div className="filter-item filter-item-switch">
              <span className="filter-label">{intl.formatMessage({ id: 'wca.multiEventRank.fourthKing' })}:</span>
              <Switch checked={fourthPlaceKing} onChange={(checked) => {
      ...
      <EventSelectorEventsOnly events={[...eventOrder]} onConfirm={(evs) => {
      ...
      <Card size="small" bordered className="stats-rank-table-card">
        ...
        <Spin spinning={loading}>
          <div className="table-wrapper">
            <Table dataSource={data} columns={columns} rowKey={(r) => r.wcaId} size="small" tableLayout="fixed" scroll={{ x: scrollX }} pagination={{
            size: 'small',
            current: page,
            pageSize,
            total,
...
//# sourceMappingURL=DiyEventsRank.jsx.map