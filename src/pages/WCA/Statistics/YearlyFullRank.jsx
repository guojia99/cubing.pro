...
function isBestResult(event, a1, a2) {
    if (a1 < 0 && a2 < 0)
        return true;
    if (a1 < 0 && a2 > 0)
        return false;
...
function assignRanks(items, eventId, isAvg, offset) {
    const getValue = (r) => (isAvg ? r.average : r.best);
    const sorted = [...items].sort((a, b) => {
        const va = getValue(a);
        const vb = getValue(b);
        if (isBestResult(eventId, va, vb) && isBestResult(eventId, vb, va))
            return 0;
...
/** 单行显示成绩（收起态） */
function formatAttemptsSingleLine(attempts, eventId, bestIndex, worstIndex) {
    ...
    attempts.forEach((time, i) => {
        ...
        const displayText = len === 5 && (i === bestIndex || i === worstIndex) ? `(${formatted})` : formatted;
        if (parts.length > 0)
            parts.push(' ');
        parts.push(<span key={i} style={{ fontWeight: len === 5 && (i === bestIndex || i === worstIndex) ? 800 : 500 }}>
        {displayText}
      </span>);
    });
    return <span className="attempts-single-line" style={{ fontFamily: 'monospace' }}>{parts}</span>;
}
function formatCompetitionTime(timeStr) {
...
const YearlyFullRank = () => {
    ...
    const toggleAttempts = useCallback((id) => {
    ...
    const rankedData = useMemo(() => assignRanks(data, eventId, isAvg, (page - 1) * pageSize), [data, eventId, isAvg, page, pageSize]);
    const singleCol = {
        title: intl.formatMessage({ id: 'wca.resultTable.single' }),
        key: 'single',
        width: eventId === '333mbf' ? 120 : 85,
        render: (_, record) => (<span style={{ fontWeight: 600 }}>
        {resultsTimeFormat(record.best, eventId, false)}
      </span>),
    };
    const avgCol = eventId !== '333mbf'
        ? {
    ...
    const columns = [
        {
            ...
            width: 56,
            fixed: 'left',
            render: (_, record) => record.displayRank,
        },
        {
        ...
        ...(isAvg && avgCol ? [avgCol] : [singleCol]),
        {
            ...
            render: (_, record) => {
                ...
                return (<div className={`attempts-cell ${expanded ? 'expanded' : ''}`}>
            <span className="attempts-toggle" onClick={() => toggleAttempts(record.id)} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && toggleAttempts(record.id)}>
              {expanded ? <DownOutlined /> : <RightOutlined />}
            </span>
            {expanded ? (<div className="attempts-detail">
                {formatAttempts(attempts, eventId, bestIdx, worstIdx, true)}
              </div>) : (formatAttemptsSingleLine(attempts, eventId, bestIdx, worstIdx))}
          </div>);
            },
        },
        {
            ...
            width: 100,
            render: (_, record) => formatCompetitionTime(record.competition_time) || '-',
        },
        {
            ...
            ellipsis: true,
            width: 180,
            render: (name) => name ? <Tag className="competition-tag">{name}</Tag> : '-',
        },
        ...(country === WORLD_KEY
            ? [
                {
    ...
    return (<div className="yearly-full-rank">
      <Card size="small" bordered className="stats-rank-filter-card">
        ...
        <div className="filter-item">
          ...
          <Select size="small" value={year} onChange={(v) => {
            setYear(v);
            setPage(1);
        }} options={years.map((y) => ({ value: y, label: String(y) }))} className="filter-select"/>
        </div>
        <div className="filter-item">
          <span className="filter-label">{intl.formatMessage({ id: 'wca.historicalRank.month' })}:</span>
          <Select size="small" value={month} onChange={(v) => {
            setMonth(v);
            setPage(1);
        }} options={monthOptions} className="filter-select"/>
        </div>
        <div className="filter-item">
          <span className="filter-label">{intl.formatMessage({ id: 'wca.players.country' })}:</span>
          <Select size="small" value={country} onChange={(v) => {
            setCountry(v);
            setPage(1);
        }} options={countryOptions} className="filter-select" showSearch optionFilterProp="label" filterOption={(input, opt) => opt?.label?.toLowerCase().includes(input.toLowerCase())}/>
        </div>
        <div className="filter-item">
          <span className="filter-label">{intl.formatMessage({ id: 'wca.resultTable.event' })}:</span>
          <Select size="small" value={eventId} onChange={(v) => {
            setEventId(v);
            if (v === '333mbf' && isAvg)
                setIsAvg(false);
            setPage(1);
        }} options={eventOptions} className="filter-select" showSearch optionFilterProp="value" filterOption={(input, opt) => opt?.value?.toLowerCase().includes(input.toLowerCase())}/>
        </div>
        {eventId !== '333mbf' && (<div className="filter-item">
            <span className="filter-label">{intl.formatMessage({ id: 'wca.historicalRank.type' })}:</span>
            <Select size="small" value={isAvg} onChange={(v) => {
                setIsAvg(v);
                setPage(1);
            }} options={[
      ...
      <Card size="small" bordered className="stats-rank-table-card">
        <Spin spinning={loading}>
          <div className="table-wrapper">
            <Table dataSource={rankedData} columns={columns} rowKey="id" size="small" tableLayout="fixed" scroll={{ x: 1050 }} pagination={{
            size: 'small',
            current: page,
            pageSize,
            total,
...
//# sourceMappingURL=YearlyFullRank.jsx.map