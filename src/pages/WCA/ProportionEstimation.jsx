...
import { getResultProportionEstimation, impliedCentisAllEvents, interpolateRatioAt, segmentRefRangeAsAnchorCs, solveBackendAnchorCs, } from '@/services/cubing-pro/wca/proportion_estimation';
import { CubeIcon } from '@/components/CubeIcon/cube_icon';
import Markdown from '@/components/Markdown/Markdown';
import { LineChartOutlined, ReadOutlined, TeamOutlined, ThunderboltOutlined } from '@ant-design/icons';
import { useIntl } from '@@/plugin-locale';
import { Alert, Button, Card, Col, InputNumber, Modal, Row, Segmented, Select, Space, Statistic, Table, Tag, Typography, } from 'antd';
import ReactECharts from 'echarts-for-react';
import React, { useCallback, useEffect, useMemo, useState } from 'react';
import './ProportionEstimation.less';
const { Title, Text } = Typography;
const WR_OPTIONS = [100, 200, 500, 1000];
const TYPE_OPTIONS = [
...
const ProportionEstimation = () => {
    const intl = useIntl();
    const [estimationType, setEstimationType] = useState('bigcube');
    const [wrN, setWrN] = useState(500);
    const [data, setData] = useState(null);
    const [loading, setLoading] = useState(false);
    const [anchorSec, setAnchorSec] = useState(DEFAULT_ANCHOR_SEC.bigcube);
    ...
    const inferResolved = useMemo(() => {
        ...
        const ta = solveBackendAnchorCs(targetCs, anchorEvent, backendAnchor, data.segments, globalMap);
    ...
    const chartOption = useMemo(() => {
        ...
        const series = events.map((ev, idx) => {
            const pts = samples.map((s) => [xForSample(s), s.estimates[ev] ?? 0]);
            return {
                name: ev,
                type: 'line',
                smooth: true,
        ...
        const fmt = (n) => secondTimeFormat(n, false);
        return {
            color: CHART_COLORS,
            tooltip: {
                trigger: 'axis',
                ...
                formatter: (params) => {
                    ...
                    const xVal = Array.isArray(v0) && typeof v0[0] === 'number'
                        ? v0[0]
            ...
            xAxis: {
                ...
                scale: true,
                axisLabel: {
                    formatter: (val) => fmt(typeof val === 'string' ? parseFloat(val) : val),
                },
            },
            yAxis: {
                type: 'value',
    ...
    const segmentColumns = useMemo(() => {
        ...
        const evCols = events.map((ev) => ({
            ...
            render: (_, row) => {
                ...
                const r = ev === backendAnchor ? 1 : row.ratio?.[ev] ?? gr[ev] ?? 1;
        ...
        return [
            ...
            },
            {
                ...
                key: 'range',
                render: (_, row) => {
                    const [loCs, hiCs] = segmentRefRangeAsAnchorCs(row.anchor_min, row.anchor_max, anchorEvent, backendAnchor, segs, gr);
    ...
    const segmentDataSource = useMemo(() => (data?.segments ?? []).map((s) => ({
        ...s,
        ratio: s.ratio ?? {},
    })), [data?.segments]);
    const inferredColumns = useMemo(() => [
        {
            ...
            render: (_, row) => {
                ...
                return (<Tag className="proportion-infer-event-tag" color={isAnchor ? 'geekblue' : 'default'} bordered={!isAnchor}>
    ...
    return (<div className="proportion-page">
      <div className="proportion-hero">
        <div className="proportion-hero-bar">
          ...
          <Button type="link" icon={<ReadOutlined />} className="proportion-algorithm-btn" onClick={() => setAlgorithmModalOpen(true)}>
      ...
      <Card className="proportion-controls" bordered={false} loading={loading}>
        <Row gutter={[16, 16]} align="middle">
          <Col xs={24} md={10} lg={8}>
            ...
            <Select className="proportion-select-full" value={estimationType} options={TYPE_OPTIONS.map((o) => ({
            value: o.value,
          ...
          <Col xs={24} md={14} lg={10}>
            ...
            <Segmented className="proportion-wr-seg" value={wrN} onChange={(v) => setWrN(v)} options={WR_OPTIONS.map((n) => ({
            label: `WR${n}`,
            value: n,
        }))}/>
          </Col>
        ...
        {data && anchorEvent && (<Row gutter={[16, 16]} className="proportion-stats-row">
            <Col xs={24} sm={12} md={8}>
              <Card size="small" className="proportion-stat-card">
                <Statistic title={<Space>
                      <TeamOutlined />
                      {intl.formatMessage({ id: 'wca.proportion.statFit' })}
                    </Space>} value={data.sample_count} suffix={intl.formatMessage({ id: 'wca.proportion.people' })}/>
              </Card>
            </Col>
            <Col xs={24} sm={12} md={8}>
              <Card size="small" className="proportion-stat-card">
                <Statistic title={intl.formatMessage({ id: 'wca.proportion.statPool' })} value={data.persons?.length ?? 0} suffix={intl.formatMessage({ id: 'wca.proportion.people' })}/>
              </Card>
            </Col>
            <Col xs={24} md={8}>
              <Card size="small" className="proportion-stat-card proportion-stat-anchor">
                <div className="proportion-anchor-block">
                  ...
                  <Select className="proportion-select-full" value={anchorEvent} options={data.events.map((e) => ({ label: e, value: e }))} onChange={(v) => setAnchorEvent(v)}/>
                </div>
              </Card>
            </Col>
          </Row>)}

        {data && data.sample_count < 5 && (<Alert type="warning" showIcon className="proportion-alert" message={intl.formatMessage({ id: 'wca.proportion.lowSample' })}/>)}
      </Card>

      {data && anchorEvent && (<>
          <Card className="proportion-section" title={intl.formatMessage({ id: 'wca.proportion.sectionInfer' })}>
              ...
              <Row gutter={[16, 16]} align="bottom">
                <Col xs={24} sm={12} md={16}>
                  ...
                  <Space align="center" className="proportion-infer-input-row">
                    <InputNumber min={0} step={0.01} value={anchorSec} onChange={(v) => setAnchorSec(typeof v === 'number' ? v : 0)} className="proportion-anchor-input"/>
                    <Text type="secondary">s</Text>
                  </Space>
                </Col>
              </Row>
            </div>
            <Table className="proportion-infer-table" size="small" pagination={false} tableLayout="fixed" columns={inferredColumns} dataSource={inferredRows} rowKey="key" onRow={(record) => ({
                className: record.event === anchorEvent ? 'proportion-infer-row--anchor' : undefined,
            })}/>
          ...
          {chartOption && (<Card className="proportion-section" title={intl.formatMessage({ id: 'wca.proportion.sectionChart' })}>
              <div className="proportion-chart-wrap">
                <ReactECharts option={chartOption} style={{ height: 380, width: '100%' }} notMerge lazyUpdate/>
              </div>
            </Card>)}

          <Card className="proportion-section" title={intl.formatMessage({ id: 'wca.proportion.sectionGlobal' })}>
            <Row gutter={[16, 8]}>
              {data.events.map((ev) => {
                const gr = ev === backendAnchor ? 1 : globalMap[ev] ?? 1;
                const sec = (globalRefBackendCs * gr) / 100;
                return (<Col xs={12} sm={8} md={6} key={ev}>
          ...
          <Card className="proportion-section" title={intl.formatMessage({ id: 'wca.proportion.sectionSegments' })}>
            <div className="proportion-table-wrap">
              <Table size="small" scroll={{ x: 'max-content' }} pagination={false} columns={segmentColumns} dataSource={segmentDataSource} rowKey={(row) => `${row.anchor_min}-${row.anchor_max}-${row.n_persons}`}/>
            </div>
          </Card>
        </>)}

      <Modal title={intl.formatMessage({ id: 'wca.proportion.algorithmModalTitle' })} open={algorithmModalOpen} onCancel={() => setAlgorithmModalOpen(false)} footer={null} width={800} destroyOnClose className="proportion-algorithm-modal">
...
//# sourceMappingURL=ProportionEstimation.jsx.map