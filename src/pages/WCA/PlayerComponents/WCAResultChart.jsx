...
// ===== 工具函数 =====
function getQuantile(arr, q, TrimHeadAndTail = 0) {
    if (arr.length === 0)
        return 0;
    // 对数组进行排序
...
const WCAResultChart = ({ data, eventId, comps }) => {
    const intl = useIntl();
    const [recentCount, setRecentCount] = useState(20);
    const [recentHeadNum, setRecentHeadNum] = useState(0);
    const [recentMin, setRecentMin] = useState(0);
    const [recentMax, setRecentMax] = useState(15);
    // ===== 数据预处理 =====
    const seriesName = (() => {
        if (eventId === '333mbf') {
    ...
    const combinedOption = useMemo(() => {
            ...
            return {
                tooltip: {
                    ...
                    formatter: (params) => {
                        ...
                        const data = p.data;
                        if (!data || !data.extra)
                            return `${p.marker}${singleLabel}: DNF`;
        ...
        const singlePoints = chartData.singles.map((v, i) => {
            ...
            let progress = null;
            if (v <= 0)
                return { value: [i, null], extra: { compName } };
            if (v < bestSingle) {
            ...
            tooltip: {
                ...
                formatter: (params) => {
                    const lines = params
                        .filter((p) => {
                        // 排除显式关闭 tooltip 的系列
                        if (p.series?.tooltip?.show === false)
                            return false;
               ...
               ${originalData ? `${compLabel}: ${originalData.comp}` : ''} <br/>
                ${p.marker}${p.seriesName}: ${resultsTimeFormat(rawHundredths, eventId, false)}`;
                            ...
                            // 其他普通线（如中位数、移动平均等）
                            const rawHundredths = Array.isArray(p.value) ? p.value[1] : p.value;
                            return `${p.marker}${p.seriesName}: ${resultsTimeFormat(rawHundredths, eventId, true)}`;
                        }
                    });
    ...
    const getTrimMax = (count) => {
        if (count <= 20)
            return 10; // 最多去除25% (5/20)
        if (count <= 50)
            return 12; // 最多去除20% (10/50)
        if (count <= 100)
            return 15; // 最多去除15% (15/100)
        return 20; // 最多去除10% (20/200及更多)
    };
    // 更新recentMax当recentCount变化时
    const handleRecentCountChange = (value) => {
        setRecentMin(0);
    ...
    if (eventId !== '333mbf') {
        tabs.push({
          ...
          <div style={{
            ...
            {/* 四分线数选择器 */}
            <Tooltip title={<div style={{ maxWidth: 500, lineHeight: 1.6 }}>
                  <p>
              ...
              <Space style={{ display: 'flex', alignItems: 'center' }}>
                ...
                <Select id="recentCountSelect" value={recentCount} onChange={handleRecentCountChange} style={{ width: 120 }} options={[
            ...
            <Space style={{ display: 'flex', alignItems: 'center' }}>
              ...
              <Slider id="recentHeadSlider" min={recentMin} max={recentMax} step={1} value={recentHeadNum} onChange={setRecentHeadNum} tooltip={{ formatter: (value) => `${value}%` }} marks={generateMarks()} style={{ flex: 1, minWidth: 200 }}/>
            </Space>
          </div>
    ...
    return (<Card bordered={false} style={{ marginBottom: 32 }}>
      <Tabs items={tabs}/>
    </Card>);
};
export default WCAResultChart;
//# sourceMappingURL=WCAResultChart.jsx.map