...
const WCA_PLAYER_TAB_KEYS = ['results', 'competitions', 'eventStats', 'milestones', 'litCities'];
function tabKeyFromSearch(search) {
    const raw = new URLSearchParams(search).get('tab');
    if (raw && WCA_PLAYER_TAB_KEYS.includes(raw)) {
        return raw;
    }
    return 'results';
}
const WCAPlayerStaticsTab = ({ wcaProfile, wcaResults, comps, wcaRankTimer }) => {
    const intl = useIntl();
    const location = useLocation();
    const [activeKey, setActiveKey] = useState(() => typeof window !== 'undefined' ? tabKeyFromSearch(window.location.search) : 'results');
    ...
    const onTabChange = useCallback((key) => {
        if (!WCA_PLAYER_TAB_KEYS.includes(key))
            return;
        const k = key;
        setActiveKey(k);
    ...
    return (<Card hoverable style={{ width: '100%', margin: '0 auto', borderRadius: 16, marginTop: 16 }} bordered={false}>
      <Tabs className="wca-player-statics-tab" activeKey={activeKey} onChange={onTabChange} size="large" tabBarStyle={{ fontSize: 16, fontWeight: 'bold', paddingLeft: 16 }} animated>
        {/* 成绩 Tab */}
...
//# sourceMappingURL=WCAPlayerStaticsTab.jsx.map