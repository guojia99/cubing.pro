...
// 赛事列表表格组件
const CompetitionTable = ({ competitions, wcaResults }) => {
    const intl = useIntl();
    ...
    const competitionRegionPrefix = (iso2) => {
    ...
    const [competitionEventMap, setCompetitionEventMap] = useState(new Map());
    useEffect(() => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (name, record) => {
                ...
                return <Button type="link" size="small" href={record.website} target="_blank" style={{ padding: 0, height: 'auto', lineHeight: 'inherit' }}>
          {cpName}
        </Button>;
            }
        },
        ...
        },
        {
            ...
            render: (city, record) => {
                ...
                if (record.country_iso2 === 'CN' || record.country_iso2 === 'TW' || record.country_iso2 === 'HK') {
                    const englishProvince = cityNames.length > 1 ? cityNames[cityNames.length - 1] : undefined;
                    const convertedCityNames = translateChinaPlaceNames
                        ? cityNames.map((cityName, idx) => {
                            ...
                            if (isProvinceSegment) {
                                const peer = cityNames.length >= 2 ? cityNames[0] : undefined;
    ...
    return (<Table columns={columns} dataSource={sortedCompetitions} rowKey="id" pagination={false} scroll={{ x: 'max-content' }} size="small"/>);
};
export default CompetitionTable;
//# sourceMappingURL=WCAPlayerStaticsTabComps.jsx.map