import { CubeIcon } from '@/components/CubeIcon/cube_icon';
import { eventOrder } from '@/pages/WCA/utils/events';
import { Tabs } from 'antd';
import React from 'react';
import './WCAPlayerStaticsTabResultsWithEvent.less';
import { useIntl } from '@@/plugin-locale';
import ResultDetailWithEvent from './ResultDetailWithEvent';
const { TabPane } = Tabs;
const WCAPlayerStaticsTabResultsWithEvent = ({ wcaResults, comps, wcaRankTimer, }) => {
    const intl = useIntl();
    // 按 event_id 分组成绩
    const resultsByEvent = new Map();
    wcaResults.forEach((result) => {
        if (!resultsByEvent.has(result.event_id)) {
            resultsByEvent.set(result.event_id, []);
        }
        resultsByEvent.get(result.event_id).push(result);
    });
    // 获取选手实际参加过的项目，并按 eventOrder 排序
    const userEvents = Array.from(resultsByEvent.keys())
        .filter((event) => eventOrder.includes(event))
        .sort((a, b) => eventOrder.indexOf(a) - eventOrder.indexOf(b));
    if (userEvents.length === 0) {
        return <div>{intl.formatMessage({ id: 'wca.results.noHistory' })}</div>;
    }
    return (<Tabs className="wca-event-tabs" defaultActiveKey={userEvents[0]} size="large" tabPosition="top" centered={true}>
      {userEvents.map((eventId) => (<TabPane tab={<strong>{CubeIcon(eventId, eventId, {})}</strong>} key={eventId}>
          {/* 将分类好的数据传递给展示组件 */}
          <ResultDetailWithEvent eventID={eventId} wcaResults={resultsByEvent.get(eventId)} comps={comps} wcaRankTimer={wcaRankTimer}/>
        </TabPane>))}
    </Tabs>);
};
export default WCAPlayerStaticsTabResultsWithEvent;
//# sourceMappingURL=WCAPlayerStaticsTabResultsWithEvent.jsx.map