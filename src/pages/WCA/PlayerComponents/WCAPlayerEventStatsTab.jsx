 ...
 */
function computeEventStats(wcaResults, comps) {
    ...
    for (const r of wcaResults) {
        ...
        if (date < stat.firstDate)
            stat.firstDate = date;
        if (date > stat.lastDate)
            stat.lastDate = date;
        stat.roundCount += 1;
    ...
    const events = Array.from(byEvent.keys()).filter((e) => eventOrder.includes(e));
...
const WCAPlayerEventStatsTab = ({ wcaResults, comps, }) => {
    const intl = useIntl();
    const dataSource = useMemo(() => computeEventStats(wcaResults, comps), [wcaResults, comps]);
    const columns = [
        {
            ...
            width: 100,
            fixed: 'left',
            render: (eventId) => (<div className="event-stats-event-cell">
    ...
    if (dataSource.length === 0) {
        return (<div className="event-stats-empty">
        {intl.formatMessage({ id: 'wca.eventStats.noData' })}
      </div>);
    }
    return (<div className="event-stats-table-wrapper">
...
//# sourceMappingURL=WCAPlayerEventStatsTab.jsx.map