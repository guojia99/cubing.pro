import { Badge, Button, Card, Form, InputNumber, Select, Slider, Space, Tag, Timeline, Typography, } from 'antd';
import React, { useEffect, useRef, useState } from 'react';
import { useIntl } from '@@/plugin-locale';
import 'antd/dist/reset.css';
import MilestoneItemContent, { MILESTONE_COLOR_MAP, defaultMilestoneTypeOptions, } from '@/pages/WCA/PlayerComponents/MilestoneContent';
import { GetMilestones, MILESTONE_TYPE_PRIORITY, } from './player_milestone';
...
// 排序工具函数
const sortMilestones = (milestones, ascending) => {
...
const MilestoneTimelines = ({ wcaProfile, wcaResults, comps, }) => {
    const intl = useIntl();
    ...
    const scaleViewportRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const onTouchMove = (e) => {
            ...
            const next = Math.min(TIMELINE_ZOOM_MAX, Math.max(TIMELINE_ZOOM_MIN, Math.round(raw * 100) / 100));
    ...
    useEffect(() => {
        const milestones = GetMilestones(wcaProfile, wcaResults, comps, improvementNumber, (descriptor, values) => intl.formatMessage(descriptor, values));
    ...
    const tagRender = (props) => {
        ...
        return (<Tag color={MILESTONE_COLOR_MAP[value]} onMouseDown={onPreventMouseDown} closable={closable} onClose={onClose} style={{ marginInlineEnd: 4 }}>
        {label}
      </Tag>);
    };
    return (<Card title={<div style={{ paddingTop: 16, paddingBottom: 16 }}>
          <Space>
          ...
          <div style={{ marginTop: 16 }}>
            <Form.Item label={intl.formatMessage({ id: 'wca.milestone.excludeLabel' })}>
              <Select mode="multiple" allowClear maxTagCount="responsive" placeholder={intl.formatMessage({ id: 'wca.milestone.excludePlaceholder' })} value={excludedTypes} onChange={(value) => setExcludedTypes(value)} style={{ width: '100%', maxWidth: 200 }} options={milestoneTypeOptions} tagRender={tagRender} optionRender={(option) => {
                return (<>
                      <Tag color={MILESTONE_COLOR_MAP[option.data.value]}>
                        {option.data.label}(
          ...
          <div style={{ marginTop: 16, minWidth: 0, width: '100%', maxWidth: 400 }}>
            <Form.Item label={intl.formatMessage({ id: 'wca.milestone.improvementThreshold' })}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <Slider min={10} max={100} value={improvementNumber} onChange={setImprovementNumber} tooltip={{ formatter: (value) => `${value}%` }} style={{ flex: 1 }}/>
                <InputNumber min={10} max={100} value={improvementNumber} onChange={(value) => {
                if (value !== null) {
                    setImprovementNumber(value);
                }
            }} formatter={(value) => `${value}%`} 
        // @ts-ignore
        parser={(displayValue) => (displayValue || '')?.replace('%', '') || ''} style={{ width: '90px' }}/>
              </div>
            </Form.Item>
          </div>
        </div>}>
      <div className="milestone-scale-hint">
        <Text type="secondary" style={{ fontSize: 12 }}>
          ...
          {timelineZoom !== TIMELINE_ZOOM_DEFAULT ? (<>
              {' '}
              <Button type="link" size="small" style={{ padding: 0, height: 'auto', fontSize: 12 }} onClick={() => setTimelineZoom(TIMELINE_ZOOM_DEFAULT)}>
                {intl.formatMessage({ id: 'wca.milestone.timelineScaleReset' })}
              </Button>
            </>) : null}
        </Text>
      </div>
      <div ref={scaleViewportRef} className="milestone-scale-viewport" aria-label={intl.formatMessage({ id: 'wca.milestone.timelineScale' })}>
        <div className="milestone-timeline-wrapper" style={{ zoom: timelineZoom }}>
          <Timeline mode="alternate">
            {filteredMilestones.map((m, index) => {
            const yearMonthDay = getYearMonthDay(m.date ||
                m.achieved_on ||
                m.new_competition_date ||
                m.last_competition_date ||
                m.date_achieved);
            const isLeft = index % 2 === 1;
            return (<Timeline.Item key={`${m.type}-${index}`} // 更健壮的 key
             dot={<Badge color={MILESTONE_COLOR_MAP[m.type]} style={{ boxShadow: '0 0 0 2px #fff', marginTop: 8, width: 16, height: 16 }}/>} label={yearMonthDay ? (<Text type="secondary" style={{ position: 'relative', top: 3 }}>
                        {yearMonthDay}
                      </Text>) : null}>
...
//# sourceMappingURL=Milestone.jsx.map