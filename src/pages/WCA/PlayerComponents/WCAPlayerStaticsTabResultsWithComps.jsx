...
const WCAPlayerStaticsTabResultsWithComp = ({ wcaResults, comps }) => {
    const intl = useIntl();
    ...
    const sortedComps = Array.from(resultsByComp.entries())
        ...
        Object.entries(groupedByEvent).forEach(([eventId, eventResults]) => {
            const sortedEventResults = eventResults.sort((a, b) => (roundSortOrder[a.round_type_id] || 99) - (roundSortOrder[b.round_type_id] || 99));
            // 标记第一轮（决赛）显示项目，其余不显示
            sortedEventResults.forEach((result, index) => {
                result.showEvent = index === 0;
    ...
    const isProgress = (compIndex, eventId, value, type) => {
        if (value <= 0)
            return false;
        ...
        for (let i = compIndex + 1; i < sortedComps.length; i++) {
            ...
            for (const r of eventResults) {
                ...
                if (v > 0)
                    prevBest = Math.min(prevBest, v);
            }
        }
        return value < prevBest;
    };
    // 判断某比赛是否有至少一个 PB（单次或平均）
    const compHasPB = (compIndex) => {
        const comp = sortedComps[compIndex];
        for (const r of comp.results) {
            if (isProgress(compIndex, r.event_id, r.best, 'best'))
                return true;
            if (r.average > 0 && isProgress(compIndex, r.event_id, r.average, 'average'))
                return true;
        }
        return false;
    };
    ...
    const columns = [
        ...
        },
        {
            ...
            width: 120,
            render: (best, record) => {
                const progress = isProgress(record.compIndex ?? 0, record.event_id, best, 'best');
                return (<Space direction="horizontal" size={4} style={{ justifyContent: 'flex-end' }}>
            <span style={{
            ...
            {record.regional_single_record && (<Tag color={record.regional_single_record.includes('WR') ? 'red' :
                            record.regional_single_record.includes('NR') ? 'green' :
                                record.regional_single_record.includes('AsR') ? 'orange' : 'gold'} style={{ fontSize: '10px', padding: '0 4px', height: '18px', lineHeight: '18px' }}>
        ...
        },
        {
            ...
            render: (avg, record) => {
                if (avg <= 0)
                    return '-';
                const progress = isProgress(record.compIndex ?? 0, record.event_id, avg, 'average');
                return (<Space direction="horizontal" size={4} style={{ justifyContent: 'flex-end' }}>
            <span style={{
                        color: progress ? 'red' : 'inherit',
                        fontWeight: progress ? 600 : 400,
            ...
            {record.regional_average_record && (<Tag color={record.regional_average_record.includes('WR') ? 'red' :
                            record.regional_average_record.includes('NR') ? 'green' :
                                record.regional_average_record.includes('AsR') ? 'orange' : 'gold'} style={{ fontSize: '10px', padding: '0 4px', height: '18px', lineHeight: '18px' }}>
        ...
        },
        {
            ...
            key: 'attempts',
            width: 360,
            render: (_, record) => (<span style={{ fontFamily: 'monospace', fontSize: '12px', whiteSpace: 'pre-wrap' }}>
    ...
    const getCompDisplayName = (c) => {
        const findName = findCubingCompetitionByIdentifier(c.compID);
        return findName ? findName.name : c.name;
    };
    return (<div className="wca-by-comp-root" style={{ marginTop: 16 }}>
      {sortedComps.length === 0 ? (<div style={{ color: 'var(--ant-color-text-tertiary)' }}>
          {intl.formatMessage({ id: 'wca.results.noValidData' })}
        </div>) : (<>
          {/* PR Streak 信息 */}
          <Card size="small" className="wca-pr-streak-card" style={{ marginBottom: 24 }} title={intl.formatMessage({ id: 'wca.results.prStreakTitle' })}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div>
                <strong>{intl.formatMessage({ id: 'wca.results.prStreakCurrent' })}：</strong>
                {prStreakData.currentStreak > 0
                ? intl.formatMessage({ id: 'wca.results.prStreakCount' }, { count: prStreakData.currentStreak })
                : intl.formatMessage({ id: 'wca.results.prStreakNone' })}
              </div>
              {prStreakData.longestStreak > 0 &&
                (prStreakData.currentStreak < sortedComps.length || prStreakData.breaks.length > 0) && (<div>
                    <strong>{intl.formatMessage({ id: 'wca.results.prStreakLongest' })}：</strong>
                    {intl.formatMessage({ id: 'wca.results.prStreakCount' }, { count: prStreakData.longestStreak })}
                  </div>)}
              {prStreakData.breaks.length > 0 && (<div>
                  <strong style={{ display: 'block', marginBottom: 8 }}>
                    {intl.formatMessage({ id: 'wca.results.prStreakBreaks' })}：
                  </strong>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {prStreakData.breaks.map((b, idx) => (<div key={idx} style={{
                        padding: '10px 12px',
                        ...
                        <div>
                          ...
                          <Tag color="green" style={{ marginLeft: 4 }}>
                            {intl.formatMessage({ id: 'wca.results.prStreakCount' }, { count: b.streakLength })}
                          </Tag>
          ...
          {sortedComps.map((comp, compIndex) => (<Card key={comp.compId} bordered={false} className="wca-by-comp-comp-card" style={{ marginBottom: 24 }} title={getCompTitle(comp)}>
            <Table className="wca-by-comp-table" columns={columns} dataSource={comp.results.map((r) => ({ ...r, compIndex }))} rowKey="round_id" pagination={false} size="small" bordered={false} showHeader={true}/>
          </Card>))}
        </>)}
    </div>);
};
export default WCAPlayerStaticsTabResultsWithComp;
//# sourceMappingURL=WCAPlayerStaticsTabResultsWithComps.jsx.map