...
const ResultDetailWithEvent = ({ eventID, wcaResults, comps, wcaRankTimer, }) => {
    const intl = useIntl();
    // 按 competition_id 分组
    ...
    Array.from(resultsByComp.entries()).forEach(([compId, results]) => {
        // 按 roundSortOrder 排序
        const sortedResults = results.sort((a, b) => (roundSortOrder[a.round_type_id] || 99) - (roundSortOrder[b.round_type_id] || 99));
    ...
    // 判断当前成绩是否为进步成绩
    const isProgress = (record, index, type) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (best, record, index) => {
                ...
                return (<Space direction="horizontal" size={4} style={{ display: 'flex', whiteSpace: 'nowrap' }}>
            <span style={{ color: progress ? 'red' : 'inherit', fontWeight: progress ? 600 : 400 }}>
              {resultsTimeFormat(best, record.event_id, false)}
            </span>
            {record.regional_single_record && (<Tag color={getRecordColor(record.regional_single_record)} style={{
                            margin: 0,
                            fontSize: '10px',
                            padding: '0 6px',
                            height: '18px',
    ...
    if (eventID !== '333mbf') {
        columns.push({
            ...
            <span style={{
              ...
              {record.regional_average_record && (<Tag color={getRecordColor(record.regional_average_record)} style={{
                            margin: 0,
                            fontSize: '10px',
                            padding: '0 6px',
                            height: '18px',
    ...
    if (dataSource.length === 0) {
        return (<div style={{ color: '#999', fontStyle: 'italic' }}>
    ...
    return (<>
      ...
      <Table columns={columns} dataSource={dataSource} rowKey={(record) => `${record.id}`} pagination={false} size="small" bordered={false} style={{ marginTop: 8 }}/>
    </>);
};
export default ResultDetailWithEvent;
//# sourceMappingURL=ResultDetailWithEvent.jsx.map