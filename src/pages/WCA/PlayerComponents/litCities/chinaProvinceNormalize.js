...
const CITY_OR_PROVINCE_FALLBACK = {
...
const CITY_ENGLISH_TO_PROVINCE_META = cityEnglishToProvinceJson.entries;
function metaFromEnglishPrefectureOrDistrictKey(name) {
    const t = name.trim();
    if (!t)
        return undefined;
...
export function metaByEnglishProvinceKey(key) {
    const k = key.trim();
    const direct = EN_PROVINCE_TO_META[k];
    if (direct)
        return direct;
    const alias = ALIAS_TO_KEY[normKey(k)];
    if (alias)
        return EN_PROVINCE_TO_META[alias];
    const lk = normKey(k);
    for (const enKey of Object.keys(EN_PROVINCE_TO_META)) {
        if (normKey(String(enKey)) === lk)
            return EN_PROVINCE_TO_META[enKey];
    }
    return undefined;
}
export function resolveChinaProvinceMeta(province, city) {
        ...
        if (m) {
            ...
            if (m.adcode === 140000 && c) {
                ...
                if (head) {
                    ...
                    if (inferred?.adcode === 610000)
                        return EN_PROVINCE_TO_META.Shaanxi;
                }
            }
            return m;
        }
...
/** 英文展示用（列表/非中文界面） */
export const MAP_NAME_ZH_TO_EN = {
    北京市: 'Beijing',
    天津市: 'Tianjin',
    河北省: 'Hebei',
    山西省: 'Shanxi',
...
export function displayProvinceName(mapNameZh, localeZh) {
    if (localeZh)
        return mapNameZh;
    return MAP_NAME_ZH_TO_EN[mapNameZh] || mapNameZh;
}
//# sourceMappingURL=chinaProvinceNormalize.js.map