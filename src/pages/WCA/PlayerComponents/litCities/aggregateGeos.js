...
export const WORLD_SCATTER_COORDS = {
    Taiwan: [121.3, 24.8],
    'Hong Kong': [114.17, 22.32],
    Macau: [113.54, 22.19],
};
export function hasVisitedChina(geos) {
    return geos.some(g => g.countryId === 'China');
}
export function aggregateWorldByCountry(geos) {
    const m = new Map();
    for (const g of geos) {
...
export function aggregateChinaProvinces(geos) {
    const m = new Map();
    for (const g of geos) {
        if (g.countryId !== 'China' && g.iso2 !== 'CN')
            continue;
...
export function filterGeosForProvince(geos, mapNameZh) {
...
export function splitWorldMapAndScatter(worldNameSet, byCountry) {
    const mapPieces = [];
    const scatterPieces = [];
...
//# sourceMappingURL=aggregateGeos.js.map