...
const CHINA_PAIRS = chinaTable.pairs;
const TAIWAN_PAIRS = taiwanTable.pairs;
function hasHan(s) {
    return /[\u4e00-\u9fff]/.test(s);
}
/** 别名命中：中文用包含；拉丁/数字别名用大小写不敏感包含 */
function haystackContainsAlias(haystack, alias) {
    const a = alias.trim();
    if (!a)
        return false;
    if (hasHan(a)) {
...
export function matchCityToPrefecture(city, prefectureNames, opts) {
    ...
    for (const n of prefectureNames) {
        if (trimmed.includes(n))
            return n;
        if (n.includes(trimmed) && trimmed.length >= 2)
            return n;
    }
    for (const hay of tryStrings) {
        for (const { alias, target } of pairs) {
            if (!set.has(target))
                continue;
            if (haystackContainsAlias(hay, alias))
                return target;
        }
    }
    return null;
}
export function aggregatePrefectureCounts(rows, prefectureNames, opts) {
    const m = new Map();
    let other = 0;
    for (const n of prefectureNames)
        m.set(n, 0);
    for (const row of rows) {
        ...
        if (hit)
            m.set(hit, (m.get(hit) || 0) + c);
        else
            other += c;
    }
    const out = prefectureNames.map(n => ({
        name: n,
        value: m.get(n) || 0,
    }));
    if (other > 0)
        out.push({ name: '__other__', value: other });
    return out;
}
//# sourceMappingURL=cityRegionMatch.js.map