...
export const PlayerEventResultPanel = ({ event, results, records, }) => {
    ...
    return (<div style={{ minWidth: 0, maxWidth: '100%' }}>
      ...
      {!m?.repeatedly && (<div style={{ marginBottom: 24, marginTop: 16 }}>
          <h2>
            <strong>统计</strong>
          </h2>
          <Tabs defaultActiveKey="success" items={[
                {
                    key: 'success',
                    label: '成功率',
...
const PlayerResultsListWithEvent = ({ events, results, records, comps, }) => {
    let items = [];
    ...
    return (<div style={{ minWidth: 0, maxWidth: '100%' }}>
      <NavTabs type="line" centered={true} items={items} tabsKey="player_result_result_list_with_events_tabs" indicator={{ size: (origin) => origin - 20, align: 'center' }}/>
    </div>);
};
export default PlayerResultsListWithEvent;
//# sourceMappingURL=PlayerResultsListWithEvent.jsx.map