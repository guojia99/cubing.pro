...
const { Option } = Select;
export const getQuantile = (sorted, p) => {
...
const RollingQuantileChart = ({ inputData, baseWindowSize = 50 }) => {
    ...
    const option = {
        ...
        yAxis: {
            ...
            inverse: false,
            axisLabel: {
                formatter: (value) => resultTimeString(value),
            },
        },
        dataZoom: [
            {
    ...
    return (<div>
      <div style={{ marginBottom: 16 }}>
        ...
        <Select value={windowSize} onChange={(value) => setWindowSize(value)} style={{ width: 120 }}>
          {[12, 50, 75, 100, 200].map((size) => (<Option key={size} value={size}>
              {size}
            </Option>))}
        </Select>
      </div>
      ...
      {(() => {
            ...
            return (<Card title="最新百分位成绩" style={{
                    marginTop: 16,
                    marginBottom: 16,
...
//# sourceMappingURL=RollingQuantileChart.jsx.map