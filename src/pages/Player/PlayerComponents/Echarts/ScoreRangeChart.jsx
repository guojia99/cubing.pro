...
const ScoreRangeChart = ({ inputData }) => {
    // 过滤掉负数
    ...
    // 动态生成区间范围
    const generateRanges = (data, interval) => {
        const ranges = [];
    ...
    return (<div>
      ...
      <Select id="interval-select" value={selectedInterval} onChange={(e) => setSelectedInterval(Number(e))} style={{ minWidth: 120 }}>
        {intervalOptions.map((option) => (<Select.Option key={option} value={option}>
            {option} 秒
          </Select.Option>))}
      </Select>
...
//# sourceMappingURL=ScoreRangeChart.jsx.map