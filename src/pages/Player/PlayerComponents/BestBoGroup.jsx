...
export const BOBox = ({ count, data, backgroundColor }) => {
    ...
    return (<Card bordered={false} hoverable={true} style={{
            width: 160,
            borderRadius: 12,
...
export const BOBestGroup = ({ data }) => {
    // 过滤掉负数后的有效成绩
    const validData = data.filter((n) => n >= 0);
    return (<Card style={{ marginTop: 24, marginBottom: 24 }}>
      <h3>
...
export const BORecentGroup = ({ data }) => {
...
export const ScoreRangeChart = ({ data, ranges }) => {
    // 统计每个区间的个数
    const rangeCounts = ranges.map(([min, max]) => data.filter((score) => score >= min && score < max).length);
    const option = {
        title: {
            text: '成绩区间统计',
            left: 'center',
...
//# sourceMappingURL=BestBoGroup.jsx.map