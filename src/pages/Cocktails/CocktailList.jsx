...
import { addCocktailFavorite, getCocktailFavorites, isCocktailFavorite, removeCocktailFavorite, } from './utils/cocktailFavoriteStorage';
import { addCocktailToTonight, getCocktailTonightList, isCocktailInTonight, removeCocktailFromTonight, } from './utils/cocktailTonightStorage';
import '@/pages/Recipes/index.less';
const COCKTAILS_JSON = '/iba/cocktails.json';
const MAX_FAVORITES = 20;
const MAX_TONIGHT = 20;
const PAGE_SIZE = 15;
const CocktailCard = ({ cocktail, onClick, isFav, onFavoriteChange, isInTonight, onTonightChange, inTonightMode }) => {
    ...
    return (<div className="recipe-card-wrapper" style={{ height: '100%' }}>
      <Card hoverable className="recipe-card" onClick={onClick} style={{
        ...
        {imgSrc ? (<div className="recipe-card-cover" style={{ flexShrink: 0 }}>
            <img src={imgSrc} alt={zh} loading="lazy" decoding="async" style={{ width: '100%', height: 350, objectFit: 'cover' }} onError={(e) => {
                e.target.src =
...
const CocktailList = () => {
    ...
    const baseList = listMode === 'favorites'
    ...
    return (<div className="recipe-list-page" style={{ padding: '16px 24px', maxWidth: 1600, margin: '0 auto' }}>
      <div style={{
        ...
        {listMode !== 'tonight' && (<Select placeholder="全部分类" allowClear style={{ width: 160, borderRadius: 8 }} value={filterCategory || undefined} onChange={(v) => setFilterCategory(v ?? '')} options={categorySelectOptions}/>)}
        <Segmented value={listMode === 'favorites' ? '我喜欢的' : listMode === 'tonight' ? '今晚喝什么' : '全部'} onChange={(v) => setListMode(v === '我喜欢的' ? 'favorites' : v === '今晚喝什么' ? 'tonight' : 'all')} options={[
            { label: '全部', value: '全部' },
            {
        ...
        <Button type="primary" icon={<SmileOutlined />} onClick={() => setPickModalOpen(true)} style={{ borderRadius: 8 }}>
          今晚喝什么
        </Button>
      </div>
      ...
      {listMode === 'tonight' ? (<div className="recipe-tonight-layout">
          <div className={`recipe-tonight-receipt-wrap ${receiptCollapsed ? 'collapsed' : ''}`}>
            <div className="recipe-tonight-receipt-inner">
              <CocktailTonightReceipt key={tonightVersion} cocktails={cocktails} tonightList={tonightRecipesList} onClear={refreshTonight}/>
            </div>
          ...
          <div className="recipe-tonight-recipes">
            {filtered.length === 0 ? (<Empty description="暂无酒款，去添加吧"/>) : (<div className="recipe-grid recipe-grid-responsive">
                {filtered.map((c) => (<CocktailCard key={c.slug} cocktail={c} onClick={() => handleCardClick(c)} isFav={isCocktailFavorite(c.slug)} onFavoriteChange={refreshFavorites} isInTonight={isCocktailInTonight(c.slug)} onTonightChange={refreshTonight} inTonightMode/>))}
              </div>)}
          </div>
        </div>) : filtered.length === 0 ? (<Empty description="暂无匹配的酒款"/>) : (<>
          <div className="recipe-grid recipe-grid-responsive">
            {paginated.map((c) => (<CocktailCard key={c.slug} cocktail={c} onClick={() => handleCardClick(c)} isFav={isCocktailFavorite(c.slug)} onFavoriteChange={refreshFavorites} isInTonight={isCocktailInTonight(c.slug)} onTonightChange={refreshTonight}/>))}
          </div>
          {totalPages > 1 && (<div style={{ marginTop: 24, display: 'flex', justifyContent: 'center' }}>
              <Pagination current={currentPage} total={filtered.length} pageSize={PAGE_SIZE} onChange={handlePageChange} showSizeChanger={false} showTotal={(total) => `共 ${total} 款`} style={{ marginBottom: 24 }}/>
            </div>)}
        </>)}

      <CocktailTonightPickModal open={pickModalOpen} onClose={() => setPickModalOpen(false)} cocktails={cocktails}/>
      <CocktailFirstVisitModal />
    </div>);
};
export default CocktailList;
//# sourceMappingURL=CocktailList.jsx.map