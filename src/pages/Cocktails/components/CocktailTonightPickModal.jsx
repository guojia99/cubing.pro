...
const CocktailTonightPickModal = ({ open, onClose, cocktails, }) => {
    ...
    const filteredCocktails = useMemo(() => cocktails.filter((c) => {
        if (filterCategories.length > 0 && !filterCategories.includes(c.category))
            return false;
        return true;
    }), [cocktails, filterCategories]);
    const getDisplayZh = (c) => getCocktailZhName(c.slug, c.name);
    const startSpin = () => {
    ...
    return (<Modal title="今晚喝什么" open={open} onCancel={onClose} footer={null} width={440} centered destroyOnClose styles={{ body: { paddingTop: 8 } }}>
      {phase === 'filter' && (<div className="today-pick-modal-filter">
          ...
          <Button type="primary" block size="large" onClick={startSpin} disabled={filteredCocktails.length === 0} style={{ borderRadius: 8 }}>
            开始随机
          </Button>
        </div>)}
      ...
      {(phase === 'spinning' || phase === 'result') && (<div className="today-pick-modal-result">
          ...
          {phase === 'result' && resultCocktail && (<div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
...
//# sourceMappingURL=CocktailTonightPickModal.jsx.map