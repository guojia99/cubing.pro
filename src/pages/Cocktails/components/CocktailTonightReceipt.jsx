...
import { clearCocktailTonightList, getCocktailTonightCheckedIngredients, toggleCocktailTonightIngredientChecked, } from '../utils/cocktailTonightStorage';
import '@/pages/Recipes/components/TonightReceipt.less';
const CocktailTonightReceipt = ({ cocktails, tonightList, onClear, }) => {
    ...
    const tonightCocktails = useMemo(() => {
        return tonightList.map((t) => cocktailMap.get(t.slug)).filter((c) => !!c);
    }, [tonightList, cocktailMap]);
    const ingredientsAggregated = useMemo(() => {
        const map = new Map();
        let orderIndex = 0;
    ...
    const [checkedIngredients, setCheckedIngredientsState] = useState(checkedSet);
    ...
    if (tonightList.length === 0) {
        return (<div className="tonight-receipt tonight-receipt-empty" style={{
    ...
    return (<>
      <div className="tonight-receipt" style={{
            background: '#fffef8',
        ...
        <div className="tonight-receipt-header">
          ...
          <div className="tonight-receipt-actions">
            <Button type="text" size="small" icon={<TagOutlined />} onClick={() => setShowTags(!showTags)} style={{
            color: showTags ? token.colorPrimary : token.colorTextTertiary,
            fontSize: 12,
        }}>
              {showTags ? '隐藏' : '显示'}酒名
            </Button>
            <Button type="text" size="small" danger icon={<DeleteOutlined />} onClick={() => setClearModalOpen(true)} style={{ fontSize: 12 }}>
              清空
            </Button>
          </div>
        </div>
        <div className="tonight-receipt-body">
          <div className="tonight-receipt-dishes">
            {tonightCocktails.map((c) => (<div key={c.slug} className="tonight-receipt-dish">
          ...
          <div className="tonight-receipt-ingredients">
            ...
            return (<div key={name} className="tonight-receipt-ingredient" style={{
                    textDecoration: checked ? 'line-through' : 'none',
                    color: checked ? token.colorTextTertiary : token.colorText,
                }}>
                  <Checkbox checked={checked} onChange={() => handleToggle(name)} style={{ marginRight: 8 }}/>
                  <span>
                    {name}
                    {count > 1 && (<span style={{ color: token.colorPrimary, marginLeft: 4 }}>
                        ({count})
                      </span>)}
                    {showTags && drinkNames.length > 0 && (<span className="tonight-receipt-tag" style={{ marginLeft: 6 }}>
                        [{drinkNames.join('、')}]
                      </span>)}
                  </span>
                </div>);
      ...
      <Modal title="清空今晚喝什么" open={clearModalOpen} onCancel={() => setClearModalOpen(false)} onOk={handleClearAll} okText="确认清空" cancelText="取消" okButtonProps={{ danger: true }}>
        <p>确定要清空「今晚喝什么」清单吗？已勾选的备料项也会被清除。</p>
      </Modal>
    </>);
};
export default CocktailTonightReceipt;
//# sourceMappingURL=CocktailTonightReceipt.jsx.map