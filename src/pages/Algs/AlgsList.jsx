...
import { exportPracticeConfig, importPracticeConfig, } from '@/services/cubing-pro/algs/practiceConfigExport';
...
const AlgsList = () => {
    ...
    const handleFileChange = (e) => {
        ...
        reader.onload = () => {
            ...
            if (result.success) {
                ...
                message.error(intl.formatMessage({ id: 'algs.practiceConfig.importFailed' }, { msg: result.message ?? '' }));
            }
        };
    ...
    const handleCardClick = (cube, cls) => {
    ...
    return (<div className="algs-page-container" style={{ padding: '16px 0' }}>
      <h2 style={{ marginBottom: 16, color: 'rgba(0,0,0,0.85)' }}>
        {intl.formatMessage({ id: 'algs.title' })}
      </h2>
      {loggedIn ? (<UserCloudKvPanel kvKey={USER_KV_KEYS.algorithm_config} title={intl.formatMessage({ id: 'algs.cloud.title' })} description={intl.formatMessage({ id: 'algs.cloud.desc' })} uploadButtonText={intl.formatMessage({ id: 'algs.cloud.upload' })} syncButtonText={intl.formatMessage({ id: 'algs.cloud.sync' })} uploadOkText={intl.formatMessage({ id: 'algs.cloud.uploadOk' })} syncOkText={intl.formatMessage({ id: 'algs.cloud.syncOk' })} getPayloadForUpload={() => {
      ...
      <div style={{ marginBottom: 16, display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12 }}>
        <Select placeholder={intl.formatMessage({ id: 'algs.filter.placeholder' })} allowClear style={{ minWidth: 180 }} value={filterCube} onChange={(v) => setFilterCube(v ?? '')} options={[
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 32 }}>
        <Col xs={24} sm={12} md={8} lg={6} xl={6}>
          <Card hoverable onClick={() => setRandomPickModalOpen(true)} style={{
            borderRadius: 12,
            ...
            <div style={{ textAlign: 'center' }}>
              <SvgRenderer svg={RANDOM_PICK_ICON_SVG} maxWidth={100} maxHeight={100} style={{ marginTop: 24, marginBottom: 28 }}/>
      ...
      {filteredCubes.flatMap((c) => classMap[c] ?? []).length === 0 ? (<Empty description={intl.formatMessage({ id: 'algs.empty' })}/>) : (<>
        <RandomPickModal open={randomPickModalOpen} onClose={() => setRandomPickModalOpen(false)} classMap={classMap} cubeKeys={filteredCubes}/>
        {filteredCubes
                .filter((cube) => (classMap[cube] ?? []).length > 0)
                .map((cube) => {
                const classes = classMap[cube] ?? [];
                return (<div key={cube} style={{ marginBottom: 32 }}>
              ...
              <Row gutter={[16, 16]}>
                {classes.map((cls) => (<Col key={`${cube}-${cls.name}`} xs={24} sm={12} md={8} lg={6} xl={6}>
                    <Card hoverable onClick={() => handleCardClick(cube, cls)} style={{
                            borderRadius: 12,
                            backgroundColor: ALGS_COLORS.cardBg,
                            borderColor: ALGS_COLORS.cardBorder,
                            overflow: 'hidden',
                      ...
                      <div style={{ textAlign: 'center' }}>
                        <SvgRenderer svg={cls.image} maxWidth={180} maxHeight={260} style={{ marginTop: 24, marginBottom: 28 }}/>
...
//# sourceMappingURL=AlgsList.jsx.map