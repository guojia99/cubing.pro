...
export async function exportAlgsPdf(element, cube, classId, onProgress) {
    const report = (p) => {
    ...
    const cards = element.querySelectorAll('[data-algs-group-card]');
    if (cards.length === 0) {
        report(10);
        ...
        for (let i = 0; i < cards.length; i++) {
            ...
            doc.addImage(canvas.toDataURL('image/png'), 'PNG', PAGE_MARGIN_LEFT, currentY, contentW, cardH);
            currentY += cardH + 8;
