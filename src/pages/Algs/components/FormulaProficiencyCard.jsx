...
const FormulaProficiencyCard = ({ cube, classId, flatAlgs, onOpenFormulaPractice, refreshKey = 0, }) => {
    const intl = useIntl();
    ...
    const handleProficiencyChange = useCallback((formulaKey, level) => {
        setFormulaProficiency(cube, classId, formulaKey, level);
        setProficiencyMap((prev) => ({ ...prev, [formulaKey]: level }));
    }, [cube, classId]);
    const markedCount = useMemo(() => {
        return Object.keys(proficiencyMap).filter((key) => flatAlgs.some((f) => buildFormulaKey(f.setName, f.groupName, f.alg.name) === key)).length;
    ...
    const groupedBySetAndGroup = useMemo(() => {
        ...
        for (const item of flatAlgs) {
            if (!map[item.setName])
                map[item.setName] = {};
            if (!map[item.setName][item.groupName])
                map[item.setName][item.groupName] = [];
    ...
    const collapseItems = useMemo(() => {
        return Object.entries(groupedBySetAndGroup).map(([setName, groups]) => ({
          ...
          {intl.formatMessage({ id: 'algs.detail.set' })}: {setName}
            ...
            children: (<Collapse size="small" ghost defaultActiveKey={Object.keys(groups).map((gn) => `${setName}-${gn}`)} items={Object.entries(groups).map(([groupName, items]) => ({
                    key: `${setName}-${groupName}`,
                    label: (<span>
                {intl.formatMessage({ id: 'algs.detail.group' })}: {groupName} ({items.length})
              </span>),
                    children: (<div style={{ display: 'flex', flexDirection: 'column', gap: 6, padding: '4px 0' }}>
                {items.map((item) => {
                      ...
                      <ProficiencySelect value={level} onChange={(v) => handleProficiencyChange(key, v)} size="small"/>
                    </div>);
                        })}
              </div>),
                }))}/>),
      ...
      <Card size="small" style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--ant-color-text)' }}>
            ...
            {markedCount > 0 && (<span style={{ fontSize: 12, color: 'var(--ant-color-text-tertiary)' }}>
                ({markedCount})
              </span>)}
          </div>
        </div>
        ...
        {onOpenFormulaPractice && (<div style={{ marginTop: 6, fontSize: 11, color: 'rgba(100,149,237,0.9)' }}>
            {intl.formatMessage({ id: 'algs.proficiencyCard.practiceHint' })}
          </div>)}
      </Card>

      <Modal open={modalOpen} onCancel={() => setModalOpen(false)} title={intl.formatMessage({ id: 'algs.proficiencyCard.modalTitle' })} footer={null} width={760} styles={{ body: { maxHeight: 560, overflowY: 'auto' } }}>
...
//# sourceMappingURL=FormulaProficiencyCard.jsx.map