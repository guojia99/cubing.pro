...
const FormulaRandomPickCard = ({ cube, classId, flatAlgs, onOpenRandom, onOpenHistory, onPickFormula, }) => {
    const intl = useIntl();
    const history = useMemo(() => getFormulaPickHistory(cube, classId), [cube, classId]);
    const latestPick = history[0];
    const findIndex = (setName, groupName, algName) => flatAlgs.findIndex((f) => f.setName === setName &&
    ...
    return (<Card size="small" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
        <div style={{
          ...
          {latestPick ? (<div onClick={handleLatestClick} style={{
                display: 'flex',
                alignItems: 'center',
                gap: 12,
                cursor: 'pointer',
...
//# sourceMappingURL=FormulaRandomPickCard.jsx.map