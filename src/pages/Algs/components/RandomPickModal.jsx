...
import { clearDailyPick, getDailyPickState, getRemainingPicks, saveDailyPick, } from '@/services/cubing-pro/algs/dailyRandomPick';
...
function collectOptions(classMap, cubeKeys, exclude) {
    const all = [];
...
const RandomPickModal = ({ open, onClose, classMap, cubeKeys, }) => {
    const intl = useIntl();
    ...
    const allOptions = useMemo(() => collectOptions(classMap, cubeKeys, picks), [classMap, cubeKeys, picks]);
    const totalOptions = allOptions.length;
    const canSpin = totalOptions > 0 && remaining > 0;
    const doSpin = useCallback(() => {
        if (spinning || remaining <= 0 || allOptions.length === 0)
            return;
        ...
        const duration = SPIN_DURATION_MIN +
    ...
    const handleOptionClick = (opt) => {
        navigate(`/algs/${encodeURIComponent(opt.cube)}/${encodeURIComponent(opt.className)}`);
        onClose();
    };
    const handleClose = () => {
        setPhase('idle');
    ...
    const wheelSvg = useMemo(() => {
        ...
        const pathForSlice = (startDeg, endDeg) => {
            const p1 = pointAt(startDeg);
            const p2 = pointAt(endDeg);
            const large = endDeg - startDeg > 180 ? 1 : 0;
            return `M ${cx} ${cy} L ${p1.x} ${p1.y} A ${r} ${r} 0 ${large} 1 ${p2.x} ${p2.y} Z`;
        };
        return (<svg viewBox="0 0 200 200" className="random-pick-wheel-svg">
        <g className="random-pick-wheel-rotatable">
          {wheelSegments.map((seg, i) => {
                ...
                return (<g key={`${seg.cube}-${seg.className}-${i}`}>
                <path d={pathForSlice(startDeg, endDeg)} fill={color.bg} stroke="rgba(255,255,255,0.6)" strokeWidth="1"/>
                <text x={textX} y={textY} textAnchor="middle" dominantBaseline="middle" className="random-pick-wheel-label" transform={`rotate(${midDeg + 90}, ${textX}, ${textY})`}>
    ...
    return (<Modal open={open} onCancel={handleClose} footer={null} width={460} centered destroyOnClose className="random-pick-modal">
      <div className="random-pick-content">
        {phase === 'idle' && (<>
            <div className="random-pick-wheel-wrap">
              <div className="random-pick-wheel random-pick-wheel-placeholder">
                <span className={`random-pick-wheel-placeholder-text ${countdown !== null ? 'random-pick-countdown' : ''}`}>
            ...
            <p className="random-pick-hint">
              {countdown !== null
                ? intl.formatMessage({ id: 'algs.randomPick.countdown' }, { count: countdown })
                : intl.formatMessage({ id: 'algs.randomPick.hintStart' })}
            </p>
            {countdown === null && (<Button type="primary" size="large" onClick={doSpin} disabled={spinning || !canSpin} className="random-pick-btn">
        ...
        {(phase === 'spinning' || phase === 'result') && wheelSegments.length > 0 && (<>
            ...
            {phase === 'spinning' && (<p className="random-pick-hint">
        ...
        {phase === 'result' && (<>
            ...
            <div className={`random-pick-options ${picks.length === 1 ? 'single' : 'double'}`}>
              {picks.map((opt) => (<div key={`${opt.cube}-${opt.className}`} className="random-pick-option-card" onClick={() => handleOptionClick(opt)}>
                  <SvgRenderer svg={opt.image} maxWidth={120} maxHeight={160} style={{ marginBottom: 8 }}/>
            ...
            {remaining > 0 && (<Button type="default" size="middle" onClick={doSpin} disabled={spinning} className="random-pick-again-btn">
                {intl.formatMessage({ id: 'algs.randomPick.again' })}
              </Button>)}
            {remaining <= 0 && (<p className="random-pick-limit-hint">
                {intl.formatMessage({ id: 'algs.randomPick.limitReached' })}
              </p>)}
            {isLocal() && (<Button type="link" size="small" danger onClick={handleClear} className="random-pick-clear-btn">
...
//# sourceMappingURL=RandomPickModal.jsx.map