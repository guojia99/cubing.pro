...
import { getAlgsSelection, setAlgsSelection, buildAlgsKey, getCube333ViewMode, setCube333ViewMode, getTwistyPanelTone, setTwistyPanelTone, getTwistyBottomFaceColor, setTwistyBottomFaceColor, } from '../utils/storage';
import { getFormulaProficiency, setFormulaProficiency, } from '@/services/cubing-pro/algs/formulaPracticeProficiency';
...
const AlgsModal = ({ open, onClose, cube, classId, items, currentIndex, onNavigate, }) => {
    const intl = useIntl();
    const { token } = theme.useToken();
    const item = items[currentIndex];
    const formulaKey = item ? buildFormulaKey(item.setName, item.groupName, item.alg.name) : '';
    const [proficiency, setProficiency] = useState(() => item ? (getFormulaProficiency(cube, classId)[formulaKey] ?? 'average') : 'average');
    const [cube333ViewMode, setCube333ViewModeState] = useState(() => getCube333ViewMode());
    const [twistyPanelTone, setTwistyPanelToneState] = useState(() => getTwistyPanelTone());
    const [twistyBottomFace, setTwistyBottomFaceState] = useState('yellow');
    useEffect(() => {
        if (item) {
    ...
    const handleProficiencyChange = useCallback((level) => {
        if (!formulaKey)
            return;
    ...
    return (<Modal open={open} onCancel={onClose} footer={null} width={700} title={<Space>
          <span>{alg.name}</span>
          {items.length > 1 && (<span style={{ fontSize: 14, fontWeight: 400, color: 'var(--ant-color-text-tertiary)' }}>
              ({currentIndex + 1} / {items.length})
            </span>)}
        </Space>} styles={{ body: { paddingTop: 8 } }}>
      <div style={{ textAlign: 'center' }}>
        {showTwisty333 ? (<>
            <div style={{ marginBottom: 8 }}>
              <Space wrap align="center" size={[8, 8]}>
                <Button size="small" onClick={() => {
                const next = cube333ViewMode === '2d' ? '3d' : '2d';
                ...
                <Space size={4} align="center">
                  ...
                  <Select size="small" style={{ minWidth: 112 }} value={twistyPanelTone} onChange={(v) => {
                setTwistyPanelToneState(v);
                setTwistyPanelTone(v);
            }} options={['cream', 'lightBlue', 'white', 'neutral'].map((v) => ({
                value: v,
                ...
                <Space size={4} align="center">
                  ...
                  <Select size="small" style={{ minWidth: 88 }} value={twistyBottomFace} onChange={(v) => {
                setTwistyBottomFaceState(v);
                setTwistyBottomFaceColor(cube, classId, v);
            }} options={['yellow', 'white', 'red', 'orange', 'green', 'blue'].map((v) => ({
                value: v,
            ...
            <AlgsTwisty333 alg={displayFormula} viewMode={cube333ViewMode} experimentalStickering={twistyStickering} panelTone={twistyPanelTone} bottomFaceColor={twistyBottomFace} style={{ marginTop: 0, marginBottom: 20 }}/>
          </>) : (<SvgRenderer svg={alg.image} maxHeight={180} style={{ marginTop: 12, marginBottom: 20 }}/>)}

        {currentScramble && (<div style={{ marginBottom: 16, textAlign: 'left' }}>
        ...
        <div style={{ marginBottom: 12, textAlign: 'left' }}>
          ...
          <div style={{
            ...
            {alg.algs.map((formula, idx) => (<div key={idx} onClick={() => hasMultiple && handleIndexChange(idx)} style={{
                padding: 12,
                /* 选中：绿色；未选中：用 token 主色浅底/边框，深色模式下不会过亮 */
                background: selectedIndex === idx ? 'rgba(82, 196, 26, 0.2)' : token.colorPrimaryBg,
                border: `2px solid ${selectedIndex === idx ? 'rgb(82, 196, 26)' : token.colorPrimaryBorder}`,
                borderRadius: 8,
                fontFamily: 'monospace',
                fontSize: 16,
                color: 'var(--ant-color-text)',
        ...
        <Space style={{ marginTop: 20, width: '100%', justifyContent: 'space-between' }}>
          <Button icon={<LeftOutlined />} disabled={!canPrev} onClick={() => onNavigate?.(currentIndex - 1)}>
            {intl.formatMessage({ id: 'algs.modal.prev' })}
          </Button>
          <Button type="primary" onClick={onClose}>
            {intl.formatMessage({ id: 'algs.modal.close' })}
          </Button>
          <Button icon={<RightOutlined />} disabled={!canNext} onClick={() => onNavigate?.(currentIndex + 1)}>
...
//# sourceMappingURL=AlgsModal.jsx.map