...
import { getFormulaPickHistory, saveFormulaPick, } from '@/services/cubing-pro/algs/formulaRandomPick';
...
const FormulaRandomPickModal = ({ open, onClose, mode, cube, classId, flatAlgs, onPickFormula, onPickSuccess, }) => {
    const intl = useIntl();
    ...
    const wheelSegmentsForWheel = useMemo(() => wheelSegments.map((s) => ({
        label: `${s.groupName} - ${s.alg.name}`,
        key: `${s.setName}-${s.groupName}-${s.alg.name}`,
    })), [wheelSegments]);
    const handlePickClick = (item) => {
        const idx = flatAlgs.findIndex((f) => f.setName === item.setName &&
    ...
    if (mode === 'history') {
        return (<Modal open={open} onCancel={handleClose} title={intl.formatMessage({ id: 'algs.formulaRandom.historyTitle' })} footer={null} width={400} className="formula-random-pick-modal formula-random-history-modal">
        <div className="formula-random-history-list">
          {history.length === 0 ? (<p className="formula-random-history-empty">
              {intl.formatMessage({ id: 'algs.formulaRandom.historyEmpty' })}
            </p>) : (history.map((item, i) => (<div key={`${item.setName}-${item.groupName}-${item.algName}-${i}`} className="formula-random-history-item" onClick={() => handleHistoryItemClick(item)}>
                <SvgRenderer svg={item.image} maxWidth={60} maxHeight={80} style={{ flexShrink: 0 }}/>
    ...
    return (<Modal open={open} onCancel={handleClose} footer={null} width={460} centered destroyOnClose className="formula-random-pick-modal random-pick-modal">
      <div className="random-pick-content">
        {phase === 'idle' && (<>
            <RandomWheel segments={[]} spinParams={null} placeholder="?" className="formula-random-wheel"/>
            <p className="random-pick-hint">
              {intl.formatMessage({ id: 'algs.formulaRandom.hintStart' })}
            </p>
            <Button type="primary" size="large" onClick={doSpin} disabled={spinning} className="random-pick-btn">
              {intl.formatMessage({ id: 'algs.formulaRandom.start' })}
            </Button>
          </>)}

        {(phase === 'spinning' || phase === 'result') && wheelSegments.length > 0 && (<>
            <RandomWheel segments={wheelSegmentsForWheel} spinParams={phase === 'spinning' ? spinParams : null} onSpinStart={() => setSpinParams(null)} className="formula-random-wheel"/>
        ...
        {phase === 'result' && winner && (<>
            ...
            <div className="formula-random-result-card formula-random-option-card" onClick={() => handlePickClick(winner)}>
              <SvgRenderer svg={winner.alg.image} maxWidth={120} maxHeight={160} style={{ marginBottom: 8 }}/>
            ...
            <Button type="default" size="middle" onClick={doSpin} disabled={spinning} className="random-pick-again-btn">
...
//# sourceMappingURL=FormulaRandomPickModal.jsx.map