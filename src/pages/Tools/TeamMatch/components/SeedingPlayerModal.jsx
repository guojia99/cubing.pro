...
function fmtSnapshot(s) {
    if (!s || (s.single === null && s.average === null))
        return '—';
    return `单次 ${fmt(s.single)} · 平均 ${fmt(s.average)}`;
}
function inferAdoptStrategy(e) {
    if (e.adoptStrategy)
        return e.adoptStrategy;
    if (e.activeSource === 'manual')
        return 'manual';
    return 'best_average';
}
const SeedingPlayerModal = ({ open, onClose, player, eventId, entry, onSave }) => {
    const [single, setSingle] = useState(null);
    const [average, setAverage] = useState(null);
    const [activeSource, setActiveSource] = useState('manual');
    const [adoptStrategy, setAdoptStrategy] = useState('best_average');
    const [wcaBest, setWcaBest] = useState(null);
    const [oneBest, setOneBest] = useState(null);
    const [preliminary, setPreliminary] = useState({ single: null, average: null });
    ...
    return (<Modal title={title} open={open && !!player && !!entry} onCancel={onClose} onOk={handleOk} width={700} destroyOnClose>
      {player && entry && (<Space direction="vertical" style={{ width: '100%' }} size="middle">
          ...
          <div>
            ...
            <div style={{ marginTop: 8 }}>
              <Space wrap align="center">
                ...
                {preliminary.single === 'DNF' ? (<Typography.Text type="danger">DNF</Typography.Text>) : (<InputNumber min={0} step={0.01} style={{ width: 120 }} value={typeof preliminary.single === 'number' ? preliminary.single : undefined} onChange={(v) => setPreliminary((p) => ({ ...p, single: v === null ? null : Number(v) }))}/>)}
                {preliminary.single !== 'DNF' && (<Button type="link" size="small" onClick={() => setPreliminary((p) => ({ ...p, single: 'DNF' }))}>
                    单次 DNF
                  </Button>)}
              </Space>
            </div>
            <div style={{ marginTop: 8 }}>
              <Space wrap align="center">
                ...
                {preliminary.average === 'DNF' ? (<Typography.Text type="danger">DNF</Typography.Text>) : (<InputNumber min={0} step={0.01} style={{ width: 120 }} value={typeof preliminary.average === 'number' ? preliminary.average : undefined} onChange={(v) => setPreliminary((p) => ({ ...p, average: v === null ? null : Number(v) }))}/>)}
                {preliminary.average !== 'DNF' && (<Button type="link" size="small" onClick={() => setPreliminary((p) => ({ ...p, average: 'DNF' }))}>
                    平均 DNF
                  </Button>)}
              </Space>
            </div>
          ...
          <div>
            ...
            <Radio.Group style={{ display: 'block', marginTop: 8 }} value={adoptStrategy} onChange={(e) => onStrategyChange(e.target.value)}>
          ...
          <div>
            <Space wrap align="center" style={{ marginBottom: 8 }}>
              ...
              {adoptStrategy === 'best_average' && (<Typography.Text type="secondary" style={{ fontSize: 12 }}>
                  （策略：自动最佳平均）
                </Typography.Text>)}
            </Space>
            <div>
              <Typography.Text type="secondary">单次（秒）</Typography.Text>
              {single === 'DNF' ? (<Space style={{ marginTop: 8, display: 'flex' }}>
                  <Typography.Text type="danger">DNF</Typography.Text>
                  <Button type="link" size="small" onClick={() => { setSingle(null); markOfficialManual(); }}>
                    改填数字
                  </Button>
                </Space>) : (<InputNumber style={{ width: '100%', marginTop: 8 }} min={0} step={0.01} value={typeof single === 'number' ? single : undefined} onChange={(v) => {
                    setSingle(v === null ? null : Number(v));
                    markOfficialManual();
                }}/>)}
              {single !== 'DNF' && (<Button type="link" size="small" style={{ paddingLeft: 0 }} onClick={() => { setSingle('DNF'); markOfficialManual(); }}>
                  设为 DNF
                </Button>)}
            </div>
            <div style={{ marginTop: 12 }}>
              <Typography.Text type="secondary">平均（秒）</Typography.Text>
              {average === 'DNF' ? (<Space style={{ marginTop: 8, display: 'flex' }}>
                  <Typography.Text type="danger">DNF</Typography.Text>
                  <Button type="link" size="small" onClick={() => { setAverage(null); markOfficialManual(); }}>
                    改填数字
                  </Button>
                </Space>) : (<InputNumber style={{ width: '100%', marginTop: 8 }} min={0} step={0.01} value={typeof average === 'number' ? average : undefined} onChange={(v) => {
                    setAverage(v === null ? null : Number(v));
                    markOfficialManual();
                }}/>)}
              {average !== 'DNF' && (<Button type="link" size="small" style={{ paddingLeft: 0 }} onClick={() => { setAverage('DNF'); markOfficialManual(); }}>
                  设为 DNF
                </Button>)}
            </div>
          </div>
...
//# sourceMappingURL=SeedingPlayerModal.jsx.map