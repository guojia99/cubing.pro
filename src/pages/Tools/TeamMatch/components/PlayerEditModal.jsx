...
const PlayerEditModal = ({ open, onClose, editing, schools, onSave }) => {
    ...
    const initialWcaIdRef = React.useRef('');
    const wcaIdWatch = Form.useWatch('wcaId', form);
    const wcaIdLen = (wcaIdWatch?.trim()?.length ?? 0);
    useEffect(() => {
        if (!open)
            return;
        if (editing) {
    ...
    const handleFetchWcaAvatar = async () => {
        const wca = form.getFieldValue('wcaId')?.trim() ?? '';
        if (wca.length !== 10) {
            message.warning('请先填写 10 位 WCA ID');
            return;
        }
        if (wcaAvatarFetchedOnce)
            return;
    ...
    return (<Modal title={editing ? '编辑选手' : '新增选手'} open={open} onCancel={onClose} width={480} footer={<Space>
      ...
      <Form form={form} layout="vertical">
        ...
        <Form.Item name="wcaId" label="WCA ID（可选）">
          <Input placeholder="10 位" maxLength={10} onChange={(e) => {
            const v = e.target.value.trim();
            if (v !== initialWcaIdRef.current)
                setWcaAvatarFetchedOnce(false);
        }}/>
        </Form.Item>
        ...
        <Form.Item label="头像">
          <Space direction="vertical" size="small" style={{ width: '100%' }}>
            <AvatarUpload value={avatarDraft} onChange={(url) => {
            setAvatarDraft(url);
            if (!url)
                setWcaAvatarFetchedOnce(false);
            ...
            <Space wrap align="center">
              <Button size="small" loading={wcaAvatarLoading} disabled={wcaAvatarFetchedOnce || wcaIdLen !== 10} onClick={() => void handleFetchWcaAvatar()}>
...
//# sourceMappingURL=PlayerEditModal.jsx.map