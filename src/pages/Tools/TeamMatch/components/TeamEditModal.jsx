...
const TeamEditModal = ({ open, onClose, editing, schools, players, usedPlayerIds, activeTeamCount, onSave, }) => {
    const [form] = Form.useForm();
    const playerOptions = useMemo(() => players.map((p) => ({
    ...
    const handleOk = () => {
        void form.validateFields().then((v) => {
            ...
            if (!getFreelancerSchoolId(schools)) {
                const autoFl = { id: uuidv4(), name: '自由人', kind: 'freelancer' };
            ...
            if (v.teamMode === 'school' && c.kind !== 'school') {
                message.warning('当前队员组合不符合学校队（需至少两名队员来自同一普通学校，且不能三人三校各一人）；请改为自由人队或调整队员');
                return;
            }
    ...
    return (<Modal title={editing ? '编辑队伍' : '添加队伍'} open={open} onCancel={onClose} width={560} footer={<Space>
      ...
      <Form form={form} layout="vertical" initialValues={{ teamMode: 'school' }}>
        ...
        <Form.Item name="p0" label="队员 1" rules={[{ required: true, message: '请选择队员' }]}>
          <Select showSearch optionFilterProp="label" options={playerOptions} placeholder="选择选手（不限学校）"/>
        </Form.Item>
...
//# sourceMappingURL=TeamEditModal.jsx.map