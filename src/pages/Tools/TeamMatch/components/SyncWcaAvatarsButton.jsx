import { playersWithWcaId, syncWcaAvatarsForPlayers, } from '@/pages/Tools/TeamMatch/syncWcaAvatars';
...
const SyncWcaAvatarsButton = ({ session, dispatch }) => {
    ...
    const run = async () => {
        ...
        setHistory([]);
        try {
            const { players, updatedCount } = await syncWcaAvatarsForPlayers(session.players, (done, total) => setPercent(Math.min(100, Math.round((done / total) * 100))), {
                signal: ac.signal,
            ...
            message.success(updatedCount > 0 ? `已更新 ${updatedCount} 人头像` : '已拉取完毕（头像无变化或 WCA 无头像）');
        }
        catch (e) {
    ...
    return (<>
      ...
      <Modal title="正在同步 WCA 头像" open={modalOpen} width={560} maskClosable={false} closable={false} keyboard={false} footer={<Button onClick={cancel} disabled={!busy}>
            取消
...
//# sourceMappingURL=SyncWcaAvatarsButton.jsx.map