...
2\t陈诚骏\t7735
3\t蔡镇宇\t10954`;
const BulkOneUidImportModal = ({ session, dispatch, disabled }) => {
    ...
    const apply = () => {
        ...
        const doCommit = () => {
            ...
            const parts = [`已更新 ${applied.length} 人的 One ID`];
            if (unmatched.length > 0)
                parts.push(`文档中 ${unmatched.length} 行姓名不在当前名单，已忽略`);
            if (batchDuplicateNameSkipped.length > 0) {
                parts.push(`粘贴内重名 ${batchDuplicateNameSkipped.length} 行仅取首次`);
            }
            if (unchanged.length > 0)
                parts.push(`${unchanged.length} 人无需变更`);
        ...
        if (needConfirm && applied.length > 0) {
            ...
            {batchDuplicateNameSkipped.length > 0 && (<Typography.Paragraph type="secondary" style={{ marginTop: 12 }}>
    ...
    return (<>
      ...
      <Modal title="同步 One ID" open={open} onCancel={() => setOpen(false)} width={720} footer={null} destroyOnClose styles={{ body: { paddingTop: 12 } }}>
        <Typography.Paragraph type="secondary" style={{ marginBottom: 8 }}>
        ...
        <Input.TextArea rows={18} value={text} onChange={(e) => setText(e.target.value)} placeholder={EXAMPLE} style={{ fontFamily: 'monospace', fontSize: 13 }}/>
...
//# sourceMappingURL=BulkOneUidImportModal.jsx.map