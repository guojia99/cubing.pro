...
async function cropToDataUrl(image, crop) {
...
const AvatarUpload = ({ value, onChange }) => {
    ...
    return (<div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        {value ? (<img alt="" src={value} style={{ width: 48, height: 48, borderRadius: 8, objectFit: 'cover' }}/>) : (<div style={{ width: 48, height: 48, borderRadius: 8, background: '#f0f0f0' }}/>)}
        ...
        {value && (<Button size="small" danger type="link" onClick={() => onChange(null)}>
            清除
          </Button>)}
      </div>
      <Modal title="裁剪头像" open={open} onOk={() => void handleOk()} onCancel={() => {
            setOpen(false);
            setSrc(null);
        }} destroyOnClose>
        {src && (<ReactCrop crop={crop} onChange={(_, p) => setCrop(p)} onComplete={(c) => setPixelCrop(c)} aspect={1}>
...
//# sourceMappingURL=AvatarUpload.jsx.map