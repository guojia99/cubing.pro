...
import PkGroupArenaFullscreen from '@/pages/Tools/TeamMatch/components/PkGroupArenaFullscreen';
...
function ElimMatchCard({ m, teams, onOpenArena }) {
    const done = !!m.winnerId;
    const clickable = !!m.pk;
    return (<Card size="small" className={classNames('tmBracketMatchCard', 'tmElimMatchCard', {
            tmMatchDone: done,
      ...
      <div className={classNames('tmMatchTeams', 'tmBracketMatchTeams', 'tmElimMatchTeams')}>
        {m.teamIds.map((tid, idx) => {
            ...
            return (<React.Fragment key={tid}>
              {idx > 0 && <div className={classNames('tmVs', 'tmBracketVs', 'tmElimGroupSep')}>·</div>}
              <div className={classNames('tmBracketTeamLine', {
                    tmLoser: lose,
                    tmBracketWinner: win,
                })}>
                <BracketFitTeamName className="tmBracketTeamName" text={t?.name ?? '—'} winner={win} tone="elim"/>
              </div>
            </React.Fragment>);
        })}
      </div>
...
const EliminationStage = ({ liveSettings, onOpenLiveSettings }) => {
    ...
    const groupArenaColumns = useMemo(() => {
        if (!focused?.pk?.teamIds?.length)
            return [];
    ...
    if (!elim?.matches.length) {
        return (<Typography.Paragraph type="secondary" style={{ marginBottom: 0 }}>
        请先在「淘汰赛抽签」完成随机分组。
      </Typography.Paragraph>);
    }
    return (<div className="tmBracketRoot tmElimRoot">
      ...
      <div className="tmElimWaves">
        {waves.map((row, wi) => (<div key={wi} className="tmElimWaveBlock">
            <Typography.Title level={5} className="tmElimWaveTitle">
              {waves.length > 1 ? `第 ${wi + 1} 波 · ` : ''}
              {row.length} 场小组
            </Typography.Title>
            <div className="tmElimWaveRow">
              {row.map((m) => (<ElimMatchCard key={m.id} m={m} teams={teams} onOpenArena={openArenaForMatch}/>))}
            </div>
          </div>))}
      </div>

      {arenaOpen && focused?.pk && groupArenaColumns.length > 0 && (<PkGroupArenaFullscreen key={fxKey} open={arenaOpen} onClose={() => setArenaOpen(false)} onOpenLiveSettings={onOpenLiveSettings} settings={liveSettings.arena} columns={groupArenaColumns} currentResults={focused.pk.currentResults} onOpenScoreEntry={() => setPkOpen(true)}/>)}

      <PkGroupModal open={pkOpen && !!focused} onClose={() => setPkOpen(false)} match={focused} teams={teams} players={players} draft={pkDraft} onDraftChange={setPkDraft} modalTitle="预选赛 · 小组录入" onSettle={() => {
...
//# sourceMappingURL=EliminationStage.jsx.map