...
import PkArenaHeroSlideOver from '@/pages/Tools/TeamMatch/components/PkArenaHeroSlideOver';
...
function PkArenaFullscreen({ open, onClose, onOpenLiveSettings, settings, teamA, teamB, playersA, playersB, dimA, dimB, currentResults, onOpenScoreEntry, }) {
    const [heroOpen, setHeroOpen] = useState(false);
    const [heroAnchor, setHeroAnchor] = useState('left');
    ...
    return (<div className="tmPkArenaFs">
      ...
      <div className="tmPkArenaFsBody">
        <div className="tmPkArenaFsScale" style={{
            transform: `scale(${settings.scale})`,
            transformOrigin: 'center center',
        }}>
          <div className="tmPkArenaFsRow">
            <div className={classNames('tmPkArenaFsSide', 'tmPkArenaFsSideLeft', { tmPkArenaFsDim: dimA })} style={{ background: `linear-gradient(180deg, ${settings.stripLeft}, transparent)` }}>
              ...
              <div className="tmPkArenaFsSideContent tmPkArenaFsSideContent--left">
                <Typography.Text strong className="tmPkArenaFsTeamName" style={{ color: settings.teamNameColor, fontSize: settings.teamNameFontPx }}>
                  {teamA?.name ?? '待定'}
                </Typography.Text>
                <PkArenaTeamBattlecry battlecry={teamA?.battlecry} settings={settings} align="left"/>
                <div className="tmPkDiagCol tmPkDiagColLeft">
                  {playersA.slice(0, 3).map((p, i) => (<div key={p.id} role="button" tabIndex={0} className="tmPkDiagCell tmPkDiagCellClickable" style={{ marginLeft: i * step }} onClick={() => openHeroA(p)} onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    openHeroA(p);
                }
            }}>
                      {p.avatarDataUrl ? (<img alt="" src={p.avatarDataUrl} className="tmPkArenaFsAvatar" style={{ width: av, height: av }}/>) : (<div className="tmPkArenaFsAvatar tmPkArenaFsAvatarPh" style={{ width: av, height: av, fontSize: av * 0.38 }}>
                          {p.name.slice(0, 1)}
                        </div>)}
                      <span className="tmPkArenaFsPlayerName" style={{ color: settings.playerNameColor, fontSize: settings.playerNameFontPx, maxWidth: av + step * 4 }}>
                        {p.name}
                      </span>
                      <span className="tmPkArenaFsPlayerMeta" style={{
                color: metaColor,
                fontSize: metaFont,
                maxWidth: av + step * 5,
                lineHeight: 1.3,
                      ...
                      <span className="tmPkArenaFsPlayerMeta tmPkArenaFsPlayerSchool" style={{
                color: metaColor,
                ...
                <PkArenaTeamCarouselBtn corner="bottom-left" running={carousel?.side === 'A'} disabled={!teamA || playersA.length === 0} onClick={toggleCarouselA}/>
              </div>
            </div>

            <div className="tmPkArenaFsCenter">
              <Typography.Title level={1} className="tmPkArenaFsPkTitle" style={{ color: settings.pkTitleColor, fontSize: settings.pkTitleFontPx, margin: 0 }}>
                PK
              </Typography.Title>
            </div>

            <div className={classNames('tmPkArenaFsSide', 'tmPkArenaFsSideRight', { tmPkArenaFsDim: dimB })} style={{ background: `linear-gradient(180deg, ${settings.stripRight}, transparent)` }}>
              ...
              <div className="tmPkArenaFsSideContent tmPkArenaFsSideContent--right">
                ...
                <div className="tmPkDiagCol tmPkDiagColRight">
                  {playersB.slice(0, 3).map((p, i) => (<div key={p.id} role="button" tabIndex={0} className="tmPkDiagCell tmPkDiagCellClickable" style={{ marginRight: i * step }} onClick={() => openHeroB(p)} onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    openHeroB(p);
                }
                ...
                <PkArenaTeamCarouselBtn corner="bottom-right" running={carousel?.side === 'B'} disabled={!teamB || playersB.length === 0} onClick={toggleCarouselB}/>
              </div>
            </div>
          </div>
        </div>
      </div>
      <PkArenaCarouselProgress active={!!carousel && heroOpen} intervalMs={carouselInterval} tick={progressTick}/>
...
//# sourceMappingURL=PkArenaFullscreen.jsx.map