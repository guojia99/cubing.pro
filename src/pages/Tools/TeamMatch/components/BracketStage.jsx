...
import { bracketSettingsToCssVars } from '@/pages/Tools/TeamMatch/liveUiSettings';
import { formatPlayerSingleAverageLine, schoolNameForPlayer, seedingEntryForPlayer, } from '@/pages/Tools/TeamMatch/playerBracketDisplay';
...
function computeTeamMedals(rounds, bronzeMatch, skipBronzeMatch) {
    const out = {};
...
function MedalMark({ kind }) {
    ...
    return (<span className={classNames('tmBracketMedal', `tmBracketMedal-${kind}`)} title={title} aria-label={title}>
      {kind === 'gold' ? '🥇' : kind === 'silver' ? '🥈' : '🥉'}
    </span>);
}
function BracketMatchCard({ m, teams, onOpenArena, compact, teamMedals }) {
    if (!m) {
        return (<Card size="small" className="tmBracketMatchCard tmBracketMatchCardEmpty tmBracketMatchCardDisabled">
    ...
    return (<Card size="small" className={classNames('tmBracketMatchCard', {
            tmMatchDone: done,
            tmBracketMatchCardCompact: compact,
            tmBracketMatchCardClickable: clickable,
            tmBracketMatchCardDisabled: !clickable,
        })} title={<span className="tmBracketMatchId">{m.id}</span>} onClick={() => {
            if (clickable)
                onOpenArena(m.id);
        }}>
      <div className={classNames('tmMatchTeams', 'tmBracketMatchTeams')}>
        <div className={classNames('tmBracketTeamLine', {
            tmLoser: loseA,
            tmBracketWinner: winA,
        })}>
          <BracketFitTeamName className="tmBracketTeamName" text={ta?.name ?? '—'} compact={compact} winner={winA}/>
          {medalA ? <MedalMark kind={medalA}/> : null}
          {m.byeWinnerId === m.teamAId ? (<span className="tmBracketByeHint">{done ? '轮空晋级' : '待决出'}</span>) : null}
        </div>
        ...
        <div className={classNames('tmBracketTeamLine', {
          ...
          <BracketFitTeamName className="tmBracketTeamName" text={tb?.name ?? '—'} compact={compact} winner={winB}/>
          {medalB ? <MedalMark kind={medalB}/> : null}
          {m.byeWinnerId === m.teamBId ? (<span className="tmBracketByeHint">{done ? '轮空晋级' : '待决出'}</span>) : null}
        </div>
      </div>
    </Card>);
}
/** 两场首轮垂直叠放，晋级场垂直居中于二者之间（淘汰赛树对齐） */
function BracketPairRow({ leafA, leafB, advance, mirror, }) {
    return (<div className={classNames('tmBracketPairRow', { tmBracketPairRowMirror: mirror })}>
...
const BracketStage = ({ liveSettings, onOpenLiveSettings }) => {
    ...
    const findMatch = useCallback((id) => {
        if (!id)
            return null;
        if (bronzeMatch?.id === id)
            return bronzeMatch;
    ...
    const teamMedals = useMemo(() => computeTeamMedals(rounds, bronzeMatch, skipBronzeMatch), [rounds, bronzeMatch, skipBronzeMatch]);
    const bracketCssVars = useMemo(() => bracketSettingsToCssVars(liveSettings.bracket), [liveSettings.bracket]);
    const renderMatch = (ri, mi, compact) => (<BracketMatchCard m={rounds[ri]?.[mi]} teams={teams} teamMedals={teamMedals} onOpenArena={openArenaForMatch} compact={compact}/>);
    const openArenaBronze = (id) => {
        openArenaForMatch(id);
    };
    const focusedPlayersA = useMemo(() => {
        if (!focused?.pk?.teamAId)
            return [];
        return (teams
    ...
    return (<div className="tmBracketRoot" style={bracketCssVars}>
      <div className="tmBracketToolbar">
        <Space size="small" wrap align="center">
          <Button type="default" ghost size="small" icon={<DownloadOutlined />} loading={exportingPng} onClick={handleExportBracketPng}>
            导出 PNG
          </Button>
        ...
        <Space size="middle" wrap align="center" className="tmBracketToolbarExtras">
          <Space size={8} align="center">
            <Switch size="small" checked={skipBronzeMatch} onChange={(v) => dispatch({ type: 'SET_SKIP_BRONZE_MATCH', value: v })}/>
      ...
      <div ref={bracketExportRef} className="tmBracketExportBlock">
        {session.name?.trim() ? (<div className="tmBracketExportTitleWrap">
            <Typography.Text className="tmBracketExportTitle">{session.name}</Typography.Text>
          </div>) : null}
        <div className="tmBracketScaleWrap" style={{ transform: `scale(${liveSettings.bracket.scale})`, transformOrigin: 'top center' }}>
        ...
        <div className="tmBracketLrRow">
          ...
          <div className="tmBracketWing tmBracketWingLeft">
            ...
            <div className="tmBracketWingCluster">
              <div className="tmBracketWingClusterLeaves">
                <BracketPairRow leafA={renderMatch(0, 0, true)} leafB={renderMatch(0, 1, true)} advance={renderMatch(1, 0, true)}/>
                <BracketPairRow leafA={renderMatch(0, 2, true)} leafB={renderMatch(0, 3, true)} advance={renderMatch(1, 1, true)}/>
              </div>
          ...
          <div className="tmBracketCenterColumn">
            ...
            <div className="tmBracketCenterBronze">
              ...
              <div className="tmBracketFinalSlot">
                {skipBronzeMatch ? (<Card size="small" className="tmBracketMatchCard tmBracketMatchCardEmpty tmBracketSkipBronzeCard">
                  ...
                  </Card>) : bronzeMatch ? (<BracketMatchCard m={bronzeMatch} teams={teams} teamMedals={teamMedals} onOpenArena={openArenaBronze}/>) : (<Card size="small" className="tmBracketMatchCard tmBracketMatchCardEmpty">
                    <Typography.Text className="tmBracketEmptyCardPrimary">待定</Typography.Text>
          ...
          <div className="tmBracketWing tmBracketWingRight">
            ...
            <div className="tmBracketWingCluster">
              ...
              <div className="tmBracketWingClusterLeaves">
                <BracketPairRow mirror leafA={renderMatch(0, 4, true)} leafB={renderMatch(0, 5, true)} advance={renderMatch(1, 2, true)}/>
                <BracketPairRow mirror leafA={renderMatch(0, 6, true)} leafB={renderMatch(0, 7, true)} advance={renderMatch(1, 3, true)}/>
              </div>
            </div>
          </div>
        </div>
      ...
      {arenaOpen && focused?.pk && focused.teamAId && focused.teamBId && (<PkArenaFullscreen key={fxKey} open={arenaOpen} onClose={() => setArenaOpen(false)} onOpenLiveSettings={onOpenLiveSettings} settings={liveSettings.arena} teamA={teams.find((t) => t.id === focused.pk.teamAId)} teamB={teams.find((t) => t.id === focused.pk.teamBId)} playersA={focusedPlayersA} playersB={focusedPlayersB} dimA={!!focused.winnerId && focused.winnerId !== focused.pk.teamAId} dimB={!!focused.winnerId && focused.winnerId !== focused.pk.teamBId} currentResults={focused.pk.currentResults} onOpenScoreEntry={openPkModal}/>)}

      <PkModal open={pkOpen && !!focused} onClose={closePk} match={focused} teams={teams} players={players} draft={pkDraft} onDraftChange={setPkDraft} onSettle={() => {
            ...
            closePk();
        }} onReplay={() => dispatch({ type: 'PK_REPLAY', matchId: focused.id })} onClearCurrent={() => dispatch({ type: 'PK_CLEAR_CURRENT', matchId: focused.id })}/>
    </div>);
};
export default BracketStage;
//# sourceMappingURL=BracketStage.jsx.map