import { getDefaultBracketPageSettings, getDefaultLiveUISettings, } from '@/pages/Tools/TeamMatch/liveUiSettings';
...
function colorFieldToString(v) {
    if (typeof v === 'string')
        return v;
    if (v && typeof v === 'object' && 'toRgbString' in v && typeof v.toRgbString === 'function') {
        return v.toRgbString();
    }
    return String(v ?? '');
}
const ARENA_COLOR_KEYS = [
    'backgroundColor',
    'backgroundColorEnd',
    'glowLeft',
    'glowRight',
...
function normalizeLiveForm(raw) {
    ...
    if (raw.arena) {
        Object.keys(arena).forEach((k) => {
            const v = raw.arena[k];
            if (v === undefined)
                return;
            if (ARENA_COLOR_KEYS.includes(k))
                arena[k] = colorFieldToString(v);
            else
                arena[k] = v;
        });
    }
    if (raw.bracket) {
        Object.keys(bracket).forEach((k) => {
            const v = raw.bracket[k];
            if (v === undefined)
                return;
            if (BRACKET_COLOR_KEYS.includes(k))
                bracket[k] = colorFieldToString(v);
            else
                bracket[k] = v;
        });
    }
...
const LiveSettingsModal = ({ open, onClose, value, onApply }) => {
    ...
    return (<Modal title="正赛与对战设置" open={open} onCancel={onClose} width={600} footer={null} destroyOnClose styles={{
            body: {
      ...
      <Form form={form} layout="vertical" size="small" initialValues={value} onFinish={(vals) => {
            onApply(normalizeLiveForm(vals));
        }}>
        ...
        <Tabs items={[
            ...
            },
            {
                ...
                children: (<>
                  ...
                  <Form.Item label="队员轮播间隔 (秒)" name={['arena', 'heroCarouselIntervalMs']} tooltip="队伍栏角「队员轮播」时，每人英雄弹窗停留时长；底部细条为倒计时进度。" getValueProps={(ms) => {
                        ...
                        const d = getDefaultPkArenaSettings().heroCarouselIntervalMs;
                        if (sec === null || sec === undefined || Number.isNaN(Number(sec)))
                            return d;
...
//# sourceMappingURL=LiveSettingsModal.jsx.map