...
import { fetchOneCommonCompList } from '@/pages/Tools/TeamMatch/oneGradeApi';
...
const OneCompPrelimImportCard = ({ session, dispatch }) => {
    const imp = session.oneCompImport ?? { cId: null, eRound: DEFAULT_E_ROUND };
    const [cIdDraft, setCIdDraft] = useState(imp.cId);
    ...
    return (<>
      <Card size="small" type="inner" title="初赛导入" style={{ marginBottom: 12 }}>
        <Space wrap align="center">
          <span>比赛ID</span>
          <InputNumber min={1} value={cIdDraft ?? undefined} onChange={(v) => {
          ...
          <span>轮次</span>
          <InputNumber min={1} max={20} value={eRoundDraft} onChange={(v) => {
      ...
      <Modal title="初赛导入预览（确认后写入）" open={previewOpen} onCancel={() => setPreviewOpen(false)} width={900} destroyOnClose footer={<Space>
        ...
        <Tabs items={previewEvents.map((ev) => ({
            key: ev.eventId,
            label: eventLabel(ev.eventId),
            children: (<Table size="small" pagination={false} rowKey={(r) => r.player.id} dataSource={ev.playerRows} scroll={{ y: 320 }} columns={[
                    ...
                    },
                    {
                        ...
                        render: (_, r) => {
                            if (r.status === 'matched')
                                return <Tag color="success">已参加初赛</Tag>;
                            if (r.status === 'no_one_id')
                                return <Tag>无 One ID</Tag>;
                    ...
                    },
                    {
                        title: '初赛单次',
                        width: 100,
                        render: (_, r) => r.preliminary?.single === 'DNF'
                            ? 'DNF'
                    ...
                    },
                    {
                        title: '初赛平均',
                        width: 100,
                        render: (_, r) => r.preliminary?.average === 'DNF'
                            ? 'DNF'
                    ...
                    },
                    {
                        title: '平台 u_id / 姓名',
                        ellipsis: true,
                        render: (_, r) => r.row ? `${r.row.u_id}${r.row.u_name ? ` · ${r.row.u_name}` : ''}` : '—',
                    },
                ]}/>),
        }))}/>
        {previewUnassigned.length > 0 && (<>
            ...
            <Table size="small" pagination={{ pageSize: 8 }} rowKey={(r) => `${r.eventId}-${r.eidOne}-${r.u_id}`} dataSource={previewUnassigned} columns={[
      ...
      <Modal title="选比赛" open={listOpen} onCancel={() => setListOpen(false)} width={720} footer={null} destroyOnClose>
        <Input value={nameFilter} onChange={(e) => setNameFilter(e.target.value)} style={{ marginBottom: 12 }} allowClear/>
        <Table size="small" rowKey="c_id" pagination={{ pageSize: 10 }} dataSource={filteredList} scroll={{ y: 360 }} columns={[
            ...
            { title: '日期', dataIndex: 'c_date', width: 120 },
            {
                ...
                key: 'act',
                width: 72,
                render: (_, row) => (<Button type="link" size="small" onClick={() => pickComp(row)}>
                  选择
                </Button>),
            },
        ]}/>
...
//# sourceMappingURL=OneCompPrelimImportCard.jsx.map