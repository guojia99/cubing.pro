...
function PkGroupArenaFullscreen({ open, onClose, onOpenLiveSettings, settings, columns, currentResults, onOpenScoreEntry, }) {
    ...
    const openHero = useCallback((colIdx, col, p) => {
        const anchor = colIdx % 2 === 0 ? 'left' : 'right';
        setCarousel(null);
        setHeroAnchor(anchor);
        setHeroData(buildPkArenaHeroDetail(toHeroInput(p), col.team, currentResults));
        setHeroOpen(true);
    }, [currentResults]);
    const toggleCarousel = useCallback((colIdx) => {
    ...
    return (<div className="tmPkArenaFs tmPkArenaFsGroup">
      <div className="tmPkArenaFsBg" style={bgStyle} aria-hidden/>
      <div className="tmPkArenaFsTopBar tmPkArenaFsGroupTopBar" style={{ background: settings.barBg }}>
      ...
      <div className="tmPkArenaFsBody">
        <div className="tmPkArenaFsScale" style={{
          ...
          <div className="tmPkArenaFsRow tmPkArenaFsGroupRow">
            ...
            return (<React.Fragment key={col.team.id}>
                  {colIdx > 0 && (<div className="tmPkArenaFsGroupSep" aria-hidden>
                      <span className="tmPkArenaFsGroupSepText">·</span>
                    </div>)}
                  <div className={classNames('tmPkArenaFsSide', 'tmPkArenaFsGroupSide', { tmPkArenaFsDim: col.dim })} style={{
                    background: `linear-gradient(180deg, ${colIdx % 2 === 0 ? settings.stripLeft : settings.stripRight}, transparent)`,
                }}>
                    <PkArenaTeamWaves enabled={settings.flagBackgroundEnabled} direction={colIdx % 2 === 0 ? 'left' : 'right'}/>
                    <div className="tmPkArenaFsSideContent tmPkArenaFsSideContent--group">
                      ...
                      <div className="tmPkDiagCol tmPkDiagColGroup">
                        {col.players.map((p, i) => (<div key={p.id} role="button" tabIndex={0} className="tmPkDiagCell tmPkArenaFsGroupCell tmPkDiagCellClickable" style={{ marginLeft: (i - midCol) * step }} onClick={() => openHero(colIdx, col, p)} onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                            ...
                            <span className="tmPkArenaFsPlayerName" style={{
                      ...
                      <PkArenaTeamCarouselBtn corner="bottom-center" running={carousel?.colIdx === colIdx} disabled={col.players.length === 0} onClick={() => toggleCarousel(colIdx)}/>
                    </div>
                  </div>
                </React.Fragment>);
        })}
...
//# sourceMappingURL=PkGroupArenaFullscreen.jsx.map