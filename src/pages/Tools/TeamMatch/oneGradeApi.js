...
export const ONE_EID_TO_WCA_EVENT = {
    1: '333',
    2: '222',
    3: '444',
    4: '555',
...
/** 数字 uid（字符串，仅数字） */
export async function fetchOneUserGrades(uid) {
    const id = uid.trim();
    if (!/^\d+$/.test(id))
        throw new Error('invalid one uid');
...
export function pickBestOneGradeForEvent(rows, eventId) {
    const eid = wcaEventIdToOneEid(eventId);
    if (eid === null)
        return { single: null, average: null };
...
export async function fetchOneCommonCompList(page = 1, size = 100) {
...
export async function fetchOneCompGrades(cId, eId, eRound) {
...
//# sourceMappingURL=oneGradeApi.js.map