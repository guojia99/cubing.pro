export function getFreelancerSchoolId(schools) {
 ...
 */
export function classifyTeamComposition(playerIds, players, schools) {
    const flSchoolId = getFreelancerSchoolId(schools);
    const plist = playerIds.map((id) => players.find((p) => p.id === id)).filter(Boolean);
    if (plist.length !== 3)
        return { ok: false, message: '请选择 3 名选手' };
    const isFl = (sid) => flSchoolId !== undefined && sid === flSchoolId;
...
/** 默认队名 */
export function defaultTeamName(kind, schoolId, schools) {
    if (kind === 'freelancer') {
 ...
 */
export function schoolIdForDrawPairing(session, teamId) {
    const t = session.teams.find((x) => x.id === teamId);
    if (!t)
        return undefined;
    if (t.kind === 'freelancer')
        return t.id;
    return t.schoolId;
}
export function teamKindLabel(team, schools) {
...
//# sourceMappingURL=teamClassify.js.map