...
export const MOCK_ONE_UIDS = [2572, 2533, 2556, 1002, 7052];
/** 测试用学校名（与业务约定一致） */
 ...
 */
export const MOCK_TEAM_PLAN_16 = [
...
const MOCK_TEAM_PLAN_8 = MOCK_SCHOOL_NAMES.slice(0, MIN_TEAMS).map((school) => ({ school, teamLabel: '一队' }));
function buildFromPlan(plan) {
    const nameToSchoolId = new Map();
    const schools = [];
    for (const row of plan) {
        if (!nameToSchoolId.has(row.school)) {
            const s = { id: uuidv4(), name: row.school, kind: 'standard' };
...
export function appendMockGroups(session, count) {
    ...
    const existingNames = new Map(session.schools.map((s) => [s.name, s]));
    const newSchools = [];
    ...
    for (const row of plan) {
        if (!nameToId.has(row.school)) {
            ...
            if (ex) {
                ...
                const sch = { id: uuidv4(), name: row.school, kind: 'standard' };
                newSchools.push(sch);
...
//# sourceMappingURL=mockLocal.js.map