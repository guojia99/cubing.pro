...
function entryFor(seeding, playerId, eventId) {
    return seeding.find((e) => e.playerId === playerId && e.eventId === eventId);
}
function valuePrimary(e, primary) {
    if (!e)
        return null;
...
export function teamSeedingSum(team, players, seeding, eventId, primary) {
    let sum = 0;
    for (const pid of team.playerIds) {
        const e = entryFor(seeding, pid, eventId);
        const v = valuePrimary(e, primary);
        if (v === null)
            return { sum: 0, valid: false };
        if (v === 'DNF')
            return { sum: 0, valid: false };
        sum += v;
    }
...
export function pickSeedTeamIds(teams, players, seeding, eventId, primary, 
 ...
 * 其余按有效数字成绩之和升序（越小越好）。
 */
export function teamSeedingSortMeta(team, _players, seeding, eventId, primary) {
...
export function sortTeamsBySeedingRank(teams, players, seeding, eventId, primary) {
    ...
    const cmpFull = (a, b) => {
        if (a.badSlotCount !== b.badSlotCount)
            return a.badSlotCount - b.badSlotCount;
        if (a.sumNumeric !== b.sumNumeric)
            return a.sumNumeric - b.sumNumeric;
 ...
 */
export function rankedBracketTeamIds(teams, players, seeding, eventId, primary) {
...
export function allRankedActiveTeamIds(teams, players, seeding, eventId, primary) {
    return sortTeamsBySeedingRank(teams, players, seeding, eventId, primary)
        .filter((t) => !t.disabled && t.playerIds.length === TEAM_PLAYERS)
        .map((t) => t.id);
}
//# sourceMappingURL=seedingMath.js.map