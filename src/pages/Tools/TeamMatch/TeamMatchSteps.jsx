...
import { BRACKET_TEAM_COUNT, MAX_ELIMINATION_BYE_TEAMS, MAX_ROSTER_TEAMS, MIN_TEAMS, TEAM_PLAYERS, WIZARD_STEP_MAIN_DRAW, WIZARD_STEP_MAIN_LIVE, WIZARD_STEP_MAIN_POOL_CONFIRM, } from '@/pages/Tools/TeamMatch/types';
import { Avatar, Button, Card, Checkbox, Form, Input, Modal, Popconfirm, Radio, Select, Space, Switch, Table, Tag, Typography, message, } from 'antd';
import React, { useEffect, useMemo, useState } from 'react';
import { v4 as uuidv4 } from 'uuid';
/* 子步骤组件定义在文件后部，运行时在渲染前已完成初始化 */
/* eslint-disable @typescript-eslint/no-use-before-define */
const TeamMatchSteps = ({ liveUISettings, onLiveUISettingsChange }) => {
    ...
    const activeTeamCount = useMemo(() => session.teams.filter((t) => !t.disabled && t.playerIds.length === TEAM_PLAYERS).length, [session.teams]);
    const playerModal = (<PlayerEditModal open={playerModalOpen} onClose={closePlayerModal} editing={editingPlayer} schools={session.schools} onSave={savePlayerFromModal}/>);
    return (<>
      {playerModal}
      {step === 0 && (<Card title="1. 学校与选手">
          ...
          <Form layout="inline" form={schoolForm} initialValues={{ kind: 'standard' }} onFinish={(v) => {
                ...
                const s = { id: uuidv4(), name: displayName, kind: v.kind };
          ...
          <Table style={{ marginTop: 16 }} size="small" rowKey="id" dataSource={session.schools} pagination={false} columns={[
                { title: '名称', dataIndex: 'name' },
                {
                    title: '类型',
                    render: (_, r) => (r.kind === 'freelancer' ? '自由人池' : '普通学校'),
                },
                {
                    title: '操作',
                    render: (_, r) => (<Button type="link" danger size="small" onClick={() => dispatch({
                            type: 'UPSERT_SCHOOLS',
          ...
          <Table size="small" rowKey="id" dataSource={session.players} pagination={false} columns={[
                { title: '姓名', dataIndex: 'name' },
                {
                    title: '学校',
                    render: (_, r) => session.schools.find((s) => s.id === r.schoolId)?.name,
                },
                { title: 'WCA', dataIndex: 'wcaId' },
                { title: 'One', dataIndex: 'oneId' },
                {
                    title: '头像',
                    render: (_, r) => r.avatarDataUrl ? (<img alt="" src={r.avatarDataUrl} style={{ width: 36, height: 36, borderRadius: 6, objectFit: 'cover' }}/>) : (<span style={{ color: '#999' }}>—</span>),
                },
                {
                    title: '操作',
                    render: (_, r) => (<Space>
                    ...
                    <Button type="link" danger size="small" onClick={() => dispatch({
                            type: 'UPSERT_PLAYERS',
      ...
      {step === 1 && (<StepTeam session={session} dispatch={dispatch} activeTeamCount={activeTeamCount} openPlayerModal={openPlayerModal}/>)}

      {step === 2 && (<StepScoreEntry session={session} dispatch={dispatch} eventId={eventId}/>)}

      {step === 3 && (<StepTeamConfirm session={session} dispatch={dispatch} eventId={eventId}/>)}

      {step === 4 && <StepElimDraw session={session} dispatch={dispatch}/>}
      {step === 5 && (<>
          <Card title="淘汰赛" className="tmElimStageCard">
            <EliminationStage liveSettings={liveUISettings} onOpenLiveSettings={() => setElimLiveSettingsOpen(true)}/>
            <div style={{ marginTop: 20 }}>
              <Button type="primary" disabled={!isEliminationComplete(session)} onClick={() => {
          ...
          <LiveSettingsModal open={elimLiveSettingsOpen} onClose={() => setElimLiveSettingsOpen(false)} value={liveUISettings} onApply={(next) => {
...
function formatTeamAverageSumLine(team, session) {
...
function schoolTagLabelForByeTeam(team, session) {
...
const StepElimDraw = ({ session, dispatch }) => {
    useEffect(() => {
    ...
    const [byeDraft, setByeDraft] = useState([]);
    const rankedTeams = useMemo(() => sortTeamsBySeedingRank(session.teams, session.players, session.seeding, eventId, session.seedingPrimary), [session.teams, session.players, session.seeding, eventId, session.seedingPrimary]);
    useEffect(() => {
        if (byeOpen && elim)
    ...
    const toggleBye = (teamId, checked) => {
        if (checked) {
    ...
    const waveBlocks = useMemo(() => {
        if (!elim?.matches.length || !elim.waveSizes.length)
            return [];
        const out = [];
        let o = 0;
    ...
    const previewPoolCount = elim && elim.drawVersion >= 1 ? computeMainBracketTeamIds(session).length : null;
    if (!over16) {
        return (<Card title="淘汰赛抽签">
        <Typography.Paragraph type="secondary">
    ...
    return (<Card title="淘汰赛抽签">
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <Radio.Group style={{ marginLeft: 12 }} value={elim?.groupSize ?? 3} onChange={(e) => dispatch({ type: 'SET_ELIM_GROUP_SIZE', value: e.target.value })} options={[
      ...
      <Space wrap style={{ marginBottom: 16 }}>
        ...
        <Popconfirm title="跳过淘汰赛？" description={`将直接按成绩排名前 ${BRACKET_TEAM_COUNT} 进入正赛名单确认，不进行预选赛 PK。`} onConfirm={() => {
            dispatch({ type: 'SKIP_ELIMINATION_TO_MAIN_DRAW' });
            dispatch({ type: 'SET_WIZARD_STEP', step: WIZARD_STEP_MAIN_POOL_CONFIRM });
            message.success('已跳过预选赛，请确认正赛名单');
        }} okText="确定跳过" cancelText="取消">
          <Button>跳过选择</Button>
        </Popconfirm>
      </Space>
      {previewPoolCount !== null && elim && !elim.skipped && (<Typography.Paragraph type="secondary" style={{ marginTop: 12 }}>
          当前晋级池按规则可得 <strong>{previewPoolCount}</strong> 支正赛名额（至多 {BRACKET_TEAM_COUNT} 支抽签）。
        </Typography.Paragraph>)}

      {elim && elim.drawVersion >= 1 && waveBlocks.length > 0 && (<div style={{ marginTop: 20 }}>
          <Typography.Title level={5}>分组预览（每标签 = 一场多队同场小组）</Typography.Title>
          {waveBlocks.map((row, wi) => (<div key={wi} style={{ marginBottom: 16 }}>
              <Typography.Text type="secondary" style={{ display: 'block', marginBottom: 8 }}>
                {waveBlocks.length > 1 ? `第 ${wi + 1} 波 · ` : ''}
                {row.length} 场小组
              </Typography.Text>
              <Space wrap>
                {row.map((m) => (<Tag key={m.id}>
                    {m.teamIds
          ...
          {(elim.naturalByeTeamIds?.length > 0 || elim.naturalByeTeamId) && (<Typography.Text type="warning">
              无法成组（仅 1 队）、直接进入晋级池：
      ...
      <Modal title={`保送队伍选择（${byeDraft.length}/${MAX_ELIMINATION_BYE_TEAMS}）`} open={byeOpen} onCancel={() => setByeOpen(false)} onOk={() => {
        ...
        <div style={{ maxHeight: 360, overflow: 'auto' }}>
          {rankedTeams.map((t) => {
            ...
            if (!full) {
                return (<div key={t.id} style={{ padding: '6px 0', borderBottom: '1px solid #f0f0f0' }}>
            ...
            return (<div key={t.id} style={{ padding: '8px 0', borderBottom: '1px solid #f0f0f0' }}>
                <Checkbox checked={byeDraft.includes(t.id)} disabled={!byeDraft.includes(t.id) && byeDraft.length >= MAX_ELIMINATION_BYE_TEAMS} onChange={(e) => toggleBye(t.id, e.target.checked)}>
...
function firstRoundPairsFromFlat(flat) {
    const pairs = [];
    for (let i = 0; i < 8; i++) {
        pairs.push({
            index: i,
...
const StepDraw = ({ session, dispatch }) => {
    const eventIdDraw = session.eventIds[0] ?? '333';
    const bracketEligibleCount = useMemo(() => {
        const fromMain = session.mainBracketTeamIds;
        if (fromMain && fromMain.length >= MIN_TEAMS)
            return fromMain.length;
        return rankedBracketTeamIds(session.teams, session.players, session.seeding, eventIdDraw, session.seedingPrimary).length;
    }, [
    ...
    const flat = useMemo(() => session.flatSlots ??
        [
    ...
    const findTeam = (id) => (id ? session.teams.find((x) => x.id === id) ?? null : null);
    const renderSide = (tid, slotLabel) => {
        const t = findTeam(tid);
        const seed = t?.isSeed;
        return (<div className={`tmDrawPairSide ${seed ? 'tmDrawPairSideSeed' : ''}`}>
        <Space direction="vertical" size={2} style={{ width: '100%' }}>
          <Space wrap size={6}>
            <Typography.Text strong ellipsis>
              {t ? t.name : '轮空'}
            </Typography.Text>
            {t?.isSeed && (<Tag color="success" style={{ marginInlineEnd: 0 }}>
                种子
              </Tag>)}
          </Space>
          {t && (<Typography.Text type="secondary" style={{ fontSize: 11 }} ellipsis>
    ...
    return (<Card title="正赛抽签 · 分区与抽签">
      <Space wrap align="center" style={{ marginBottom: 12 }}>
        <Typography.Text type="secondary">随机方式：</Typography.Text>
        <Radio.Group value={session.drawRandomMode} onChange={(e) => dispatch({ type: 'SET_DRAW_RANDOM_MODE', mode: e.target.value })} options={[
            { label: '按成绩分区（与种子强弱相关）', value: 'score' },
            { label: '完全随机（不参考成绩）', value: 'pure' },
        ]}/>
      </Space>
      <div style={{ marginBottom: 12 }}>
        <Checkbox checked={session.drawAvoidSameSchool} onChange={(e) => dispatch({ type: 'SET_DRAW_AVOID_SAME_SCHOOL', value: e.target.checked })}>
      ...
      <Space wrap>
        <Button type="primary" onClick={() => {
            if (bracketEligibleCount < MIN_TEAMS) {
                message.warning(`按成绩排名进入正赛至少 ${MIN_TEAMS} 支队伍（满编未缺席中取前 ${BRACKET_TEAM_COUNT} 名），当前 ${bracketEligibleCount} 支`);
                return;
            }
      ...
      <div className="tmDrawGrid">
        {flat.map((tid, i) => {
            ...
            const seed = !!t?.isSeed;
            return (<div key={i} className={`tmDrawCell ${!tid ? 'tmDrawCellEmpty' : ''} ${tid && seed ? 'tmDrawCellSeed' : ''}`}>
              {t ? (<>
                  <Space wrap size={4}>
                    <Typography.Text strong>{t.name}</Typography.Text>
                    {t.isSeed && (<Tag color="success" style={{ margin: 0 }}>
                        种子
                      </Tag>)}
                  </Space>
      ...
      {session.drawVersion >= 1 && (<div style={{ marginTop: 20 }}>
          <Typography.Title level={5} style={{ marginTop: 0, marginBottom: 12 }}>
            首轮对阵（共 8 场）
          </Typography.Title>
          {firstRoundPairsFromFlat(flat).map((p) => (<div key={p.index} className="tmDrawPairBlock">
...
function mainPoolRoleTagColor(tag) {
    if (tag === '保送')
        return 'blue';
    if (tag === '小组赛晋级')
        return 'green';
...
const StepMainPoolConfirm = ({ session, dispatch }) => {
    ...
    const eventId = session.eventIds[0] ?? '333';
    const seedFromMain16 = useMemo(() => new Set(pickSeedTeamIds(session.teams, session.players, session.seeding, eventId, session.seedingPrimary, poolIds)), [session.teams, session.players, session.seeding, eventId, session.seedingPrimary, poolIds]);
    const rows = useMemo(() => displayRows.map((dr) => {
    ...
    const seedPreviewLine = useMemo(() => {
        const ids = pickSeedTeamIds(session.teams, session.players, session.seeding, eventId, session.seedingPrimary, poolIds);
        if (!ids.length)
            return '—（晋级队中暂无足够「三人有效主排序成绩」的队时可能不足 4 支）';
    ...
    return (<Card title="正赛名单确认">
      <Space wrap style={{ marginBottom: 12 }}>
        <Typography.Text>主排序依据（计算种子用，与步骤 4 一致）：</Typography.Text>
        <Select style={{ width: 120 }} value={session.seedingPrimary} onChange={(v) => dispatch({ type: 'SET_SEEDING_PRIMARY', primary: v })} options={[
      ...
      <Table size="small" pagination={false} dataSource={rows} rowKey="key" scroll={{ x: 'max-content' }} columns={[
            ...
            {
                title: '预选赛 / 出线',
                render: (_, r) => (<Space size={[4, 4]} wrap>
                {r.roleTags.map((t) => (<Tag key={t} color={mainPoolRoleTagColor(t)}>
                    {t}
                  </Tag>))}
              </Space>),
            },
      ...
      <div style={{ marginTop: 20 }}>
        <Button type="primary" disabled={poolIds.length < MIN_TEAMS} onClick={() => {
...
function usedPlayerIdsExceptTeam(session, excludeTeamId) {
    const s = new Set();
    for (const t of session.teams) {
        if (excludeTeamId && t.id === excludeTeamId)
            continue;
...
const StepTeam = ({ session, dispatch, activeTeamCount, openPlayerModal }) => {
    ...
    return (<Card title="2. 组队（每队 3 人）">
      ...
      <TeamEditModal open={teamModalOpen} onClose={closeTeamModal} editing={editingTeam} schools={session.schools} players={session.players} usedPlayerIds={usedForModal} activeTeamCount={activeTeamCount} onSave={saveTeam}/>
      <Table style={{ marginTop: 16 }} size="small" rowKey="id" dataSource={session.teams} pagination={false} columns={[
            { title: '队名', dataIndex: 'name' },
            {
                title: '归属',
                render: (_, r) => teamKindLabel(r, session.schools),
            },
            {
                title: '队员',
                render: (_, r) => (<Space wrap size={[8, 8]}>
                {r.playerIds.map((pid) => {
                        const p = session.players.find((x) => x.id === pid);
                        if (!p)
                            return null;
                        return (<Space key={pid} size={6} align="center">
            ...
            },
            {
                ...
                <Popconfirm title="删除该队伍？" description="选手不会删除，仍可在「选手」步骤中编辑或编入其他队。" onConfirm={() => dispatch({ type: 'DELETE_TEAM', teamId: r.id })} okText="删除" cancelText="取消">
...
function patchSeedingEntry(session, eventId, playerId, patch) {
...
function useSeedingTableHelpers(session, eventId, dispatch) {
    const [seedingModalPlayer, setSeedingModalPlayer] = useState(null);
    const [batchPrelimOpen, setBatchPrelimOpen] = useState(false);
    const renderPlayerRow = (player) => {
        const r = session.seeding.find((e) => e.playerId === player.id && e.eventId === eventId) ?? {
            playerId: player.id,
            eventId,
            single: null,
            average: null,
        ...
        const fmt = (v) => v === 'DNF' ? 'DNF' : v === null ? '—' : String(v);
        ...
        return {
          ...
          {player.name}
            ...
            scores: (<Space direction="vertical" size={2}>
          <Typography.Text type="secondary">
            正式 单次 {fmt(r.single)} · 平均 {fmt(r.average)} 「{sourceInline}」
          </Typography.Text>
          {hasPre && pre && (<Typography.Text type="secondary" style={{ fontSize: 12 }}>
              初赛 单次 {fmt(pre.single)} · 平均 {fmt(pre.average)}
            </Typography.Text>)}
        </Space>),
            action: (<Button size="small" type="primary" onClick={() => setSeedingModalPlayer(player)}>
          编辑成绩
        </Button>),
        };
    };
    ...
    const seedingModalEntry = seedingModalPlayer
        ? session.seeding.find((e) => e.playerId === seedingModalPlayer.id && e.eventId === eventId) ?? {
        ...
        : null;
    const modal = (<>
      <SeedingPlayerModal open={!!seedingModalPlayer} onClose={() => setSeedingModalPlayer(null)} player={seedingModalPlayer} eventId={eventId} entry={seedingModalEntry} onSave={(next) => dispatch({
            type: 'SET_SEEDING',
            seeding: patchSeedingEntry(session, eventId, next.playerId, next),
        })}/>
      <PreliminaryBatchModal open={batchPrelimOpen} onClose={() => setBatchPrelimOpen(false)} session={session} eventId={eventId} dispatch={dispatch}/>
    </>);
    return { renderPlayerRow, columns, modal, openBatchPrelim: () => setBatchPrelimOpen(true) };
}
const StepScoreEntry = ({ session, dispatch, eventId }) => {
    ...
    const orphanPlayers = useMemo(() => session.players.filter((p) => !inAnyTeam.has(p.id)), [session.players, inAnyTeam]);
    const teamsSorted = useMemo(() => sortTeamsBySeedingRank(session.teams, session.players, session.seeding, eventId, session.seedingPrimary), [session.teams, session.players, session.seeding, eventId, session.seedingPrimary]);
    const bracketEligibleIds = useMemo(() => new Set(rankedBracketTeamIds(session.teams, session.players, session.seeding, eventId, session.seedingPrimary)), [session.teams, session.players, session.seeding, eventId, session.seedingPrimary]);
    return (<Card title="3. 成绩录入（按队伍分组）">
      {modal}
      <Space wrap style={{ marginBottom: 12 }}>
      ...
      {teamsSorted.map((team) => {
            ...
            return (<Card key={team.id} size="small" style={{ marginBottom: 12 }} title={<Space wrap>
                ...
                {!team.disabled && team.playerIds.length === TEAM_PLAYERS && (<>
                    {bracketEligibleIds.has(team.id) ? (<Tag color="processing">正赛出线</Tag>) : (<Tag>未进前 {BRACKET_TEAM_COUNT}</Tag>)}
                  </>)}
              </Space>}>
      ...
      {orphanPlayers.length > 0 && (<Card size="small" type="inner" title="未入任何队伍的选手" style={{ marginBottom: 12 }}>
          <Table size="small" rowKey="key" pagination={false} columns={columns} dataSource={orphanPlayers.map((p) => renderPlayerRow(p))} scroll={{ x: true }}/>
        </Card>)}
    </Card>);
};
const StepTeamConfirm = ({ session, dispatch, eventId }) => {
    const { renderPlayerRow, columns: confirmColumns, modal: confirmSeedingModal } = useSeedingTableHelpers(session, eventId, dispatch);
    const teamsSorted = useMemo(() => sortTeamsBySeedingRank(session.teams, session.players, session.seeding, eventId, session.seedingPrimary), [session.teams, session.players, session.seeding, eventId, session.seedingPrimary]);
    const bracketEligibleIds = useMemo(() => new Set(rankedBracketTeamIds(session.teams, session.players, session.seeding, eventId, session.seedingPrimary)), [session.teams, session.players, session.seeding, eventId, session.seedingPrimary]);
    return (<Card title="4. 队伍确认">
      {confirmSeedingModal}
      <Space wrap style={{ marginBottom: 12 }}>
      ...
      {teamsSorted.map((team) => {
            ...
            return (<Card key={team.id} size="small" style={{ marginBottom: 12 }} title={<Space wrap>
              ...
              </Space>} extra={<Space align="center" size={8}>
                <Typography.Text type="secondary" style={{ fontSize: 12 }}>
                  缺席
                </Typography.Text>
                <Switch checked={team.disabled} onChange={(c) => dispatch({ type: 'SET_TEAM_DISABLED', teamId: team.id, disabled: c })}/>
              </Space>}>
            <Table size="small" rowKey="key" pagination={false} columns={confirmColumns} dataSource={rows} scroll={{ x: true }}/>
          </Card>);
        })}
    </Card>);
};
export default TeamMatchSteps;
//# sourceMappingURL=TeamMatchSteps.jsx.map