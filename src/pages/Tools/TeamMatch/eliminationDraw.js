...
export function isEliminationGroupMatch(m) {
    return Array.isArray(m.teamIds);
}
/** 两两配对；奇数时最后一队轮空（无 PK，视为直接晋级池）——仅用于旧逻辑/测试 */
export function pairTeamIdsRandom(teamIds, rng) {
...
export function buildEliminationBracketMatches(pairs, waveSizes) {
 ...
 */
export function packRemainderIntoGroupMatches(remainder, groupSize) {
    const r = [...remainder];
    const extraGroups = [];
    ...
    if (r.length === 1)
        return { extraGroups, naturalByeTeamIds: r };
    extraGroups.push([...r]);
    return { extraGroups, naturalByeTeamIds: [] };
}
/**
 ...
 */
export function groupTeamIdsRandom(teamIds, groupSize, rng) {
    const shuffled = shuffle([...teamIds], rng);
    const groups = [];
...
export function buildElimGroupMatches(groups, waveSizes) {
...
export function defaultEliminationState(session) {
    void session;
    return {
        skipped: false,
        groupSize: 3,
...
//# sourceMappingURL=eliminationDraw.js.map