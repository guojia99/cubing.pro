function normalizeSeedingEntry(e) {
    return {
        ...e,
...
const emptyRegion = () => [null, null, null, null];
function migrateWizardStep(s) {
...
function migrateWizardMergeSchoolPlayer(s) {
    const ver = s.wizardSchemaVersion ?? 1;
    if (ver >= 3) {
        return { wizardStep: s.wizardStep, wizardSchemaVersion: ver };
    }
...
function migrateWizardElimSteps(s) {
    const ver = s.wizardSchemaVersion ?? 1;
    if (ver >= 4) {
        return { wizardStep: s.wizardStep, wizardSchemaVersion: ver };
    }
...
function migrateWizardMainPoolConfirmStep(s) {
    const ver = s.wizardSchemaVersion ?? 1;
    if (ver >= 5) {
        return { wizardStep: s.wizardStep, wizardSchemaVersion: ver };
    }
...
function isLegacyElimBracketMatch(m) {
...
function migrateEliminationPhase(elim) {
    if (!elim)
        return null;
    const first = elim.matches?.[0];
    let matches = (elim.matches ?? []);
    if (first && isLegacyElimBracketMatch(first)) {
        matches = elim.matches.map((m) => ({
            id: m.id,
            teamIds: [m.teamAId, m.teamBId].filter(Boolean),
            winnerId: m.winnerId,
            pk: m.pk
                ? {
    ...
    const naturalByeTeamIds = elim.naturalByeTeamIds?.length > 0
        ? [...elim.naturalByeTeamIds]
        : elim.naturalByeTeamId
            ? [elim.naturalByeTeamId]
            : [];
    const groupSize = elim.groupSize ?? (first && isLegacyElimBracketMatch(first) ? 2 : 3);
    const out = {
...
export function normalizeSession(s) {
    ...
    return {
        ...
        teams: s.teams.map((t) => ({ ...t, kind: t.kind ?? 'school', battlecry: t.battlecry ?? '' })),
        players: s.players.map((p) => ({ ...p, oneId: p.oneId ?? null, battlecry: p.battlecry ?? '' })),
