...
export function parseTeamColumn(cell) {
    const trimmed = cell.trim();
    const m = trimmed.match(/^(.+?)[（(]([^）)]+)[）)]\s*$/);
    if (m)
        return { teamName: m[1].trim(), schoolName: m[2].trim() };
    return { teamName: trimmed, schoolName: null };
}
/** 姓名末尾可接 10 位 WCA ID（如 2017XUGE01） */
export function parsePlayerCell(raw) {
    const s = raw.trim();
    if (!s)
        return { name: '', wcaId: null };
...
function ensureFreelancerSchool(schools) {
    if (getFreelancerSchoolId(schools))
        return schools;
    return [...schools, { id: uuidv4(), name: '自由人', kind: 'freelancer' }];
}
function findOrAddStandardSchool(schools, name) {
    const hit = schools.find((s) => s.name === name && s.kind !== 'freelancer');
    if (hit)
        return { schools, schoolId: hit.id };
    const s = { id: uuidv4(), name, kind: 'standard' };
    return { schools: [...schools, s], schoolId: s.id };
}
function activeFullTeamCount(teams) {
 ...
 */
export function buildTeamRosterPasteImport(session, text) {
    ...
    for (let li = 0; li < lines.length; li++) {
        ...
        if (!teamName)
            return { ok: false, message: `第 ${li + 1} 行：队名为空` };
        let targetSchoolId;
        if (schoolName) {
        ...
        for (let pi = 0; pi < playerCols.length; pi++) {
            ...
            if (!name)
                return { ok: false, message: `第 ${li + 1} 行：第 ${pi + 1} 名队员姓名为空` };
            if (wcaId) {
        ...
        const triple = [pids[0], pids[1], pids[2]];
...
//# sourceMappingURL=teamRosterPasteImport.js.map