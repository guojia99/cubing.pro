...
function teamStrength(session, teamId) {
    const team = session.teams.find((t) => t.id === teamId);
    if (!team)
        return Number.POSITIVE_INFINITY;
    const eventId = session.eventIds[0] ?? '333';
    const { sum, valid } = teamSeedingSum(team, session.players, session.seeding, eventId, session.seedingPrimary);
 ...
 */
function worstTwoByValidAverageSum(nonSeedIds, session) {
    const eventId = session.eventIds[0] ?? '333';
    const rows = [];
...
function shuffleWithinStrengthBands(ids, session, rng) {
...
const PROTECTED_SEED_OPP_SLOTS = new Set([...SEED_FLAT, ...SEED_OPP_SLOTS]);
/**
 * 首轮轮空（null）尽量左右半区均衡：在可交换槽位上把 null 与另一侧半区的队伍对换。
 * 有种子时：种子位与种子对位槽 0–3 区、1–5–9–13 等不交换（避免拆种子 vs 对位）。
 * 无种子时：任意 [0–7] / [8–15] 半区对调。
 */
function balanceNullsAcrossHalves(flat, protectSeedOppSlots) {
    const movable = (i) => !protectSeedOppSlots || !PROTECTED_SEED_OPP_SLOTS.has(i);
    function countNulls(lo, hi) {
        let n = 0;
        for (let i = lo; i <= hi; i++)
            if (flat[i] === null)
                n++;
 ...
 */
function tryPairDifferentSchools(ids, session) {
    const school = (tid) => schoolIdForDrawPairing(session, tid);
    const out = [...ids];
...
function permuteOpponentsForSeeds(seedTeamIds, opp, session) {
    const school = (tid) => schoolIdForDrawPairing(session, tid);
    const seedSchool = (i) => {
...
export function regionsFromFlat(flat) {
    const out = [
        flat.slice(0, 4),
        flat.slice(4, 8),
        flat.slice(8, 12),
 ...
 */
function splitNonSeedsForSeedBracket(nonSeedIds, session, mode, rng) {
 ...
 */
export function randomizeDraw(session) {
    ...
    const bracketIds = fromMain && fromMain.length >= MIN_TEAMS
        ? fromMain
        : rankedBracketTeamIds(session.teams, session.players, session.seeding, eventId, session.seedingPrimary);
    const n = bracketIds.length;
    if (n < MIN_TEAMS) {
        return session.regionSlots;
    }
    const bracketIdSet = new Set(bracketIds);
    const seedTeamIdsBracket = [
    ...
    for (let i = 0; i < Math.min(4, oppOrdered.length); i++) {
        if (oppOrdered[i])
            flat[SEED_OPP_SLOTS[i]] = oppOrdered[i];
    }
    balanceNullsAcrossHalves(flat, true);
    return regionsFromFlat(flat);
}
//# sourceMappingURL=drawRandom.js.map