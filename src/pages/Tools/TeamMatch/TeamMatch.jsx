...
import { loadLiveUISettings, saveLiveUISettings } from '@/pages/Tools/TeamMatch/liveUiSettings';
import { canAccessStepBarIndex, hasOverSixteenFullTeams, stepBarIndexToWizardStep, wizardStepToBarIndex, } from '@/pages/Tools/TeamMatch/stepAccess';
...
import { MAX_ROSTER_TEAMS, MIN_TEAMS, WIZARD_STEP_ELIM_DRAW, WIZARD_STEP_ELIM_LIVE, WIZARD_STEP_MAIN_DRAW, WIZARD_STEP_MAIN_LIVE, WIZARD_STEP_MAIN_POOL_CONFIRM, WIZARD_STEP_PODIUM, } from '@/pages/Tools/TeamMatch/types';
...
const ROSTER_STEP_TITLES = ['学校与选手', '组队', '成绩录入', '队伍确认'];
function buildStepBarTitles(over16) {
    if (over16) {
...
function MatchEventField() {
    ...
    return (<Space wrap align="center">
      ...
      <Select style={{ minWidth: 200 }} value={ev} options={TEAM_MATCH_WCA_EVENT_OPTIONS} onChange={(v) => dispatch({ type: 'SET_EVENT_IDS', eventIds: [v] })} showSearch optionFilterProp="label"/>
    </Space>);
}
function MatchNameField({ variant = 'default' }) {
    const { state, dispatch } = useTeamMatchStore();
    ...
    return (<Space wrap align="center">
      {!dark && (<Typography.Text strong type="secondary">
          比赛名称
        </Typography.Text>)}
      {dark && (<Typography.Text style={{ color: 'rgba(255,255,255,0.85)' }}>比赛名称</Typography.Text>)}
      <Input style={{
            maxWidth: 420,
            minWidth: 200,
            ...(dark
                ...
                : {}),
        }} placeholder="填写比赛名称" value={v} onChange={(e) => setV(e.target.value)} onBlur={commit} onPressEnter={commit}/>
    </Space>);
}
function TeamMatchInner() {
    ...
    const [liveUISettings, setLiveUISettings] = React.useState(() => loadLiveUISettings());
    ...
    React.useEffect(() => {
        if (step !== WIZARD_STEP_MAIN_LIVE)
            setLiveSettingsOpen(false);
    }, [step]);
    const activeTeamCount = session.teams.filter((t) => !t.disabled && t.playerIds.length === 3).length;
    const canNext = () => {
        switch (step) {
            case 0:
    ...
    const goToStepBarIndex = (stepIndex) => {
    ...
    const jsonToolbarProps = {
        session,
        liveUISettings,
        onHydrate: (s) => dispatch({ type: 'HYDRATE', session: s }),
    ...
    if (session.status === 'live' && (step === WIZARD_STEP_MAIN_LIVE || step === WIZARD_STEP_PODIUM)) {
        ...
        const podiumBlockHint = session.skipBronzeMatch
            ? '请完成全部淘汰赛（已跳过季军赛）'
            : '请完成全部淘汰赛与铜牌战';
        return (<div className="tmFullscreen">
        ...
        <div style={{
          ...
          <Space wrap>
            {step === WIZARD_STEP_PODIUM ? (<Button type="primary" onClick={() => dispatch({ type: 'SET_WIZARD_STEP', step: WIZARD_STEP_MAIN_LIVE })}>
                返回正赛
              </Button>) : (<>
                ...
                <Button type="primary" disabled={!bracketComplete} title={!bracketComplete ? podiumBlockHint : undefined} onClick={() => dispatch({ type: 'SET_WIZARD_STEP', step: WIZARD_STEP_PODIUM })}>
                  下一步：领奖台
                </Button>
              </>)}
          </Space>
        ...
        <div className="tmFullscreenInner">
          {step === WIZARD_STEP_PODIUM ? (<PodiumStage />) : (<BracketStage liveSettings={liveUISettings} onOpenLiveSettings={() => setLiveSettingsOpen(true)}/>)}
        </div>
        {step === WIZARD_STEP_MAIN_LIVE && (<LiveSettingsModal open={liveSettingsOpen} onClose={() => setLiveSettingsOpen(false)} value={liveUISettings} onApply={(next) => {
    ...
    return (<>
      ...
      <Space direction="vertical" style={{ width: '100%' }} size="large">
        ...
        <Space wrap>
          ...
          <Popconfirm title="用测试数据覆盖当前比赛？" description="将替换为学校/选手/队伍及随机种子成绩（满16队：广工5队等预设校名）。" onConfirm={() => {
          ...
          <Modal title="追加测试分组" open={mockModalOpen} onCancel={() => setMockModalOpen(false)} onOk={() => {
            ...
            <Space align="center">
              ...
              <InputNumber min={1} max={Math.max(1, MAX_ROSTER_TEAMS - session.teams.length)} value={mockGroupCount} onChange={(v) => setMockGroupCount(v === null ? 1 : Number(v))}/>
            </Space>
          </Modal>
        </Space>
        <List size="small" header={<Typography.Text strong>最近 {state.root.historyIds.length} 场</Typography.Text>} dataSource={state.root.historyIds} renderItem={(id) => {
            const s = state.root.sessions[id];
            if (!s)
                return null;
            const meta = metaFromSession(s);
            return (<List.Item actions={[
                    <Button key="load" type="link" size="small" onClick={() => dispatch({ type: 'LOAD_SESSION', id })}>
                    打开
                  </Button>,
                    <Button key="del" type="link" danger size="small" onClick={() => dispatch({ type: 'DELETE_SESSION', id })}>
                    删除
                  </Button>,
                ]}>
        ...
        <Steps current={wizardStepToBarIndex(step, over16)} onChange={goToStepBarIndex} items={stepBarTitles.map((t) => ({ title: t }))}/>
...
const TeamMatch = () => (<TeamMatchProvider>
    <TeamMatchInner />
  </TeamMatchProvider>);
export default TeamMatch;
//# sourceMappingURL=TeamMatch.jsx.map