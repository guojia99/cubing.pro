import { advanceWinners, newSnapshotId, rebuildBracketFromSession, regionsToFlat, syncBronzeToSession, } from '@/pages/Tools/TeamMatch/bracketGen';
import { randomizeDraw } from '@/pages/Tools/TeamMatch/drawRandom';
import { buildElimGroupMatches, defaultEliminationState, groupTeamIdsRandom, isEliminationGroupMatch, } from '@/pages/Tools/TeamMatch/eliminationDraw';
...
import { MAX_ELIMINATION_BYE_TEAMS, TEAM_PLAYERS, WIZARD_STEP_MAIN_DRAW, WIZARD_STEP_MAIN_LIVE, } from '@/pages/Tools/TeamMatch/types';
function touch(s) {
    return { ...normalizeSession(s), updatedAt: Date.now() };
}
function emptyRosterRegionSlots() {
    const row = [null, null, null, null];
    return [row, row, row, row];
}
/** 队伍确认（step≥3）之后任意队伍/种子变更：清空预选赛、正赛名单与对阵，回到队伍确认 */
function resetDownstreamAfterRosterEdit(s) {
    if (s.wizardStep < 3)
        return s;
...
export function mergeSeeding(prev, players, eventIds) {
    const map = new Map();
    for (const e of prev) {
    ...
    return [...map.values()].filter((e) => players.some((p) => p.id === e.playerId) && eventIds.includes(e.eventId));
}
...
function patchBracketOrElimMatch(session, matchId, patch) {
    ...
    return setMatch(session, matchId, patch);
}
function cascadeRounds(session) {
...
export function reduceSession(session, action) {
    let next = session;
    switch (action.type) {
        case 'RESET_SESSION':
        ...
        case 'MOCK_APPEND_GROUPS': {
            const { schools: addSchools, players: addPlayers, teams: addTeams } = appendMockGroups(next, action.count);
            ...
            if (next.wizardStep >= 3)
                next = resetDownstreamAfterRosterEdit(next);
            break;
        }
        ...
        case 'DELETE_TEAM': {
            ...
            const seedTeamIds = next.seedTeamIds.map((sid) => (sid === id ? null : sid));
            const seedSet = new Set(seedTeamIds.filter(Boolean));
            const teamsWithSeed = teams.map((t) => ({ ...t, isSeed: seedSet.has(t.id) }));
            const mapSlot = (tid) => (tid === id ? null : tid);
            const regionSlots = next.regionSlots.map((row) => row.map(mapSlot));
            const flatSlots = next.flatSlots ? next.flatSlots.map(mapSlot) : null;
            let patched = {
                ...next,
                teams: teamsWithSeed,
                seedTeamIds,
            ...
            if (patched.elimination) {
                ...
                const touched = e.byeTeamIds.includes(id) || e.matches.some((m) => m.teamIds.includes(id));
                patched = {
                    ...patched,
                    elimination: touched
                        ? {
        ...
        case 'SET_ELIM_BYE_TEAM_IDS': {
            ...
            const ranked = new Set(allRankedActiveTeamIds(next.teams, next.players, next.seeding, eventId, next.seedingPrimary));
        ...
        case 'CONFIRM_MAIN_BRACKET_POOL': {
            ...
            const eventId = next.eventIds[0] ?? '333';
            const picked = pickSeedTeamIds(next.teams, next.players, next.seeding, eventId, next.seedingPrimary, mainIds);
            const seedTeamIds = [
                picked[0] ?? null,
                picked[1] ?? null,
                picked[2] ?? null,
        ...
        case 'COMPUTE_SEED_TEAMS': {
            const eventId = next.eventIds[0] ?? '333';
            const mainIds = computeMainBracketTeamIds(next);
            const ids = pickSeedTeamIds(next.teams, next.players, next.seeding, eventId, next.seedingPrimary, mainIds);
            const seedTeamIds = [
                ids[0] ?? null,
                ids[1] ?? null,
                ids[2] ?? null,
        ...
        case 'SET_PK_DRAFT': {
            ...
            if (isEliminationGroupMatch(m)) {
                const pk = { ...m.pk, currentResults: action.results };
        ...
        case 'PK_SETTLE': {
            ...
            next = patchBracketOrElimMatch(next, action.matchId, patch);
            if (patch.winnerId && !isEliminationMatchId(next, action.matchId))
        ...
        case 'PK_MANUAL_WINNER': {
            ...
            if (!m?.pk)
                break;
            if (isEliminationGroupMatch(m)) {
                if (!m.pk.teamIds.includes(action.winnerTeamId))
                    break;
                const pk = {
                    ...m.pk,
            ...
            if (!isEliminationMatchId(next, action.matchId))
                next = cascadeRounds(next);
            break;
        }
        case 'PK_REPLAY': {
...
export function buildPkTemplateRows(teamAId, teamBId, teamA, teamB) {
    const rows = [];
...
export function buildPkGroupTemplateRows(teamIds, teams) {
    const rows = [];
