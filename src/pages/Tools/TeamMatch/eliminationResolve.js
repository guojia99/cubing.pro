import { allRankedActiveTeamIds, rankedBracketTeamIds, } from '@/pages/Tools/TeamMatch/seedingMath';
 ...
 */
export function computeMainBracketTeamIds(session) {
    ...
    const pool = new Set(elim.byeTeamIds);
    ...
    if (elim.naturalByeTeamId)
        pool.add(elim.naturalByeTeamId);
    return ranked.filter((id) => pool.has(id)).slice(0, BRACKET_TEAM_COUNT);
}
export function isEliminationComplete(session) {
    const elim = session.elimination;
    if (!elim || elim.skipped || elim.drawVersion < 1)
        return true;
 ...
 */
export function buildMainPoolDisplayRows(session) {
    const eventId = session.eventIds[0] ?? '333';
    const { teams, players, seeding, seedingPrimary } = session;
    const ranked = allRankedActiveTeamIds(teams, players, seeding, eventId, seedingPrimary);
    ...
    if (!elim || elim.skipped) {
        ...
        return ranked.map((id, i) => {
            const roleTags = mainSet.has(id) ? ['正赛16强'] : ['未进正赛16强'];
            return {
                teamId: id,
                rankInList: i + 1,
    ...
    if (elim.naturalByeTeamId)
        naturalSet.add(elim.naturalByeTeamId);
    const wonGroup = new Set();
    const eliminated = new Set();
    const pendingGroup = new Set();
    for (const m of elim.matches) {
        if (m.winnerId) {
            ...
            for (const tid of m.teamIds)
                pendingGroup.add(tid);
        }
    }
    const poolSet = new Set([...byeSet, ...wonGroup, ...naturalSet]);
    ...
    return orderedIds.map((id) => {
        ...
        if (byeSet.has(id))
            roleTags.push('保送');
        if (naturalSet.has(id))
            roleTags.push('轮空进池');
        if (wonGroup.has(id))
            roleTags.push('小组赛晋级');
        if (eliminated.has(id))
            roleTags.push('预选赛淘汰');
        if (pendingGroup.has(id) && !wonGroup.has(id) && !eliminated.has(id))
            roleTags.push('小组赛待定');
        const inMain = mainRankById.has(id);
        if (inMain)
            roleTags.push('正赛16强');
        else if (poolSet.has(id) && !eliminated.has(id))
            roleTags.push('晋级池未出线');
        if (roleTags.length === 0)
            roleTags.push('未编入预选赛');
        return {
            teamId: id,
...
//# sourceMappingURL=eliminationResolve.js.map