...
/** 半决赛均决出胜负后：负者争铜牌 */
export function syncBronzeFromSemis(rounds, prev) {
    const r2 = rounds[2];
    if (!r2 || r2.length < 2)
        return null;
    const s0 = r2[0];
...
export function syncBronzeToSession(session) {
...
function flatRegions(regionSlots) {
    const out = [...regionSlots[0], ...regionSlots[1], ...regionSlots[2], ...regionSlots[3]];
    return out;
}
/** 从 16 槽生成淘汰赛：相邻配对 (0,1)(2,3)... */
export function buildRoundsFromFlat16(flat) {
    const rounds = [];
    // Round 0: 8 matches
    const r0 = [];
    for (let i = 0; i < 8; i++) {
 ...
 */
function isSubtreePermanentDoubleEmptyFromFlat(flat, m) {
    ...
    for (let j = r0Start; j <= r0End; j++) {
        const p = j * 2;
        if (flat[p] !== null || flat[p + 1] !== null)
            return false;
    }
    return true;
}
/** 将上一轮胜者填入下一轮；双亲齐全时生成 PK。若对阵与上一轮相同则保留已有 pk / winner */
export function advanceWinners(rounds, flat) {
    const next = rounds.map((r) => r.map((m) => ({
        ...m,
        pk: m.pk
            ? {
                ...m.pk,
    ...
    for (let r = 0; r < next.length - 1; r++) {
        ...
        for (let i = 0; i < nxt.length; i++) {
            ...
            const pendingPk = (m) => !!(m?.teamAId && m.teamBId && !m.winnerId);
            ...
            if (!wa || !wb) {
                ...
                const emptyFromDraw = emptyFeeder &&
                    !emptyFeeder.teamAId &&
                    !emptyFeeder.teamBId &&
...
export function rebuildBracketFromSession(session) {
...
export function regionsToFlat(regionSlots) {
    return flatRegions(regionSlots);
}
export function newSnapshotId() {
    return uuidv4();
}
//# sourceMappingURL=bracketGen.js.map