...
export function canBulkFillSeeding(session) {
    return session.players.some((p) => (p.wcaId?.length === 10) || !!(p.oneId?.trim() && /^\d+$/.test(p.oneId.trim())));
}
...
export async function bulkFillSeedingScores(session, eventIds, onProgress, options) {
    ...
    for (const ev of eventIds) {
        for (const p of players) {
            ...
            const preliminary = base.preliminary ?? { single: base.single, average: base.average };
            if (p.wcaId?.length === 10) {
                try {
            ...
            const strategy = base.adoptStrategy ?? 'best_average';
            const next = {
                ...base,
                playerId: p.id,
                eventId: ev,
    ...
    onProgress(100);
    return [...map.values()].filter((e) => players.some((pl) => pl.id === e.playerId) && eventIds.includes(e.eventId));
}
//# sourceMappingURL=bulkFillSeeding.js.map