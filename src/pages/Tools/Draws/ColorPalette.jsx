...
const ColorPalette = ({ onSelectColor, presetColors = [], storageKey = 'color-history', allKeys = [], }) => {
    ...
    const renderColorBlock = (color) => {
        ...
        return (<div key={color} className="color-block" style={{
        ...
        {selectedColor === color && (<CheckOutlined style={{ color: isTransparent ? 'black' : 'white' }}/>)}
      </div>);
    };
    // @ts-ignore
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      {/* 历史颜色 */}
      {historyColors.length > 0 && (<div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 7 }}>
          {historyColors.map(renderColorBlock)}
        </div>)}

      {/* 自定义颜色 */}
      <Popover content={<SketchPicker color={selectedColor} onChangeComplete={handleCustomColor}/>} trigger="click" visible={pickerVisible} onVisibleChange={setPickerVisible}>
