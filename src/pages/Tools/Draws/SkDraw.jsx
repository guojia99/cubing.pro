...
const SimpleSkDraw = () => {
    ...
    const [skPointsM, setSkPointsM] = useState([]);
    useEffect(() => {
        const v = [];
    ...
    return (<div>
      <DrawPalette svgPoints={skPointsM} presetColors={baseSkColor} storageKey={'simpleSkDraw'} viewBox={'0 0 130 76'} strokeWidthNum={0.2}/>
    </div>);
};
const SK3DDraw = () => {
    const baseSvgs = [
    ...
    const [skLinePointsM, setSkLinePointsM] = useState([]);
    ...
    const selectLine = (k) => {
        ...
        if (newMap.has(k)) {
            ...
            setSkLinePointsM(skLinePointsM.filter((e) => {
    ...
    return (<div>
      <DrawPalette svgPoints={[...skPointsM, ...skLinePointsM]} presetColors={baseSkColor} storageKey={'SK3DDraw'} viewBox={'0 0 78 82'} strokeWidthNum={0.2} buttons={<div style={{ textAlign: 'center' }}>
            <h2>
              <FormattedMessage id="draws.flank"/>
            </h2>
            <div>
              <Row gutter={16}>
                {lineSvgs.map((btn, index) => (<Col span={4} key={index} style={{ marginBottom: 10 }}>
                    <Button type={skLineSelectMap.get(btn.key) ? 'primary' : undefined} size={'small'} onClick={() => selectLine(btn.key)} block>
                      {btn.name}
                    </Button>
                  </Col>))}
              </Row>
...
const SKDraw = () => {
    ...
    return (<>
      <NavTabs type="line" items={items} tabsKey="sk_draw_tabs" indicator={{ size: (origin) => origin - 20, align: 'center' }}/>
    </>);
};
export default SKDraw;
//# sourceMappingURL=SkDraw.jsx.map