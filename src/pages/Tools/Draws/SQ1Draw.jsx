...
import DrawPalette from '@/pages/Tools/Draws/DrawPalette';
...
const SimpleSq1Draw = () => {
    ...
    const [cpsOpt, setCpsOpt] = useState([]);
    ...
    return (<div>
      <DrawPalette svgPoints={[...linePoint, ...svgPoints]} presetColors={baseMinxColor} storageKey={'sq1Draw'} viewBox={'0 0 75 75'} strokeWidthNum={0.2} buttons={<div>
            ...
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              <Space>
                <Form.Item label={<FormattedMessage id="draws.sq1.Central_axis"/>} style={{ marginBottom: 0 }}>
                  <Select defaultValue={30} onChange={resetLineReg} options={[
                ...
                <Form.Item label={<FormattedMessage id="draws.sq1.Default"/>} style={{ marginBottom: 0 }}>
                  {/*// @ts-ignore*/}
                  <Select onChange={setDefault} value={defaultVal} options={cpsOpt} defaultValue={'星 Star'} style={{ width: 150 }}/>
                </Form.Item>
              </Space>
            </div>
            ...
            {/*按钮*/}
            <div>
              <Button onClick={reset} key={'reset_button'} style={{ marginLeft: 10 }} disabled={!(cubes.length > 0)}>
                <FormattedMessage id="draws.reset"/>
              </Button>
              <Button onClick={removeHandler} key={'removeHandler_button'} style={{ marginLeft: 10 }} disabled={!(cubes.length > 0)}>
                <FormattedMessage id="draws.delete"/>
              </Button>
              <Button onClick={addCorner} key={'addCorner_button'} style={{ marginLeft: 10 }} disabled={!(corner < 6 && reg + 60 <= 360)}>
                <FormattedMessage id="draws.sq1.add_corner"/>
              </Button>
              <Button onClick={addEdge} style={{ marginLeft: 10 }} key={'addEdge_button'} disabled={!(edge < 8 && reg + 30 <= 360)}>
...
const DoubleSq1Draw = () => {
    ...
    const [svgPointsTop, setSvgPointsTop] = useState([]);
    const [linePointTop, setLinePointTop] = useState([]);
    const [topCspOption, setTopCspOptions] = useState([]);
    const [downReg, setDownReg] = useState(0);
    const [downDefaultVal, setDownDefaultVal] = useState();
    const [svgPointsDown, setSvgPointsDown] = useState([]);
    const [linePointDown, setLinePointDown] = useState([]);
    const [downCspOption, setDownCspOptions] = useState([]);
    // 输入文字
    const [useFont, setUseFont] = useState();
    const [fontsTop, setFontsTop] = useState([]);
    const [fontsDown, setFontsDown] = useState([]);
    const handleSetFontSwitch = (showFont) => {
    ...
    const handleTopFontChange = (index, value) => {
    ...
    const handleDownFontChange = (index, value) => {
    ...
    const setDraws = (e, topDown) => {
        const v = cspMap.get(e);
        if (!v) {
            return;
        }
        let newSvgPoints = [];
        let curEdge = 0, curCorner = 0;
    ...
    return (<div>
      <DrawPalette width={300} height={600} svgPoints={[...linePointTop, ...linePointDown, ...svgPointsTop, ...svgPointsDown]} presetColors={baseMinxColor} storageKey={'sq1DrawDouble'} viewBox={'0 0 75 150'} strokeWidthNum={0.2} buttons={<>
            ...
            <div style={{ display: 'flex', gap: 16 }}>
              <Form.Item label={<strong>顶层</strong>} style={{ marginBottom: 0 }}>
                {/* @ts-ignore */}
                <Select onChange={handleUpdateTopOpt} options={topCspOption} defaultValue={'星 Star'} style={{ width: 150 }}/>
              </Form.Item>

              <Form.Item label={'中轴线'} style={{ marginBottom: 0 }}>
                <Select defaultValue={30} onChange={(e) => {
                resetLineReg(e, true);
            }} options={[
                { value: 0, label: <FormattedMessage id="draws.sq1.None"/> },
                { value: 30, label: <FormattedMessage id="draws.sq1.Positive_15"/> },
              ...
              <div style={{ textAlign: 'center', width: '50%' }}>
                <Form.Item label={<FormattedMessage id="draws.sq1.Rotate"/>}>
                  <Slider defaultValue={0} step={30} min={0} max={180} onChange={(e) => {
                resetBaseReg(e, 'top');
            }}/>
                </Form.Item>
              </div>
            ...
            {useFont && fontsTop && fontsTop.length > 0 && (<>
                <Form.Item label={<strong>顶层文字</strong>} style={{ marginBottom: 0 }}>
                  <Row gutter={[8, 8]}>
                    {fontsTop.map((font, index) => (<Col span={8} key={`top-font-${index}`}>
                        <Input value={font} maxLength={1} onChange={(e) => handleTopFontChange(index, e.target.value)} placeholder="Enter Value"/>
                      </Col>))}
                  </Row>
                </Form.Item>
              </>)}
            ...
            <div style={{ display: 'flex', gap: 16, marginTop: 15 }}>
              <Form.Item label={<strong>底层</strong>} style={{ marginBottom: 0 }}>
                {/* @ts-ignore */}
                <Select onChange={handleUpdateDownOpt} options={downCspOption} value={downDefaultVal} style={{ width: 150 }}/>
              </Form.Item>
            ...
            {useFont && fontsDown && fontsDown.length > 0 && (<>
                <Form.Item label={<strong>底层文字</strong>} style={{ marginBottom: 0 }}>
                  <Row gutter={[8, 8]}>
                    {fontsDown.map((font, index) => (<Col span={8} key={`down-font-${index}`}>
                        <Input value={font} maxLength={1} onChange={(e) => handleDownFontChange(index, e.target.value)} placeholder="Enter Value"/>
                      </Col>))}
                  </Row>
                </Form.Item>
              </>)}
...
const Sq1Draw = () => {
    ...
    return (<>
      <NavTabs type="line" items={items} tabsKey="sq1_draw_tabs" indicator={{ size: (origin) => origin - 20, align: 'center' }}/>
    </>);
};
export default Sq1Draw;
//# sourceMappingURL=SQ1Draw.jsx.map