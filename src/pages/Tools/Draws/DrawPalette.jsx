...
const DrawPalette = ({ presetColors = [], storageKey = 'color-history', svgPoints = [], viewBox, strokeWidthNum = 1, buttons = [], width = 400, height = 400, }) => {
    const [colors, setColors] = useState({});
    const [keys, setKeys] = useState([]);
    const svgRef = useRef(null);
    const [downloadName, setDownloadName] = useState();
    const [strokeWidth, setStrokeWidth] = useState(1);
    const handleColrChange = (key, color) => {
    ...
    const handleImageDownload = async (format = 'png') => {
    ...
    const buildTransform = (elem) => {
        if (elem.transformStr)
            return elem.transformStr;
        const parts = [];
        if (elem.translate?.length)
            parts.push(`translate(${elem.translate.join(' ')})`);
        if (elem.rotatePoint) {
        ...
        <Col xs={24} sm={24} md={24} lg={16} xl={16} style={{ paddingRight: 16 }}>
          <Card>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <Input value={downloadName} onChange={(e) => setDownloadName(e.target.value)} placeholder={intl.formatMessage({ id: 'draws.palette.input' })} style={{ width: 175 }}/>
              <DownloadOutlined onClick={() => handleSvgDownload()} style={{ cursor: 'pointer', fontSize: 24 }} size={30}/>
              <FileJpgOutlined onClick={() => handleImageDownload('jpg')} style={{ cursor: 'pointer', fontSize: 24 }} size={30}/>
              <FileImageOutlined onClick={() => handleImageDownload('png')} style={{ cursor: 'pointer', fontSize: 24 }} size={30}/>
            </div>
            ...
            if (elem.text) {
                ...
                return (<text 
                // @ts-ignore
                fill={colors[key]} key={key} data-key={key} textAnchor="start" fontFamily={'Noto Sans JP'} fontSize={elem.textSize} 
                // @ts-ignore
                x={elem.textPoint[0]} 
                // @ts-ignore
                y={elem.textPoint[1]} transform={textTransform} opacity={elem.disShow ? 0 : 1}>
                        {elem.text}
                      </text>);
            }
            if (elem.d) {
                return (<path 
                // @ts-ignore
                fill={colors[key]} key={key} data-key={key} 
                /* eslint-disable-next-line react/no-unknown-property */
                uncolor-data-key={elem.unColorBindKey} d={elem.d} stroke={'black'} strokeWidth={curStrokeWidth} strokeLinejoin={'round'} style={{ cursor: 'pointer' }} transform={transform}></path>);
            }
            if (elem.points) {
                return (<polygon 
                // @ts-ignore
                fill={colors[key]} key={key} data-key={key} points={elem.points} stroke={'black'} strokeWidth={curStrokeWidth} strokeLinejoin={'round'} style={{ cursor: 'pointer' }} transform={transform}></polygon>);
            }
            if (elem.line) {
                return (<line 
                // @ts-ignore
                // fill={colors[key]}
                x1={elem.line.x1} x2={elem.line.x2} y1={elem.line.y1} y2={elem.line.y2} key={key} data-key={key} stroke={'black'} strokeWidth={curStrokeWidth} strokeLinejoin={'round'} style={{ cursor: 'pointer' }} transform={transform}></line>);
            }
            return <></>;
        })}
              </svg>
        ...
        <Col xs={24} sm={24} md={24} lg={8} xl={8}>
          <div style={{
            ...
            <Card style={{ minWidth: 400, width: 400 }}>
              ...
              <Slider defaultValue={1} min={1} max={10} onChange={(v) => {
            setStrokeWidth(v);
        }}/>
              <Divider style={{ borderColor: '#7cb305' }} dashed>
                {intl.formatMessage({ id: 'draws.palette.color_setting' })}
              </Divider>
              <ColorPalette onSelectColor={handleColrChange} presetColors={presetColors} storageKey={storageKey} allKeys={keys}/>
            </Card>
          </div>
        </Col>
      </Row>
...
//# sourceMappingURL=DrawPalette.jsx.map