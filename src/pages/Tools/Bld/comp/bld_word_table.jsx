...
import { Table, Button, message, Upload, Space, Modal, Input, } from 'antd';
import { getSubKeyValue, setSubKeyValue } from '@/services/cubing-pro/key_value/keyvalue_store';
import { COL_KEYS, csvToJson, downloadFile, jsonToCsv, ROW_KEYS, } from '@/pages/Tools/Bld/comp/utils';
import { DownloadOutlined, UploadOutlined, } from '@ant-design/icons';
// todo 1 手动点击上传下载
// todo 2 本地只保存
// todo 3 再写一个modal框
const AssociativeWordTable = forwardRef((_, ref) => {
    ...
    const saveToLocalCache = (newData) => {
    ...
    return (<div style={{ overflow: 'auto' }}>
      <Space style={{ marginBottom: 16 }}>
        ...
        <Button icon={<DownloadOutlined />} onClick={() => downloadFile('联想词.json', JSON.stringify(data, null, 2), 'application/json')}>
          导出 JSON
        </Button>

        <Button icon={<DownloadOutlined />} onClick={() => downloadFile('联想词.csv', jsonToCsv(data), 'text/csv')}>
          导出 CSV
        </Button>
        ...
        <Upload showUploadList={false} accept=".csv" beforeUpload={file => {
            const reader = new FileReader();
            reader.onload = e => {
                try {
      ...
      <Table 
    // @ts-ignore
    columns={columns} dataSource={tableData} pagination={false} scroll={{ x: 'max-content', y: 600 }} bordered size="small"/>

      <Modal title={`请输入 ${editingKey} 的联想词`} open={modalOpen} onOk={handleModalOk} onCancel={handleModalCancel} okText="确认" cancelText="取消">
        <Input placeholder="请输入联想词" value={inputValue} onChange={e => setInputValue(e.target.value)} onPressEnter={handleModalOk}/>
      </Modal>
    </div>);
});
export default AssociativeWordTable;
//# sourceMappingURL=bld_word_table.jsx.map