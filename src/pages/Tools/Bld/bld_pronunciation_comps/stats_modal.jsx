...
const StatsModal = ({ visible, records, onClose }) => {
    const { token } = theme.useToken();
    const [activeMode, setActiveMode] = useState("pinyinToLetter");
    ...
    const recentColumns = [
        ...
        },
        {
            ...
            dataIndex: "isCorrect",
            key: "isCorrect",
            render: (isCorrect) => (<span style={{ color: isCorrect ? "#52c41a" : "#f5222d" }}>{isCorrect ? "正确" : "错误"}</span>),
        },
        {
            title: "用时(秒)",
            dataIndex: "timeSpent",
    ...
    return (<Modal title="练习统计" open={visible} onCancel={onClose} footer={null} width={800}>
      <Tabs activeKey={activeMode} onChange={(key) => setActiveMode(key)} items={items}/>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <Row gutter={16}>
          ...
          <Col span={6}>
            <Card>
              <Statistic title="正确率" value={overallStats.accuracy} precision={2} suffix="%" valueStyle={{ color: overallStats.accuracy >= 80 ? "#52c41a" : "#f5222d" }}/>
            </Card>
          </Col>
          <Col span={6}>
            <Card>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <Table dataSource={combinationStats} columns={combinationColumns} rowKey="combination" pagination={{ pageSize: 5 }} size="small"/>
      </div>

      <div>
        <Title level={4}>最近记录</Title>
        <Table dataSource={filteredRecords.slice().reverse().slice(0, 10)} columns={recentColumns} rowKey="timestamp" pagination={false} size="small"/>
      </div>
    </Modal>);
};
export default StatsModal;
//# sourceMappingURL=stats_modal.jsx.map