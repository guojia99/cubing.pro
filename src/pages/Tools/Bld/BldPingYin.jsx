...
export default function BldPingYin() {
    ...
    const [mode, setMode] = useState("pinyinToLetter");
    const [currentFirstLetter, setCurrentFirstLetter] = useState("");
    const [currentSecondLetter, setCurrentSecondLetter] = useState("");
    ...
    const applyToneToInput = useCallback((toneNumber) => {
    ...
    return (<ConfigProvider locale={zhCN} theme={{
      ...
      <div style={{
        ...
        <Title level={2} style={{ marginBottom: "20px" }}>
          拼音练习
          <Tooltip title="查看快捷键">
            <QuestionCircleOutlined style={{ fontSize: "16px", marginLeft: "10px", cursor: "pointer" }} onClick={() => setIsShortcutsModalVisible(true)}/>
          </Tooltip>
        </Title>
        ...
        <div style={{
          ...
          <Space direction="vertical" size="large" style={{ width: "300px" }}>
            <Input ref={inputRef} size="large" placeholder={mode === "pinyinToLetter" ? "请输入对应的字母组合" : "请输入对应的拼音"} value={userInput} onChange={(e) => setUserInput(e.target.value)} onPressEnter={checkAnswer} autoFocus/>

            {mode === "letterToPinyin" && (<div>
        ...
        <ConfigModal visible={isConfigModalVisible} config={config} onClose={() => setIsConfigModalVisible(false)} onSave={saveConfig}/>

        <StatsModal visible={isStatsModalVisible} records={records} onClose={() => setIsStatsModalVisible(false)}/>

        <ShortcutsModal visible={isShortcutsModalVisible} onClose={() => setIsShortcutsModalVisible(false)} mode={mode}/>
      </div>
    </ConfigProvider>);
}
//# sourceMappingURL=BldPingYin.jsx.map