...
const BldMemoryTest = () => {
    ...
    const getFailureRateData = (count) => {
    ...
    return (<div style={{ margin: 'auto', textAlign: 'center', padding: 20 }}>
      <Card title={'记忆练习工具'} style={{ marginBottom: 20 }} extra={<div>
      ...
      <Modal title="数据" open={isDatasetModalVisible} onCancel={() => setIsDatasetModalVisible(false)} footer={null}>
        {/* 切换按钮 */}
        <div style={{ marginBottom: 10 }}>
          <Button type={isInputMode ? 'primary' : 'default'} onClick={() => setIsInputMode(true)}>
            输入模式
          </Button>
          <Button type={!isInputMode ? 'primary' : 'default'} onClick={() => setIsInputMode(false)} style={{ marginLeft: 10 }}>
            展示模式
          </Button>
        </div>
        ...
        {/* 输入模式 */}
        {isInputMode && (<>
            <TextArea rows={10} placeholder="输入单词，每行一个，或用逗号、空格分隔" value={tempDataset} onChange={(e) => {
                setTempDataset(e.target.value);
            }} // 仅更新临时状态
        />
            <Button type="primary" style={{ marginTop: 10, float: 'right' }} onClick={() => {
        ...
        {/* 展示模式 */}
        {!isInputMode && (<Input.TextArea rows={10} value={Array.from(dataset).join('\n')} readOnly/>)}
      ...
      <Modal title="统计" open={isStatsModalVisible} onCancel={() => setIsStatsModalVisible(false)} footer={null} width="80%" style={{ maxHeight: '75vh', height: '75vh', top: '20px' }}>
        <Tabs>
          ...
          <Tabs.TabPane tab="最近" key="history">
            <List dataSource={history} renderItem={(item) => (<List.Item>
...
Z: ABC DEF GHJ
* */
//# sourceMappingURL=BldMeor.jsx.map