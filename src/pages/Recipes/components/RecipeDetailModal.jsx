...
import { addFavorite, isFavorite, removeFavorite, } from '../utils/favoriteStorage';
import { addToTonight, isInTonight, removeFromTonight, } from '../utils/tonightStorage';
import '../detail.less';
import './RecipeDetailModal.less';
const RECIPES_JSON = '/recipes.json';
const MAX_FAVORITES = 20;
const MAX_TONIGHT = 20;
const RecipeDetailModal = ({ open, category, id, onClose, onFavoriteChange, onTonightChange, }) => {
    ...
    useEffect(() => {
        ...
        fetch(RECIPES_JSON)
            .then((r) => r.json())
            .then((data) => {
            const recipe = data.recipes?.find((r) => r.category === category && r.id === id);
    ...
    const mdComponents = {
        a({ href, children, ...props }) {
            ...
            if (tipsTarget) {
                return (<a href="#" onClick={(e) => {
                        e.preventDefault();
                        onClose();
                        navigate(`/other/kitchen-skills/${encodeURIComponent(tipsTarget.category)}/${encodeURIComponent(tipsTarget.id)}`);
                    }} style={{ color: token.colorPrimary }} {...props}>
            {children}
          </a>);
            }
            if (href?.startsWith('#')) {
        ...
        img({ src, alt }) {
            ...
            return (<Image src={fullSrc} alt={alt || ''} style={{ maxWidth: '100%', borderRadius: 8, cursor: 'pointer' }} preview={{
                    mask: '点击放大',
                }}/>);
        },
        h1({ children, ...props }) {
    ...
    return (<Modal open={open} onCancel={onClose} footer={null} width="100%" className="recipe-detail-modal-fullscreen" styles={{
        ...
        }} centered destroyOnClose closable={false}>
      <div className="recipe-detail-modal-inner" style={{ padding: 24 }}>
        <div style={{
          ...
          <Button type="text" icon={<CloseOutlined />} onClick={onClose} style={{ borderRadius: 8 }}>
            关闭
          </Button>
        ...
        {loading ? (<div style={{ padding: 48, textAlign: 'center' }}>
            <Spin size="large"/>
          </div>) : (<div ref={contentRef} className="markdown-body recipe-detail-content" style={{
                padding: 24,
                borderRadius: 12,
...
//# sourceMappingURL=RecipeDetailModal.jsx.map