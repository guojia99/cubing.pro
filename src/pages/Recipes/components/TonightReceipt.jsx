...
import { clearTonightList, getCheckedIngredients, toggleIngredientChecked, } from '../utils/tonightStorage';
import './TonightReceipt.less';
const TonightReceipt = ({ recipes, tonightList, onClear, }) => {
    ...
    return (<>
      <div className="tonight-receipt" style={{
        ...
        <div className="tonight-receipt-body">
          <div className="tonight-receipt-dishes">
            {tonightRecipes.map((r) => (<div key={`${r.category}-${r.id}`} className="tonight-receipt-dish">
                {r.title.replace(/的做法$/, '')}
              </div>))}
          </div>
          ...
          <div className="tonight-receipt-ingredients">
            ...
            return (<div key={name} className="tonight-receipt-ingredient" style={{
                  ...
                  <span>
                    ...
                    {showTags && dishNames.length > 0 && (<span className="tonight-receipt-tag" style={{ marginLeft: 6 }}>
                        [{dishNames.join('、')}]
                      </span>)}
                  </span>
                </div>);
      ...
      <Modal title="清空今晚吃啥" open={clearModalOpen} onCancel={() => setClearModalOpen(false)} onOk={handleClearAll} okText="确认清空" cancelText="取消" okButtonProps={{ danger: true }}>
        <p>确定要清空「今晚吃啥」清单吗？已勾选的购物项也会被清除。</p>
      </Modal>
    </>);
};
export default TonightReceipt;
//# sourceMappingURL=TonightReceipt.jsx.map