...
const CompetitionDetail = ({ comp }) => {
    ...
    // );
    // 比赛性质
    bodys.push(<div key={'__comps_detail_status_key'}>
      <Card style={{ marginBottom: '20px' }}>
        <ProDescriptions column={1} title="比赛性质">
          ...
          {wca && (<ProDescriptions.Item label="认证">
              WCA认证
              <img src="https://cubing.com/f/images/wca.png" alt="wca_icon" style={{ width: 16, marginLeft: 8 }}/>
            </ProDescriptions.Item>)}
    ...
    // 城市
    bodys.push(<div key={'__comps_detail_city_key'}>
      <Card style={{ marginBottom: '20px' }}>
    ...
    if (comp?.data.OrganizersID) {
        bodys.push(<div key={'__comps_detail_org_key'}>
        <Card style={{ marginBottom: '20px' }}>
          <ProDescriptions column={1} title="主办团队">
            <ProDescriptions.Item label="主办">{comp.data.Org.Name}</ProDescriptions.Item>
            {comp.data.Org.Email && (<ProDescriptions.Item label="邮箱">{comp.data.Org.Email}</ProDescriptions.Item>)}
            {comp.data.Group.qq_groups && (<ProDescriptions.Item label="QQ">{comp.data.Group.qq_groups}</ProDescriptions.Item>)}
            {comp.data.Group.name && (<ProDescriptions.Item label="QQ群">{comp.data.Group.name}</ProDescriptions.Item>)}
          </ProDescriptions>
        </Card>
      </div>);
    }
    ...
    // todo 其他时间
    if (comp !== undefined) {
        bodys.push(<div key={'__comps_detail_times_key'}>
        <Card style={{ marginBottom: '20px' }}>
          <ProDescriptions column={1} title="比赛时间">
            ...
            {comp?.data.RegistrationStartTime && (<ProDescriptions.Item label="报名开始时间">
                {[parseDateTime(comp?.data.RegistrationStartTime)]}
              </ProDescriptions.Item>)}

            {comp?.data.RegistrationEndTime && (<ProDescriptions.Item label="报名结束时间">
                {[parseDateTime(comp?.data.RegistrationEndTime)]}
              </ProDescriptions.Item>)}

            {comp?.data.RegistrationRestartTime && (<ProDescriptions.Item label="报名重开时间">
                {[parseDateTime(comp?.data.RegistrationRestartTime)]}
              </ProDescriptions.Item>)}

            {comp?.data.RegistrationCancelDeadlineTime && (<ProDescriptions.Item label="退赛截止时间">
    ...
    // 比赛人数
    bodys.push(<div key={'__comps_detail_person_key'}>
      <Card style={{ marginBottom: '20px' }}>
    ...
    bodys.push(<div key={'__comps_detail_events_key'}>
      <Card style={{ marginBottom: '20px' }}>
    ...
    bodys.push(<div key={'__comps_detail_other_key'}>
      <Card style={{
            borderRadius: 8,
            minWidth: '100%',
        }}>
...
//# sourceMappingURL=CompetitionDetail.jsx.map