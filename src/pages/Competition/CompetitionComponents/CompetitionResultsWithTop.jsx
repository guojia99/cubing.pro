...
import { DBest, DNF, resultToBest, sortResults } from '@/components/Data/types/result';
import { Divider } from 'antd';
import React from 'react';
const CompetitionResultsWithTop = ({ 
// eslint-disable-next-line @typescript-eslint/no-unused-vars
comp, results, events, topRank, event_divider, records, with_best, }) => {
    if (events === undefined) {
        return <></>;
    }
    ...
    if (!event_divider) {
        return (<>
        {ResultsTable(dataSource, ['EventName', 'Best', 'Average', 'PersonName', 'Result'], records)}
      </>);
    }
    let body = [];
    let last_idx = 0;
    ...
    const push_body = (idx) => {
        ...
        {ResultsTable(dataSource.slice(last_idx, idx), ['Rank', 'PersonName', 'Best', 'Average', 'Result'], records)}
      </div>);
        let this_msg = '';
...
//# sourceMappingURL=CompetitionResultsWithTop.jsx.map