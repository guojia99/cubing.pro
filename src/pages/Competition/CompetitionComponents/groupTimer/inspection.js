...
export function isBlindEventId(eventId) {
    return eventId === '333bf' || eventId === '444bf' || eventId === '555bf' || eventId === '333mbf';
}
/** 显示用：按精度显示秒数 */
export function formatInspectionSeconds(mode, elapsedMs, precision) {
    if (precision === 'tenth') {
        if (mode === 'countup') {
...
export function inspectionPlus2FromElapsed(elapsedMs) {
    return elapsedMs > INSPECTION_SAFE_MS && elapsedMs <= INSPECTION_DNF_MS;
}
export function inspectionDnfFromElapsed(elapsedMs) {
