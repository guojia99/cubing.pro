...
export function effectiveTimeMs(rec) {
    if (rec.dns || rec.dnf || rec.timeMs === null) {
        return null;
    }
    return rec.plus2 ? rec.timeMs + 2000 : rec.timeMs;
}
export function formatMsForDisplay(ms, eventId) {
    const sec = ms / 1000;
    if (eventId === '333mbf') {
...
/** 短格式 x.xx 与 冒号格式 */
export function formatDualTimes(ms, eventId) {
    const sec = ms / 1000;
 ...
 */
export function roundBestMs(records, eventId) {
    let best = null;
 ...
 */
export function eventCompetitionAverage(timesMs, route, integer) {
