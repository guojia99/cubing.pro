...
export function getMainIndicesForRound(ctx, scheduleIdx) {
...
/** 全部主赛程 slot 顺序（跨轮） */
export function getAllMainSlots(ctx) {
    const slots = [];
...
export function getAttemptInRound(ctx, scheduleIdx, lineIndex) {
...
export function getFirstUnfilledMainSlot(ctx, solves, eventId) {
...
export function getNextMainSlot(ctx, cur) {
    const slots = getAllMainSlots(ctx);
    const key = (s) => `${s.scheduleIdx}|${s.lineIndex}`;
...
export function getPrevMainSlot(ctx, cur) {
    const slots = getAllMainSlots(ctx);
    const key = (s) => `${s.scheduleIdx}|${s.lineIndex}`;
    const i = slots.findIndex((s) => key(s) === key(cur));
    if (i <= 0) {
...
/** 该轮主赛程每一格是否均已录入 */
export function isRoundFullyRecorded(ctx, solves, eventId, scheduleIdx) {
    const mains = getMainIndicesForRound(ctx, scheduleIdx);
    if (mains.length === 0) {
        return false;
    }
...
export function roundHasMissingSlots(ctx, solves, eventId, scheduleIdx) {
    const mains = getMainIndicesForRound(ctx, scheduleIdx);
    for (const li of mains) {
        const k = makeSlotKey(eventId, scheduleIdx, li);
        if (!solves[k]) {
...
export function roundHasAnyRecorded(ctx, solves, eventId, scheduleIdx) {
    const mains = getMainIndicesForRound(ctx, scheduleIdx);
    for (const li of mains) {
        const k = makeSlotKey(eventId, scheduleIdx, li);
        if (solves[k]) {
 ...
 */
export function getRoundStatsScheduleIdx(ctx, solves, eventId, cursorScheduleIdx) {
...
export function isEventFullyRecorded(ctx, solves, eventId) {
    const slots = getAllMainSlots(ctx);
    for (const s of slots) {
        const k = makeSlotKey(eventId, s.scheduleIdx, s.lineIndex);
        if (!solves[k]) {
...
export function hasStartedEvent(ctx, solves, eventId) {
    const slots = getAllMainSlots(ctx);
    for (const s of slots) {
        const k = makeSlotKey(eventId, s.scheduleIdx, s.lineIndex);
        if (solves[k]) {
...
export function findIncompleteOtherEvents(contexts, solves, exceptEventId) {
    const ids = [];
...
export function getRoundExportNumber(scheduleIdx) {
    return scheduleIdx + 1;
}
export function findContextByEventId(contexts, eventId) {
    return contexts.find((c) => c.event.EventID === eventId);
}
export function defaultCursor(contexts) {
...
export function lastRecordedCursor(contexts, solves) {
    let best = null;
    for (const ctx of contexts) {
...
export function reopenCursor(contexts, solves) {
...
export function normalizeCursor(contexts, cursor) {
    ...
    const slots = getAllMainSlots(ctx);
    const hit = slots.find((s) => s.scheduleIdx === cursor.scheduleIdx && s.lineIndex === cursor.lineIndex);
    if (hit) {
