...
import { formatTimerSingleBlob, isMbfEventId, isRelayMetaEvent, parseScramblePuzzleIds, resolveStructuredSegments, } from '../scrambleSegments';
/**
 * 与 CompetitionScrambles 展示规则一致；连拧/多盲在计时器内合并为「一把」一行（正文含多段）。
 */
export function buildScrambleRowsForGroup(ssc, ev, baseEvent) {
...
export function getMainLineIndices(rows, ev) {
    const m = eventRouteM(ev.EventRoute);
...
/** 备打乱列表（Ex#） */
export function getExtraRows(rows) {
    return rows.filter((r) => r.isExtra);
}
//# sourceMappingURL=buildScrambleRows.js.map