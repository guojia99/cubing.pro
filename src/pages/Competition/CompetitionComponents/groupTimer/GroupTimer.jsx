...
import { Button, Card, Checkbox, ColorPicker, Drawer, Dropdown, Form, Input, InputNumber, Modal, Radio, Select, Space, Typography, message, } from 'antd';
...
import { findContextByEventId, findIncompleteOtherEvents, getAllMainSlots, getAttemptInRound, getFirstUnfilledMainSlot, getMainIndicesForRound, getNextMainSlot, getRoundExportNumber, getRoundStatsScheduleIdx, hasStartedEvent, isEventFullyRecorded, normalizeCursor, reopenCursor, roundHasAnyRecorded, roundHasMissingSlots, } from './navigation';
...
import { defaultGroupTimerUi, loadGroupTimerUiFromStorage, normalizeGroupTimerUi, saveGroupTimerUiToStorage, } from './uiConfig';
import { effectiveTimeMs, eventCompetitionAverage, formatMsForDisplay, roundBestMs, } from './stats';
import { makeSlotKey } from './types';
import { INSPECTION_DNF_MS, INSPECTION_WARN_ELAPSED_MS, formatInspectionSeconds, inspectionDnfFromElapsed, inspectionPlus2FromElapsed, isBlindEventId, } from './inspection';
...
function formatHistoryRecordedAt(iso) {
...
function exportRoundKey(eid, scheduleIdx) {
...
function formatResultLine(rec, eventId) {
...
function consumedExtraLineIndices(eventId, solves) {
    const set = new Set();
...
const GroupTimer = ({ comp, baseEvents, open, onClose }) => {
    ...
    const contexts = useMemo(() => buildEventContexts(comp.data.comp_json.Events, baseEvents), [comp.data.comp_json.Events, baseEvents]);
    const [store, setStore] = useState(() => loadGroupTimer(compId));
    ...
    const [mbfPhase, setMbfPhase] = useState(null);
    const [mbfAttemptedStr, setMbfAttemptedStr] = useState('');
    const [mbfSolvedStr, setMbfSolvedStr] = useState('');
    const [pendingSpare, setPendingSpare] = useState(null);
    const [listOpen, setListOpen] = useState(false);
    const [historyModalOpen, setHistoryModalOpen] = useState(false);
    const [exportOpen, setExportOpen] = useState(false);
    /** 按轮次 scheduleIdx 字符串勾选 */
    const [exportCheckedRounds, setExportCheckedRounds] = useState({});
    ...
    const [uiConfig, setUiConfig] = useState(() => loadGroupTimerUiFromStorage());
    const [uiForm] = Form.useForm();
    const holdStartMsRef = useRef(null);
    ...
    const shouldUseInspection = mode === 'timer' &&
    ...
    const commitRecord = useCallback((rec) => {
        if (!ctx) {
            return;
        }
    ...
    const commitTimeMs = useCallback(async (ms) => {
        if (!ctx || !currentRow) {
            return;
        }
        if (isMbf) {
    ...
    const finalizeTimerStop = useCallback((elapsed) => {
    ...
    const tryFinishInspectionAndStartSolve = useCallback((insElapsedMs) => {
    ...
    useEffect(() => {
        ...
        const onKeyUp = (e) => {
            ...
            if (lastSpaceCountingEmitAtRef.current !== null &&
    ...
    useEffect(() => {
        ...
        const onKeyUp = (e) => {
            ...
            spaceAwaitingKeyDownRef.current = false;
            const now = performance.now();
            if (lastSpaceAwaitingEmitAtRef.current !== null &&
    ...
    const onTimerTapPointerDown = useCallback((e) => {
    ...
    const onTimerTapPointerUp = useCallback((e) => {
    ...
    const onTimerTapPointerCancel = useCallback((e) => {
        if (holdPointerIdRef.current !== e.pointerId) {
            return;
        }
        holdPointerIdRef.current = null;
    ...
    const showBackToLatest = useMemo(() => {
        ...
        return (store.cursor.eventId !== eventId ||
    ...
    const incompleteOtherIds = useMemo(() => (ctx ? findIncompleteOtherEvents(contexts, store.solves, eventId) : []), [ctx, contexts, store.solves, eventId]);
    ...
    const selectedExportKeys = useMemo(() => Object.keys(exportCheckedRounds).filter((k) => exportCheckedRounds[k]), [exportCheckedRounds]);
    const exportPreviewText = useMemo(() => buildCompactExportLines(contexts, store.solves, selectedExportKeys), [contexts, store.solves, selectedExportKeys]);
    useEffect(() => {
        if (!exportOpen) {
            return;
        }
            ...
            for (const sr of c.scheduleRounds.filter((r) => !r.skipped)) {
                ...
                for (const li of mains) {
                    ...
                    out.push({
                        ...
                        isCurrent: eid === store.cursor.eventId &&
    ...
    if (comp.data.IsDone) {
        return createPortal(<div className="group-timer-fs">
    ...
    return createPortal(<>
      <div className={`group-timer-fs ${runningFullscreen ? 'group-timer-fs--minimal' : ''}`} style={{
            ...(uiConfig.fullscreenBg ? { backgroundColor: uiConfig.fullscreenBg } : {}),
            ['--gt-tap-min-vh']: `${uiConfig.timerTapMinVh}vh`,
        }}>
        {!runningFullscreen && (<>
            <div className="group-timer-corner-right">
              <Dropdown getPopupContainer={() => document.body} overlayClassName="group-timer-popup-z" overlayStyle={{ zIndex: 3100 }} menu={{
                items: [
                    ...(getToken()?.token
                        ? []
                        : [
        ...
        {runningFullscreen ? (<div className="group-timer-running" tabIndex={0} role="presentation" onClick={() => {
                const elapsed = performance.now() - startRef.current;
                finalizeTimerStop(elapsed);
            }} onKeyDown={(e) => {
            ...
            <div className="group-timer-top">
              <div className="group-timer-scramble-box" key={`${eventId}-${store.cursor.scheduleIdx}-${store.cursor.lineIndex}-${pendingSpare?.extraLineIndex ?? 'n'}`} style={{ fontSize: uiConfig.scrambleFontPx }}>
                {typeof scrambleDisplay === 'string' ? scrambleDisplay : scrambleDisplay}
              </div>
            </div>

            {ctx ? (<div className="group-timer-meta-bar" style={{ fontSize: uiConfig.metaFontPx }}>
                <div className="group-timer-meta-bar__row">
                  <Select className="group-timer-meta-select" value={store.cursor.eventId} onChange={(ev) => {
                ...
                }))} {...selectPopupProps} getPopupContainer={() => document.body} popupMatchSelectWidth={false}/>
                  <Select className="group-timer-meta-select" value={store.cursor.scheduleIdx} onChange={(sch) => {
                    setStore((s) => {
                ...
                }} options={findContextByEventId(contexts, store.cursor.eventId)?.scheduleRounds
                    .filter((r) => !r.skipped)
                    .map((r) => ({ label: r.roundTitle, value: r.scheduleIdx })) ?? []} {...selectPopupProps} getPopupContainer={() => document.body} popupMatchSelectWidth={false}/>
                  <Select className="group-timer-meta-select group-timer-meta-select--attempt" value={currentRoundMains.includes(store.cursor.lineIndex)
                    ? store.cursor.lineIndex
                    : (currentRoundMains[0] ?? store.cursor.lineIndex)} disabled={currentRoundMains.length === 0} onChange={(lineIndex) => {
                    setStore((s) => ({
                        ...s,
            ...
            <div className="group-timer-stack group-timer-stack--wide">
              ...
              {inputLocked && currentSlotRec ? (<div className="group-timer-current-result">
                  {formatResultLine(currentSlotRec, eventId)}
                </div>) : null}

              {isMbf && (<div className="group-timer-mbf">
                  {!mbfPhase && (<>
                      <Text type="secondary">多盲：先输入本次尝试魔方数</Text>
                      <Input inputMode="numeric" placeholder="尝试个数" value={mbfAttemptedStr} disabled={inputLocked} onChange={(e) => setMbfAttemptedStr(e.target.value.replace(/\D/g, ''))}/>
                      <Button type="primary" disabled={inputLocked} onClick={() => {
                  ...
                  {mbfPhase === 'time' && (<>
                      ...
                      {mode === 'manual' ? (<div className="group-timer-input-row group-timer-input-row--inline">
                          <Input value={manualRaw} disabled={inputLocked} onChange={(e) => setManualRaw(e.target.value)} placeholder="时间" onPressEnter={onManualConfirm}/>
                          <Button type="primary" disabled={inputLocked} onClick={onManualConfirm}>
                            确认时间
                          </Button>
                        </div>) : (<div className={`group-timer-tap-area group-timer-tap-area--compact${inputLocked ? ' group-timer-tap-area--locked' : ''}`} tabIndex={0} role="presentation" onPointerDown={onTimerTapPointerDown} onPointerUp={onTimerTapPointerUp} onPointerCancel={onTimerTapPointerCancel}>
                          {!running ? (<Text className="group-timer-tap-area__label">按住 · 松手开始计时</Text>) : (<Text type="secondary">计时中…</Text>)}
                        </div>)}
                    </>)}
                  {mbfPhase === 'solved' && (<>
                      <Text>还原个数（已成功）</Text>
                      <Input inputMode="numeric" value={mbfSolvedStr} disabled={inputLocked} onChange={(e) => setMbfSolvedStr(e.target.value.replace(/\D/g, ''))}/>
                      <Button type="primary" disabled={inputLocked} onClick={() => {
                        const ms = displayMs;
                        if (!ms && mode === 'manual') {
              ...
              {!isMbf && (<div className={`group-timer-input-row${mode === 'manual'
                    ? ' group-timer-input-row--inline'
                    : !inputLocked
                        ? ' group-timer-input-row--timer-fill'
                        : ''}`}>
                  {mode === 'manual' ? (<>
                      <Input size="large" value={manualRaw} disabled={inputLocked} onChange={(e) => setManualRaw(e.target.value)} placeholder="输入时间" onPressEnter={onManualConfirm}/>
                      <Button type="primary" size="large" disabled={inputLocked} onClick={onManualConfirm}>
                        确认
                      </Button>
                    </>) : (<div className={`group-timer-tap-area${inputLocked ? ' group-timer-tap-area--locked' : ''}`} tabIndex={0} role="presentation" onPointerDown={onTimerTapPointerDown} onPointerUp={onTimerTapPointerUp} onPointerCancel={onTimerTapPointerCancel}>
                      {!running ? (shouldUseInspection ? (inspectionCounting ? (<div className="group-timer-inspection">
                              <div className={`group-timer-inspection__value${uiConfig.inspectionDisplayMode === 'countdown'
                            ...
                            : ''}`}>
                                {formatInspectionSeconds(uiConfig.inspectionDisplayMode, inspectionElapsedMs, uiConfig.inspectionPrecision)}
                              </div>
                              <Text type="secondary" className="group-timer-inspection__hint">
                                长按开始计时
                              </Text>
                            </div>) : (<Text className="group-timer-tap-area__label">点击计时区开始观察</Text>)) : (<Text className="group-timer-tap-area__label">按住 · 松手开始计时</Text>)) : (<Text type="secondary">计时中…</Text>)}
                    </div>)}
                </div>)}
              ...
              <Space wrap size="small" className="group-timer-actions">
                <Button size="small" className="group-timer-btn-dnf" onClick={() => (inputLocked ? toggleCurrentDnf() : onDnf())}>
                  DNF
                </Button>
                <Button size="small" className="group-timer-btn-plus2" disabled={!inputLocked || isMbf || Boolean(currentSlotRec?.dns)} onClick={toggleCurrentPlus2}>
                  +2
                </Button>
                {!isMbf && (<Button size="small" className="group-timer-btn-dns" disabled={inputLocked && !currentSlotRec?.dns} onClick={() => {
                ...
                {!isMbf && (<Button size="small" disabled={inputLocked || !hasAnyRecordInCursorRound} onClick={onSpare}>
            ...
            <div className="group-timer-side-row">
              <div className="group-timer-side-row__left">
                <div className="group-timer-history-trigger">
                  <Button type="default" size="small" className="group-timer-history-open-btn" icon={<UnorderedListOutlined />} onClick={() => setHistoryModalOpen(true)}>
                    成绩收纳
                ...
                <div className="group-timer-bl group-timer-bl--stats">
                  <div>
                    ...
                    {statsScheduleIdx !== store.cursor.scheduleIdx && statsRoundTitle ? (<Text type="secondary" style={{ marginLeft: 6 }}>
                        （{statsRoundTitle}）
                      </Text>) : null}
                  </div>
                ...
                : roundRecords.map((x) => formatResultLine(x.rec, eventId)).join('，')}
                    {statsScheduleIdx !== store.cursor.scheduleIdx && statsRoundTitle ? (<Text type="secondary"> （{statsRoundTitle}）</Text>) : null}
                  </div>
                  <div className="group-timer-small">
                    本场该项目平均：{eventAvg ?? '—'}
                  </div>
                ...
                {showBackToLatest && nextPlaySlot ? (<div className="group-timer-back-latest">
                    <Button type="primary" size="small" onClick={() => setStore((s) => ({
                    ...s,
              ...
              {currentRow &&
                ctx &&
                !shouldHideScrambleImage(eventId, ctx.baseEvent) && (<aside className="group-timer-side-row__right" aria-label="打乱图示">
                    <div className="group-timer-scramble-preview">
                      <ScrambleImageMini key={`${eventId}-${store.cursor.scheduleIdx}-${store.cursor.lineIndex}-${(pendingSpare ? pendingSpare.spareScramble : currentRow.scramble).slice(0, 32)}`} className="group-timer-scramble-preview__img" scramble={pendingSpare ? pendingSpare.spareScramble : currentRow.scramble} puzzleId={currentRow.puzzleIdForImage}/>
                    </div>
                  </aside>)}
            </div>
            </div>
        ...
        {timerHoldOverlay && !running && (<div className="group-timer-hold-overlay" aria-hidden>
            <span className="group-timer-hold-overlay__text">松手开始计时</span>
          </div>)}
      </div>

      <Drawer title="群赛计时 · 设置" placement="right" open={settingsOpen} onClose={() => setSettingsOpen(false)} width={380} zIndex={3100} rootClassName="group-timer-popup-z" footer={<Button type="primary" block onClick={() => setSettingsOpen(false)}>
            关闭
          </Button>}>
        ...
        <Form form={uiForm} layout="vertical" onValuesChange={() => scheduleUiSettingsPersist()}>
        ...
        ]} getPopupContainer={() => document.body} {...selectPopupProps}/>
            </Form.Item>
          ...
          <Form.Item label="全屏背景色" name="fullscreenBg" extra="留空则使用主题默认；可点清除恢复默认" getValueFromEvent={(color) => {
            if (color == null) {
                return '';
            }
            ...
            <ColorPicker showText format="hex" allowClear disabledAlpha style={{ width: '100%' }} presets={[
            {
                label: '浅色',
      ...
      <Drawer title="成绩列表（点击成绩跳转）" open={listOpen} onClose={() => setListOpen(false)} width={480} zIndex={3100} rootClassName="group-timer-popup-z">
        {contexts.map((c) => {
            const eid = c.event.EventID;
            return (<div key={eid} className="group-timer-list-event">
              <div className="group-timer-list-event-title">{CubesCn(eid)}</div>
              {c.scheduleRounds
                    .filter((r) => !r.skipped)
                    .map((sr) => {
                    const mains = getMainIndicesForRound(c, sr.scheduleIdx);
                    return (<div key={sr.scheduleIdx} className="group-timer-list-round">
                      ...
                      <div className="group-timer-list-scores">
                        {mains.map((li) => {
                            ...
                            return (<button key={k} type="button" className="group-timer-list-score-chip" onClick={() => {
                                    setStore((st) => ({
                                        ...st,
      ...
      <Modal title={<span className="group-timer-history-modal__title">
            历史成绩
            <Text type="secondary" className="group-timer-history-modal__title-hint">
              按还原时间倒序 · 点击切换
            </Text>
          </span>} open={historyModalOpen} onCancel={() => setHistoryModalOpen(false)} footer={<div className="group-timer-history-modal__footer">
          ...
          </div>} width={420} centered destroyOnClose zIndex={3100} rootClassName="group-timer-popup-z" className="group-timer-history-modal" styles={{
        ...
        {historyEntries.length === 0 ? (<div className="group-timer-muted group-timer-history-empty">暂无已录成绩</div>) : (<div className="group-timer-history-modal__list">
            {historyEntries.map((h) => (<div key={h.key} className={`group-timer-history-item${h.isCurrent ? ' group-timer-history-item--current' : ''}`}>
                <button type="button" className="group-timer-history-item__main group-timer-history-item__main--modal" onClick={() => {
                    setStore((s) => ({
                        ...s,
                        cursor: {
                            eventId: h.eventId,
      ...
      <Modal title="本项目全部轮次已完成" open={eventDoneGuideOpen} onOk={dismissEventDoneGuide} onCancel={dismissEventDoneGuide} okText="知道了" cancelButtonProps={{ style: { display: 'none' } }} zIndex={3100} rootClassName="group-timer-popup-z" width={480}>
        <p>您当前项目的所有轮次、所有把次均已录入成绩。</p>
        {incompleteOtherIds.length > 0 ? (<div style={{ marginTop: 12 }}>
            <p style={{ marginBottom: 8 }}>
              您可通过顶部项目下拉框或「更多」里的「成绩列表」切换至尚未完成的其它项目：
            </p>
            <Space wrap>
              {incompleteOtherIds.map((id) => (<Button key={id} type="link" size="small" onClick={() => {
          ...
          </div>) : (<p className="group-timer-muted" style={{ marginTop: 12 }}>
            本场暂无其它未完成项目。
          </p>)}
      </Modal>

      <Modal title="导出成绩（全部项目）" open={exportOpen} onCancel={() => setExportOpen(false)} footer={<Space>
          ...
          </Space>} width={600} zIndex={3100} rootClassName="group-timer-popup-z">
        <div className="group-timer-export-check-scroll">
          <Checkbox.Group className="group-timer-export-check-group" value={Object.keys(exportCheckedRounds).filter((k) => exportCheckedRounds[k])} onChange={(vals) => {
            const set = new Set(vals);
            const n = { ...exportCheckedRounds };
            ...
            return c.scheduleRounds
                ...
                const filledCount = mains.filter((li) => store.solves[makeSlotKey(eid, r.scheduleIdx, li)]).length;
                return (<Checkbox key={k} value={k}>
                      {CubesCn(eid)} · {r.roundTitle}（第 {getRoundExportNumber(r.scheduleIdx)} 轮）
                      {miss ? (<span className="group-timer-muted">
        ...
        <Input.TextArea style={{ marginTop: 12, fontFamily: 'ui-monospace, monospace' }} rows={10} value={exportDraft} onChange={(e) => setExportDraft(e.target.value)} placeholder={hasExportableRoundAnyEvent
            ? '勾选轮次后在此预览，可编辑后再复制'
            : '请先在任意轮次录入至少一把成绩（含 DNF）'}/>
        {!hasExportableRoundAnyEvent ? (<p className="group-timer-muted" style={{ marginTop: 8 }}>
            请先在任意轮次录入至少一把成绩（含 DNF）后，该轮才会出现在导出列表中。
          </p>) : null}
      </Modal>

      <Modal title="项目进度（点击切换项目）" open={eventOverviewOpen} onCancel={() => setEventOverviewOpen(false)} footer={null} width={520} zIndex={3100} rootClassName="group-timer-popup-z group-timer-overview-modal-root" className="group-timer-overview-modal" destroyOnClose styles={{ body: { maxHeight: 'min(72vh, 560px)', overflowY: 'auto', paddingTop: 8 } }}>
        ...
        {contexts.map((c) => {
            ...
            return (<div key={id} className={`group-timer-overview-row ${rowTone}`} role="button" tabIndex={0} onClick={() => {
              ...
              <div className="group-timer-overview-row__inner">
                ...
                <div className="group-timer-overview-rounds">
                  {c.scheduleRounds
                    ...
                    return (<div key={sr.scheduleIdx} className="group-timer-overview-round-line">
                          {sr.roundTitle}（第 {getRoundExportNumber(sr.scheduleIdx)} 轮）：已录 {filled}/{total} 把
                          {filled === total && total > 0 ? (<span className="group-timer-overview-round-done"> · 已满</span>) : null}
                        </div>);
                })}
                </div>
              </div>
...
//# sourceMappingURL=GroupTimer.jsx.map