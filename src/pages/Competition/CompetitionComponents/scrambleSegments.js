...
export function parseScramblePuzzleIds(scrambleValue) {
...
export function isMbfEventId(eventId) {
    return eventId === '333mbf';
}
/** 连拧等：class 或 id 含 relay（events 接口含 class 字段） */
export function isRelayMetaEvent(base) {
    const ext = base;
    const cls = ext.class ?? '';
    if (cls.includes('连拧')) {
        return true;
...
/** 连拧 / 多盲：打乱区不生成 cube 图 */
export function shouldHideScrambleImage(eventId, base) {
 ...
 */
export function splitScrambleBlob(blob, expectCount) {
    if (expectCount < 1) {
        return null;
    }
    const t = blob.trim();
...
export function resolveStructuredSegments(ssc, puzzleIds) {
...
/** 群赛计时器单行：连拧为「222\\n打乱\\n\\n333\\n…」；多盲为「#1\\n…」 */
export function formatTimerSingleBlob(segments, puzzleIds, isMbf) {
    return segments
        .map((s, i) => {
        if (isMbf) {
            return `#${i + 1}\n${s}`;
...
//# sourceMappingURL=scrambleSegments.js.map