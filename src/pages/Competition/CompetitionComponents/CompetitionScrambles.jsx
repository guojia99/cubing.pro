...
import { isMbfEventId, isRelayMetaEvent, parseScramblePuzzleIds, resolveStructuredSegments, shouldHideScrambleImage, } from './scrambleSegments';
...
const ScrambleEventRounds = ({ eventKey, rounds, rowClassName, isCompact, }) => {
    ...
    const renderTableOrMobile = (tb) => isCompact ? (<div className="scramble-mobile-list">
        {tb.map((row, idx) => (<div key={`${row.Index}-${idx}`} className="scramble-mobile-item">
      ...
      </div>) : (<Table style={{ marginTop: 0, marginBottom: 0 }} dataSource={tb} rowKey={(record, index) => `${record.Index}-${index}`} size={'small'} rowClassName={rowClassName} scroll={{ x: 'max-content' }} columns={[
            {
                title: '序号',
                dataIndex: 'Index',
                key: 'index',
            ...
            },
            {
                ...
                minWidth: 400,
                onCell: () => ({ style: { verticalAlign: 'top' } }),
                render: (_, record) => (<div className="scramble-pair-frame">
    ...
    return (<div className="scramble-round-block">
      {showToolbar ? (<div className="scramble-group-toolbar scramble-toolbar-combo">
          {showRoundSelect ? (<>
              <span className="scramble-group-toolbar__label">赛程</span>
              <Select className="scramble-group-toolbar__select" value={safeRoundIdx} onChange={setRoundIndex} options={rounds.map((r, i) => ({ label: r.roundTitle, value: i }))} popupMatchSelectWidth={false}/>
            </>) : null}
          {showGroupSelect ? (<>
              <span className="scramble-group-toolbar__label scramble-toolbar-combo__gap">打乱组</span>
              <Select className="scramble-group-toolbar__select" value={safeGroupIdx} onChange={setGroupIndex} options={groups.map((g, i) => ({ label: g.label, value: i }))} popupMatchSelectWidth={false}/>
            </>) : null}
        </div>) : null}
      {currentRound ? (<h3 className="scramble-round-title">
          <strong>{currentRound.roundTitle}</strong>
        </h3>) : null}
      {currentGroup ? (<div className="scramble-group-panel">{renderTableOrMobile(currentGroup.tb)}</div>) : null}
    </div>);
};
const CompetitionScrambles = ({ comp }) => {
    ...
    const scrambleValue = (sc, compact) => {
        return (<p className={compact ? 'scramble-text scramble-text--compact' : 'scramble-text'} style={{
                whiteSpace: 'pre-line',
                ...(compact ? {} : { width: 500 }),
                wordWrap: 'break-word',
    ...
    const scrambleTable = (ev) => {
        ...
        const scrambleImageCell = (sc, evForImg) => hideImg ? (<span className="scramble-image-none" aria-hidden/>) : (<ScrambleImage sc={sc} ev={evForImg}/>);
        const buildGroupRows = (ssc) => {
            const tb = [];
            ...
            if (puzzleIds.length >= 2) {
                ...
                for (let evIdx = 0; evIdx < puzzleIds.length; evIdx++) {
                    tb.push({
                        ...
                        Scramble: scrambleValue(ssc[evIdx], isCompact),
                        ScrambleImage: hideImg ? (<span className="scramble-image-none" aria-hidden/>) : (<ScrambleImage sc={ssc[evIdx]} ev={puzzleIds[evIdx]}/>),
                    });
                }
                return tb;
            }
        ...
        return (<ScrambleEventRounds key={ev.EventID} eventKey={ev.EventID} rounds={scheduleRounds} rowClassName={rowClassName} isCompact={isCompact}/>);
    };
    const items = [];
    ...
    return (<>
      <NavTabs type="line" items={items} tabsKey="scrambles_key" indicator={{ size: (origin) => origin - 20, align: 'center' }}/>
      {(() => {
            ...
            const btn = (<Button type="primary" className="competition-scrambles-group-timer-btn" disabled={groupTimerDisabled} onClick={openGroupTimer}>
            群赛计时器
          </Button>);
            return tip ? (<Tooltip title={tip}>
            <span style={{ display: 'inline-block' }}>{btn}</span>
          </Tooltip>) : (btn);
        })()}
      <GroupTimer comp={comp} baseEvents={baseEvents} open={groupTimerOpen} onClose={() => {
...
//# sourceMappingURL=CompetitionScrambles.jsx.map