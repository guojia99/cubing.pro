...
import { apiCreateSportResult, apiDeleteSportResult, apiGetSportEvents, apiGetSportResults, } from '@/services/cubing-pro/sports/sports';
import { ProTable } from '@ant-design/pro-components';
import { Button, DatePicker, Form, Input, Modal, Popconfirm, Select, message } from 'antd';
import dayjs from 'dayjs';
import React, { useEffect, useRef, useState } from 'react';
function parseTimeToSeconds(inputTime) {
    ...
    if (reversed[0])
        seconds += parseFloat(reversed[0]); // 秒（含毫秒）
    if (reversed[1])
        seconds += parseInt(reversed[1]) * 60; // 分
    if (reversed[2])
        seconds += parseInt(reversed[2]) * 3600; // 时
    return seconds;
}
...
const SportResultList = () => {
    ...
    return (<>
      ...
      <ProTable rowKey="id" actionRef={actionRef} search={false} columns={[
        ...
        }} columnsState={{}} pagination={{ pageSize: 10 }}/>

      <Modal title="添加运动成绩" open={modalOpen} onCancel={() => setModalOpen(false)} onOk={handleCreate} destroyOnClose>
        <Form form={form} layout="vertical">
          <Form.Item name="event_id" label="项目" rules={[{ required: true }]}>
            <Select showSearch options={events.map((event) => ({
            label: event.name,
            value: event.id,
          ...
          <Form.Item label="用户" required>
            <AdminUserSearchSelect placeholder="输入姓名或 CubeID 搜索并选择用户" onPlayerChange={(p) => {
            form.setFieldsValue({
                user_id: p?.id,
                cube_id: p?.CubeID,
            });
          ...
          <Form.Item name="result" label="成绩时间" rules={[{ required: true, message: '请输入成绩时间' }]}>
...
//# sourceMappingURL=Sports.jsx.map