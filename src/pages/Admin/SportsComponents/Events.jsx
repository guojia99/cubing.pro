import { apiCreateSportEvent, apiDeleteSportEvent, apiGetSportEvents, } from '@/services/cubing-pro/sports/sports';
import { ProTable } from '@ant-design/pro-components';
import { Button, Form, Input, Modal, Popconfirm, Select, message } from 'antd';
import React, { useState } from 'react';
import { GiRunningShoe, GiTrail, GiSprint, GiMountainClimbing, GiRoad, GiCycling, GiSwimfins, } from 'react-icons/gi';
import { FaBiking, FaSwimmer, FaHiking, FaWalking, } from 'react-icons/fa';
...
const SportEventList = () => {
    ...
    return (<>
      ...
      <ProTable rowKey="id" actionRef={actionRef} columns={[
            {
                title: '项目名',
                dataIndex: 'name',
                render: (text, record) => {
      ...
      <Modal title="新建项目" open={modalOpen} onCancel={() => setModalOpen(false)} onOk={handleCreate}>
        <Form form={form} layout="vertical">
          <Form.Item name="name" label="项目名" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item name="icon" label="图标">
            <Select options={iconOptions.map((item) => ({
            label: (<span>
