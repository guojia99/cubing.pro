...
export default function UserInfo(user) {
    ...
    const updateFiled = async (key, record, originRow) => {
        const canUpdateF = ['Name', 'EnName', 'WcaID', 'QQ', 'Sex', 'Birthdate', 'Sign'];
        if (!canUpdateF.find((value) => {
            return value === key;
        })) {
    ...
    return (<Card>
      <ProDescriptions actionRef={actionRef} title="个人信息" request={getUser} 
    // @ts-ignore
    columns={baseColumns}>
        <ProDescriptions.Item span={3} contentStyle={{ maxWidth: '80%' }} label="权限" 
    // @ts-ignore
    render={(value) => {
        ...
        <ProDescriptions.Item span={3} valueType="text" contentStyle={{ maxWidth: '80%' }} copyable={true} ellipsis label="QQ唯一验证码">
          {user.data.QQUniID}
        </ProDescriptions.Item>
      </ProDescriptions>
      <Divider />
      <ProDescriptions actionRef={actionRef} tooltip="可修改的用户数据, 点击某项进行修改,无法修改的内容需要联系管理员进行修正" request={getUser} editable={{
            // @ts-ignore
            onSave: updateFiled,
        }} 
    // @ts-ignore
    columns={editColumns}></ProDescriptions>
    </Card>);
}
//# sourceMappingURL=UserInfo.jsx.map