...
import { Avatar, Button, Card, Input, Modal, Spin, Tabs, Upload, message } from 'antd';
import React, { useCallback, useRef, useState } from 'react';
import ReactCrop from 'react-image-crop';
import 'react-image-crop/dist/ReactCrop.css';
export default function AvatarWithUpdate(user) {
    ...
    const items = [
        {
          ...
          {uploadedImage && (<ReactCrop 
                // @ts-ignore
                src={uploadedImage} crop={crop} onChange={(newCrop) => setCrop(newCrop)} onComplete={(c) => setCompletedCrop(c)} aspect={1}>
        ...
        },
        {
            ...
            label: '链接上传',
            children: (<div>
          <Input placeholder="你的远程头像链接(注意是否可访问)" value={imageUrl} onChange={handleUrlChange}/>
    ...
    return (<div>
      <Card style={{ marginBottom: '20px', textAlign: 'center' }}>
        <Avatar style={{ backgroundColor: '#68d0ce' }} size={{ xs: 100, sm: 100, md: 100, lg: 100, xl: 120, xxl: 120 }} src={<img src={AvatarURL(user.data.Avatar)} alt=""/>} icon={<UserOutlined />} onClick={showModal}/>
        <h3 style={{ marginTop: '20px' }}>
          <strong>{user.data.Name}</strong>
        </h3>
      </Card>

      <Modal title="更新头像" open={isModalVisible} onOk={handleSubmit} onCancel={handleCancel} maskClosable={!uploading} closable={!uploading} okButtonProps={{ disabled: uploading }} cancelButtonProps={{ disabled: uploading }}>
