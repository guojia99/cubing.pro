...
export const CaptchaCodeTab = ({ form, update, }) => {
    ...
    return (<div style={{ display: 'flex' }}>
      <img style={{
            ...
            borderRadius: '5px',
            marginTop: '2px',
        }} src={code.image} onClick={autoUpdateCode} alt={'点击刷新'}/>
      <ProFormText name="verifyId" hidden={true}>
        {code.id}
      </ProFormText>
      <ProFormText style={{ width: '70%', height: '35px' }} name="verifyValue" fieldProps={{
            size: 'large',
            prefix: <SafetyOutlined />,
        }} placeholder={'输入图形验证码'} rules={[
            {
                required: true,
                message: '验证码是必填项!',
            },
...
export const LoginTab = () => {
    return (<>
      {/*cubeID*/}
      <ProFormText name="loginID" fieldProps={{
            size: 'large',
            prefix: <UserOutlined />,
        }} placeholder={'CubeId / Email / LoginId'} rules={[
            {
                required: true,
...
const LoginFormC = () => {
    ...
    return (<LoginForm form={form} title="账号密码登录（旧版）" initialValues={{ autoLogin: true }} onFinish={async (values) => {
...
const OldLogin = () => {
    return (<div style={{
      ...
      {' / '}
      <Link to={'/login'} style={{ textAlign: 'center', color: '#a09d9d' }}>
        使用 WCA 登录
      </Link>
    </div>);
};
export default OldLogin;
//# sourceMappingURL=OldLogin.jsx.map