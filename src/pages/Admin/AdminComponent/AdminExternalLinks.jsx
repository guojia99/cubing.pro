...
import ExternalLinksGroupedView, { EXTERNAL_LINKS_LIST_DESC_MAX, } from '@/pages/ExternalLinks/ExternalLinksGroupedView';
import { ExternalLinkIconField } from '@/pages/ExternalLinks/externalLinkIconField';
import '@/pages/ExternalLinks/ExternalLinks.less';
import { findGroupForLinkKey, groupDropId, groupSortId, MAX_TOP_LINKS, moveLinkBetweenGroups, newEmptyLink, parseGroupDropId, parseGroupSortId, pruneOrphanLinks, removeGroup, renameGroup, reorderGroupsOrder, reorderKeysInGroup, toggleTop, } from '@/pages/ExternalLinks/utils';
import { getOtherLinksWithAdmin, setOtherLinksWithAdmin, } from '@/services/cubing-pro/auth/system';
...
function linkMapFrom(links) {
    const m = new Map();
    for (const l of links)
        m.set(l.key, l);
    return m;
}
const SortableGroupRow = ({ id, children }) => {
    ...
    return (<div ref={setNodeRef} style={style}>
      <div style={{
        ...
        <span {...attributes} {...listeners} style={{ cursor: 'grab', userSelect: 'none', fontSize: 16 }} title="拖动排序分组">
          ⋮⋮
        </span>
        <span>拖动整组排序</span>
      </div>
...
const SortableLinkWrap = ({ id, link, admin }) => {
    const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
        id,
    });
    const style = {
    ...
    return (<div ref={setNodeRef} style={style} className="external-links-sortable-link">
      <ExternalLinkCard link={link} descMaxChars={EXTERNAL_LINKS_LIST_DESC_MAX} admin={{
            ...admin,
            dragHandleProps: { ...listeners, ...attributes },
        }}/>
    </div>);
};
const GroupDropZone = ({ groupName, children }) => {
    const { setNodeRef, isOver } = useDroppable({
        id: groupDropId(groupName),
    });
    return (<div ref={setNodeRef} className={classNames('external-links-group-drop', isOver && 'external-links-group-drop--over')}>
      {children}
    </div>);
};
const AdminExternalLinks = () => {
    ...
    const [data, setData] = useState(emptyOtherLinks());
    ...
    return (<PageContainer title={intl.formatMessage({ id: 'menu.ExternalLinks.adminTitle' })} loading={loading}>
      <Space style={{ marginBottom: 16 }} wrap>
        <Segmented options={[
            { label: '编辑', value: 'edit' },
            { label: '展示', value: 'preview' },
        ]} value={viewMode} onChange={(v) => setViewMode(v)}/>
        {viewMode === 'edit' && (<>
      ...
      {viewMode === 'preview' ? (<>
          {data.groups.length === 0 ? (<Card size="small" style={{ marginBottom: 16 }}>
              暂无分组，请先切换到「编辑」并添加分组与链接。
            </Card>) : (<ExternalLinksGroupedView data={data}/>)}
        </>) : (<>
      {data.groups.length === 0 && (<Card size="small" style={{ marginBottom: 16 }}>
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={groupSortItems} strategy={verticalListSortingStrategy}>
          {data.groups.map((g) => {
                ...
                return (<SortableGroupRow key={g} id={groupSortId(g)}>
                <Card title={<Space wrap>
                    ...
                    </Space>} extra={<Button type="primary" size="small" onClick={() => openAddLink(g)}>
                      添加链接
                    </Button>} styles={{ body: { padding: 16 } }}>
                  <SortableContext id={`links-${g}`} items={keys} strategy={rectSortingStrategy}>
                    <div className="external-links-page">
                      <GroupDropZone groupName={g}>
                        ...
                        return (<SortableLinkWrap key={key} id={key} link={link} admin={{
                                pinned,
                        ...
                        {keys.length === 0 && (<div style={{ color: 'var(--ant-color-text-tertiary)', fontSize: 13 }}>
                            该分组暂无链接（可拖入其他分组的卡片）
                          </div>)}
                      </GroupDropZone>
                    </div>
      ...
      <Modal title={editingKey ? '编辑链接' : '添加链接'} open={linkModalOpen} onOk={submitLink} onCancel={() => setLinkModalOpen(false)} destroyOnClose width={580}>
      ...
      <Modal title="添加分组" open={groupModalOpen} onOk={addGroup} onCancel={() => setGroupModalOpen(false)}>
        <Input placeholder="分组名称" value={newGroupName} onChange={(e) => setNewGroupName(e.target.value)} onPressEnter={addGroup}/>
      </Modal>

      <Modal title="重命名分组" open={renameModalOpen} onOk={confirmRename} onCancel={() => setRenameModalOpen(false)}>
        <Input value={renameValue} onChange={(e) => setRenameValue(e.target.value)} onPressEnter={confirmRename}/>
      </Modal>

      <Modal title="移动到分组" open={moveModalOpen} onOk={applyMove} onCancel={() => setMoveModalOpen(false)}>
        <Select style={{ width: '100%' }} value={moveTargetGroup} onChange={setMoveTargetGroup} options={data.groups
            .filter((g) => moveKey && findGroupForLinkKey(data, moveKey) !== g)
            .map((x) => ({ label: x, value: x }))}/>
      </Modal>

      <Modal title={`删除分组「${deleteGroupName}」`} open={deleteGroupOpen} onCancel={() => setDeleteGroupOpen(false)} footer={<Space wrap>
...
//# sourceMappingURL=AdminExternalLinks.jsx.map