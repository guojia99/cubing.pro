...
import { apiAdminCreatePlayer, apiAdminPlayers, apiAdminUpdatePlayerName, apiAdminUpdatePlayerWCAID, apiMergePlayers, apiUpdatePlayerAuth, } from '@/services/cubing-pro/auth/admin';
...
const Users = () => {
    ...
    const [curUpdatePlayer, setCurUpdatePlayer] = useState();
    ...
    const handleSaveAuth = async () => {
        ...
        const oldA = curUpdatePlayer.Auth;
        const newA = authChecked.reduce((acc, v) => acc | v, 0);
        const revokingOrg = hasAuth(oldA, Auth.AuthOrganizers) && !hasAuth(newA, Auth.AuthOrganizers);
        if (revokingOrg) {
            await new Promise((resolve, reject) => {
                Modal.confirm({
                    title: '撤销主办权限',
                    content: '若该用户仍在任一主办团队中担任组长或成员，服务端将拒绝。请先在主办管理中移出成员或变更组长后再撤销。',
                    okText: '仍要提交',
                    cancelText: '取消',
                    onOk: async () => {
                        try {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (value) => {
                ...
                const truncateText = (text, frontLength, endLength) => {
            ...
            <Button size="small" style={{ color: 'blueviolet' }} icon={<CopyOutlined />} onClick={handleCopy}></Button>
          </div>);
            },
        },
        {
        ...
        },
        {
            ...
            <Button type="default" size={'small'} style={{ marginLeft: 8 }} onClick={() => {
                        setCurUpdatePlayer(player);
                        setUpdatePlayerWcaModal(true);
                    }}>
              修改WCA ID
            </Button>
            <Button type="default" size={'small'} danger={true} style={{ marginLeft: 8 }} onClick={() => {
                        setCurUpdatePlayer(player);
                        setMergePlayerWcaModal(true);
                    }}>
              合并用户
            </Button>
            <Button type="default" danger={true} size={'small'} style={{ marginLeft: 8 }} onClick={() => {
                        setCurUpdatePlayer(player);
    ...
    return (<>
      <Button type="primary" icon={<UserAddOutlined />} size="small" onClick={() => {
            setCreateUserModalOpen(true);
        }} style={{ marginBottom: 20, borderRadius: 6, float: 'right' }}>
        创建用户
      </Button>

      <Modal title="创建用户" open={createUserModalOpen} onCancel={() => setCreateUserModalOpen(false)} onOk={handleCreateUser} confirmLoading={confirmLoading} okText="提交" cancelText="取消">
      ...
      <ProTable title={() => {
        ...
        }} size={'small'} scroll={{ x: 'max-content' }} rowClassName={rowClassNameWithStyleLines} columns={columns} onReset={resetParams} params={tableParams} request={async (params) => {
            // @ts-ignore
      ...
      <Modal title="修改用户名称" open={updatePlayerNameModal} onOk={handleUpdateName} confirmLoading={confirmLoading} onCancel={() => {
            setUpdatePlayerNameModal(false);
        }} destroyOnClose>
        <Form form={updateNameForm} layout="vertical" preserve={false}>
          <Form.Item label="用户名称" name="name" rules={[{ required: true, message: '请输入新名称' }]}>
            <Input placeholder="请输入新名称" defaultValue={curUpdatePlayer?.Name}/>
          </Form.Item>
        </Form>
      </Modal>

      <Modal title={`修改用户${curUpdatePlayer?.Name} WCAID`} open={updatePlayerWcaModal} onOk={handleUpdateWCA} confirmLoading={confirmLoading} onCancel={() => {
            setUpdatePlayerWcaModal(false);
        }} destroyOnClose>
        <Form form={updateWCAForm} layout="vertical" preserve={false}>
          <Form.Item label="WCAID" name="wca_id" rules={[
      ...
      <Modal title={`修改权限 — ${curUpdatePlayer?.Name} (${curUpdatePlayer?.CubeID})`} open={authModalOpen} onCancel={() => setAuthModalOpen(false)} onOk={handleSaveAuth} confirmLoading={confirmLoading} width={520} destroyOnClose>
        <p style={{ marginBottom: 12, color: '#666' }}>
          按位更新权限；撤销主办前请确认用户已从主办团队中移出（见对接说明）。
        </p>
        <Checkbox.Group style={{ display: 'flex', flexDirection: 'column', gap: 8 }} value={authChecked} onChange={(list) => setAuthChecked(list.map((x) => Number(x)))}>
          {AUTH_MANAGE_BITS.map((b) => (<Checkbox key={b.value} value={b.value}>
              {b.label}
            </Checkbox>))}
        </Checkbox.Group>
      </Modal>

      <Modal title={`合并用户 ${curUpdatePlayer?.Name}`} open={mergePlayerWcaModal} onOk={handleMergePlayer} confirmLoading={confirmLoading} onCancel={() => {
            setMergePlayerWcaModal(false);
        }} destroyOnClose>
        <Form form={mergePlayerForm} layout="vertical" preserve={false}>
          <Form.Item label="合并到账号（被合并方）" name="other_cube_id" rules={[
