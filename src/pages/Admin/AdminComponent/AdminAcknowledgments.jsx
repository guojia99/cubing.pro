...
import { getAcknowledgmentsWithAdmin, setAcknowledgmentsWithAdmin, } from '@/services/cubing-pro/auth/system';
...
const AdminAcknowledgments = () => {
    ...
    return (<PageContainer>
      <Card>
        ...
        <Table loading={loading} columns={columns} dataSource={displayList} rowKey={(_, i) => String(i)} pagination={false} onChange={(_, __, sorter) => {
      ...
      <Modal title={editIndex >= 0 ? '编辑赞助' : '新增赞助'} open={editModalOpen} onCancel={() => setEditModalOpen(false)} onOk={handleSave} destroyOnClose>
        <Form form={form} layout="vertical">
          <Form.Item name="nickname" label="名称" rules={[{ required: true, message: '请输入名称' }]}>
      ...
      <Modal title="预览展示效果" open={previewOpen} onCancel={() => setPreviewOpen(false)} footer={null} width={800}>
        {displayList.length > 0 ? (<ThanksSection data={displayList}/>) : (<div style={{ padding: 24, textAlign: 'center', color: '#999' }}>暂无赞助数据</div>)}
      </Modal>
    </PageContainer>);
};
export default AdminAcknowledgments;
//# sourceMappingURL=AdminAcknowledgments.jsx.map