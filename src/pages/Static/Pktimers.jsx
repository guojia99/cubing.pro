...
import { GetPKTimer } from '@/services/cubing-pro/pktimer/pktimer';
...
const PKTimerTable = () => {
    ...
    const expandedRowRender = (record) => {
        ...
        const playerColumns = [
            { title: '玩家', dataIndex: 'userName', key: 'userName' },
            {
                ...
                dataIndex: 'best',
                key: 'best',
                render: (_, pp) => resultTimeString(pp.best),
            },
            {
                title: '平均',
                dataIndex: 'average',
                key: 'average',
                render: (_, pp) => resultTimeString(pp.average),
            },
            {
                title: '成绩列表',
                key: 'results',
                render: (_, player) => {
                    ...
                    if (!d || d.length === 0)
                        return '-';
                    return (<>
              {d.map((time, idx) => (<Tag key={`${record.id}-${player.userId}-${idx}`} color="blue">
                  {time}
                </Tag>))}
            </>);
                },
        ...
        <Table dataSource={players} columns={playerColumns} pagination={false} rowKey={(player) => player.userId} size="small"/>
      </Card>);
    };
    const columns = [
        {
    ...
    const expandable = {
        ...
        expandedRowRender,
        // @ts-ignore
        expandIcon: ({ expanded, onExpand, record }) => expanded ? (<UpOutlined onClick={(e) => onExpand(record, e)}/>) : (<DownOutlined onClick={(e) => onExpand(record, e)}/>),
    };
    return (<div>
...
//# sourceMappingURL=Pktimers.jsx.map