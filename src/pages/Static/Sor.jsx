...
const { Option } = Select;
const DiyRankingSor = ({ keys, pages }) => {
    ...
    const handleUpdateTable = (selectEvents) => {
        ...
        if (tableParams.withSingle) {
            columns.push({
                ...
                render: (score, sor) => {
                    ...
                    if (find.Rank === 1) {
                        return (<Tooltip title={`单次: ${find.ResultString}`}>
        ...
        if (tableParams.withAvg && ev !== '333mbf') {
            columns.push({
                ...
                render: (score, sor) => {
                    ...
                    if (find.Rank === 1) {
                        return (<Tooltip title={`平均: ${find.ResultString}`}>
                <strong style={{ color: 'red' }}>{find.Rank}</strong>
              </Tooltip>);
                    }
                    return (<Tooltip title={`平均: ${find.ResultString}`}>
        ...
        <Card style={{ marginBottom: '20px' }}>
          <Form layout="vertical">
            <Form.Item label="成绩来源">
              <Select value={getCurrentValue()} onChange={handleSelectChange} style={{ width: 200 }} placeholder="请选择成绩来源">
    ...
    columns={columns} params={tableParams} request={async () => {
        ...
        }} options={false} actionRef={actionRef} size={'small'} style={{ fontSize: '14px' }} sticky scroll={{ x: 'max-content' }}/>
    </>);
};
export default DiyRankingSor;
//# sourceMappingURL=Sor.jsx.map