...
const KinCh = ({ isSenior, isCountry, otherDataFn, pages = true }) => {
    ...
    columns.push({
        ...
        render: (score) => {
            if (useColor) {
                return (<Text style={{ color: getScoreColor(score), fontWeight: score > 0 ? 'bold' : 'normal' }}>
    ...
    for (let i = 0; i < hasEvents.length; i++) {
        ...
        columns.push({
            ...
            hideInSearch: true,
            render: (value, sor) => {
                const filter = sor.Results.filter((item) => {
              ...
              <Tag color={tagColor} style={{ marginLeft: 4 }}>
                {tagLabel}
                    ...
                    // 文字样式：无成绩时置灰
                    const textStyle = hasValidResult
                        ? {}
                ...
                <span style={{ color: '#ccc' }}>0.0</span>
                    ...
                    if (useColor) {
                        displayText = find.IsBest ? (<strong style={{ color: getScoreColor(find.Result) }}>{formattedResult}</strong>) : (<Text style={{ color: getScoreColor(find.Result) }}>{formattedResult}</Text>);
                    }
                    else {
                        displayText = find.IsBest ? (<strong style={{ color: '#b40000' }}>{formattedResult}</strong>) : (formattedResult);
                    }
                    return (<Tooltip title={tooltipContent}>
    ...
    const handleRowClick = (record) => {
        return {
            onClick: (e) => {
                // 如果点击的是选手列，不触发详情弹窗
                if (e.target.closest('td')?.className?.includes('ant-table-column-name')) {
                    return;
                }
                // 否则打开详情弹窗
    ...
    // 变化时
    const handleUpdateTable = (selectEvents, age, country) => {
    ...
    return (<>
      <>
        <EventSelector events={allEvents} isSenior={isSenior} onConfirm={handleUpdateTable} isCountry={isCountry}/>
        <Card style={{ marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Switch checkedChildren="开启颜色" unCheckedChildren="关闭颜色" value={useColor} onChange={(e) => setUseColor(e)}/>
          </div>
        </Card>
      </>
    ...
    // @ts-ignore
    columns={columns} params={tableParams} request={async () => {
            let value = undefined;
        ...
        }} search={false} pagination={pages
            ? {
                showQuickJumper: true,
                current: tableParams.page,
                pageSize: tableParams.size,
            }
            : false} onChange={(pagination) => {
            setTableParams({
                ...tableParams,
        ...
        }} options={false} actionRef={actionRef} size={'small'} style={{ fontSize: '14px' }} sticky scroll={{ x: 'max-content' }} onRow={handleRowClick}/>

      {/* 玩家详细成绩弹窗 */}
      {selectedPlayer && (<KinchPlayerDetailModal visible={detailModalVisible} onCancel={() => setDetailModalVisible(false)} player={selectedPlayer} isSenior={isSenior}/>)}
    </>);
};
export default KinCh;
//# sourceMappingURL=Kinsor.jsx.map