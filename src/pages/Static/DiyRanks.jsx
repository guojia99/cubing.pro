...
import { apiDiyRanking, apiDiyRankingKinch, apiDiyRankingPersons, apiGetAllDiyRankingKey, } from '@/services/cubing-pro/statistics/diy_ranking';
...
const DiyRanksWithEvent = ({ keys }) => {
    // const actionRef = useRef();
    const [diyRankResp, setDiyRankResp] = useState();
    ...
    return (<>
      <NavTabs type="line" items={eventItems} tabsKey={keys + '_event'} indicator={{ size: (origin) => origin - 20, align: 'center' }}/>
    </>);
};
const DiyRanksWithKinch = ({ keys }) => {
...
const DiyRankPersons = ({ keys }) => {
    ...
    return (<Spin spinning={loading}>
      <Table dataSource={persons} columns={columns} rowKey={(record) => ` ${record.wca_id}- ${record.sub_id}`} pagination={false} size="middle" scroll={{ x: 600 }}/>
    </Spin>);
};
const DiyRanks = ({ keys }) => {
    const items = [
    ...
    return (<>
      <NavTabs type="line" items={items} tabsKey={keys} indicator={{ size: (origin) => origin - 20, align: 'center' }}/>
    </>);
};
const DiyRankView = () => {
    // @ts-ignore
    ...
    return (<>
      ...
      {items && items.length > 0 && (<NavTabs type="line" items={items} tabsKey="wca_tabs" indicator={{ size: (origin) => origin - 20, align: 'center' }}/>)}
    </>);
};
export default DiyRankView;
//# sourceMappingURL=DiyRanks.jsx.map