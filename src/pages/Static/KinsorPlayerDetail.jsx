...
const KinchPlayerDetailModal = ({ visible, onCancel, player, isSenior }) => {
    // 将玩家的成绩数据转换为表格数据
    ...
    const columns = [
        {
            ...
            key: 'event',
            align: 'center',
            render: (event) => (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        },
        {
            ...
            key: 'eventResult',
            align: 'center',
            render: (score) => (<Text style={{ color: getScoreColor(score), fontWeight: score > 0 ? 'bold' : 'normal' }}>
    ...
    return (<Modal title={<div style={{ textAlign: 'center' }}>
          {isSenior ? (WCALinkWithCnName(player.wca_id, player.WcaName)) : (<>{PlayerLink(player.CubeId, player.PlayerName, 'rgb(29,177,236)')}</>)}
          <div style={{ marginTop: 8, fontSize: '16px', color: '#f23f3f' }}>
            总分: <strong>{player.Result.toFixed(3)}</strong>
          </div>
        </div>} open={visible} onCancel={onCancel} footer={null} width={800} centered>
      <Table 
    // @ts-ignore
    columns={columns} dataSource={tableData} pagination={false} size="small" rowKey="key" style={{ marginTop: 16 }}/>
    </Modal>);
};
export default KinchPlayerDetailModal;
//# sourceMappingURL=KinsorPlayerDetail.jsx.map