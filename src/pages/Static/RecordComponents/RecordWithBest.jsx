...
const RecordsWithBest = ({ groupId, onGroupIdChange }) => {
    const [events, setEvents] = useState([]);
    const [bestRecords, setBestRecords] = useState();
    const [avgRecords, setAvgRecords] = useState();
    ...
    for (let i = 0; i < events.length; i++) {
        ...
        for (let i = 0; i < maxLength; i++) {
            const b = best ? best[i] : undefined;
            const a = avg ? avg[i] : undefined;
            const record = b || a;
            // @ts-ignore
            const mRecord = {
                ...record,
    ...
    return (<>
      ...
      <Select value={groupId} style={{ marginBottom: '20px', width: '150px' }} options={orgItems} onChange={(value) => onGroupIdChange(value)}/>

      {RecordsTable(records, ['MEventId', 'BestUserName', 'Best', 'Average', 'AvgUserName'], false)}
    </>);
};
export default RecordsWithBest;
//# sourceMappingURL=RecordWithBest.jsx.map