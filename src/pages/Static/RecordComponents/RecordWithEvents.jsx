...
const RecordsWithEvents = ({ groupId, eventId, onGroupIdChange, onEventIdChange, }) => {
    ...
    const items = [
        {
            label: 'WCA项目',
            title: 'WCA项目',
            options: GroupChildren(events.filter((value) => {
    ...
    return (<>
      ...
      <Select value={eventId} style={{ marginBottom: '20px', width: '150px' }} loading={loading} options={items} onChange={(value) => onEventIdChange(value)}/>

      <Select value={groupId} style={{ marginBottom: '20px', marginLeft: '20px', width: '150px' }} loading={loading} options={orgItems} onChange={(value) => onGroupIdChange(value)}/>

      {RecordsTable(rc, ['Index', 'UserName', 'Best', 'Average', 'ResultTime', 'CompsName'])}
    </>);
};
export default RecordsWithEvents;
//# sourceMappingURL=RecordWithEvents.jsx.map