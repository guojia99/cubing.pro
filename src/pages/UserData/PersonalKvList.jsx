...
import { getUserKv, listUserKvs } from '@/services/cubing-pro/user/user_kv';
...
const PersonalKvList = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            key: 'action',
            width: 120,
            render: (_, r) => (<Button type="link" size="small" onClick={() => void openDetail(r.key)}>
          {intl.formatMessage({ id: 'userKv.viewJson' })}
        </Button>),
        },
    ];
    return (<PageContainer title={intl.formatMessage({ id: 'userKv.pageTitle' })}>
      <Space style={{ marginBottom: 16 }}>
        <Button onClick={() => void load()} loading={loading}>
          {intl.formatMessage({ id: 'userKv.refresh' })}
        </Button>
      </Space>
      <Table rowKey="key" loading={loading} columns={columns} dataSource={rows} pagination={false}/>
      <Modal title={jsonModalTitle} open={jsonModalOpen} onCancel={() => setJsonModalOpen(false)} footer={null} width="min(96vw, 1120px)" centered styles={{
            body: { paddingTop: token.paddingSM },
        }} destroyOnClose>
        {detailLoading ? (<div style={{ textAlign: 'center', padding: token.paddingLG }}>
            <Spin />
          </div>) : (<pre style={jsonPreStyle}>{jsonModalBody || '—'}</pre>)}
      </Modal>
    </PageContainer>);
};
export default PersonalKvList;
//# sourceMappingURL=PersonalKvList.jsx.map