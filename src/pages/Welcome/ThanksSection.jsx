...
const intl = getIntl();
const DEFAULT_AVATAR = 'https://assets.worldcubeassociation.org/assets/062b138/assets/missing_avatar_thumb-d77f478a307a91a9d4a083ad197012a391d5410f6dd26cb0b0e3118a5de71438.png';
const MAX_VISIBLE = 12;
function hasValidWcaId(wcaID) {
    return !!wcaID && wcaID !== '-' && wcaID.length === 10;
}
function thankToThanksPerson(t) {
    return {
        wcaID: t.wcaID ?? '',
        nickname: t.nickname ?? '',
        amount: t.amount ?? 0,
 ...
 */
function getAvatarUrl(person, avatarCache) {
    if (person.avatar)
        return person.avatar;
    if (avatarCache[person.wcaID])
        return avatarCache[person.wcaID];
    return DEFAULT_AVATAR;
}
/**
 * 致谢栏 - 感谢支持者的赞助
 */
const ThanksSection = ({ data }) => {
    ...
    if (loading || thanksList.length === 0)
        return null;
    return (<Card style={{
            borderRadius: 8,
            marginBottom: 24,
      ...
      <div style={{
        ...
        {displayList.map((person, index) => (<div key={hasValidWcaId(person.wcaID) ? person.wcaID : `anon-${index}`} style={{
            ...
            <Avatar src={getAvatarUrl(person, avatarCache)} alt={person.nickname} size={56} style={{ flexShrink: 0 }}/>
            <div style={{ textAlign: 'center' }}>
              ...
              {hasValidWcaId(person.wcaID) && (<div style={{ fontSize: 11, color: token.colorTextTertiary, marginBottom: 4 }}>
                  {person.wcaID}
                </div>)}
      ...
      {hasMore && (<div style={{ marginTop: 16, textAlign: 'center' }}>
...
//# sourceMappingURL=ThanksSection.jsx.map