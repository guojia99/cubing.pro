import {
  playersWithWcaId,
  syncCubingAvatarsForPlayers,
  type CubingAvatarSyncStep,
} from '@/views/TeamMatch/syncCubingAvatars';
import type { TeamMatchStoreAction } from '@/views/TeamMatch/TeamMatchContext';
import type { TeamMatchSession } from '@/views/TeamMatch/types';
...
const SyncCubingAvatarsButton: React.FC<Props> = ({ session, dispatch }) => {
  const [busy, setBusy] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [percent, setPercent] = useState(0);
  const [step, setStep] = useState<CubingAvatarSyncStep | null>(null);
  ...
  const run = async () => {
    ...
    setHistory([]);
    try {
      const { players, updatedCount } = await syncCubingAvatarsForPlayers(
        session.players,
      ...
      message.success(
        updatedCount > 0 ? `已更新 ${updatedCount} 人头像` : '已拉取完毕（头像无变化或粗饼无头像）',
      );
    } catch (e) {
