...
/**
 * HTR 状态校验：每个贴纸的颜色所属轴必须等于其所在面的轴。
 * 这同时保证了角/棱朝向正确以及中层棱不串层（G3 的必要条件）。
 */
export function isHtrState(state: CubeState): boolean {
  for (let i = 0; i < 54; i++) {
    ...
    if (colorAxis(state[i]) !== faceAxis) return false;
  }
  return true;
}

/** 是否满足 FR 形态：该轴 0 坏棱且角块为 0 角形（可能是真 FR 或假 FR） */
...
function sideSticker(edge: CubieView, axis: AxisIndex) {
  ...
  if (!s) throw new Error("edge side sticker missing");
  return s;
}

/** 棱块的两个非轴向颜色（排序后拼接）作为“柱子”分组键 */
...
export function classifyAxis(
  ...
  for (const e of layerEdges) {
    ...
    if (side.color !== faceLetter) {
      if (e.coord[axis] === 1) badTopEdges.push(e);
  ...
  if (badCount === 0) edgeLabel = "0bad";
  else if (badCount === 2) edgeLabel = "2bad";
  else if (badCount === 6) edgeLabel = "6bad";
  else if (badCount === 8) edgeLabel = "8bad";
  else {
    // badCount === 4
    const hi = Math.max(badTop, badBottom);
    if (hi === 4) edgeLabel = "4-0";
    else if (hi === 3) edgeLabel = "3-1";
    else {
      // 2-2: 看同层两个坏棱所在侧面是否同轴（对面=o，邻面=a）
  ...
  if (sameLayerGroup) {
    ...
    for (const sa of sideAxes) if (a.coord[sa] === b.coord[sa]) sharedAxis = sa;
