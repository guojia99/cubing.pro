/**
 * 纯几何 3x3x3 魔方模型（与 cubing.js 无关，仅用于 FR 分析与求解）。
 *
 * 坐标系：x(R=+1,L=-1), y(U=+1,D=-1), z(F=+1,B=-1)。
 * 状态用 54 个贴纸颜色（面字母）表示，索引固定为 LOCS 的顺序。
 *
 * 各面转动方向（基础顺时针）已通过与真实魔方记号端到端校验确定。
 */
