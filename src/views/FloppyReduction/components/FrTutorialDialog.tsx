...
export function FrTutorialDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
    ...
    >
      <Portal>
        ...
        <Dialog.Positioner>
          <Dialog.Content>
            <Dialog.Header>
              <Dialog.Title>{t("fr.tutorial.title")}</Dialog.Title>
            </Dialog.Header>
            <Dialog.Body>
              <Stack gap="6">
                {/* 1. 前言 */}
                ...
                <Box>
                  ...
                  <Stack gap="2" mb="4">
                    <Text fontSize="xs" fontWeight="semibold" color="fg.muted">
                      {zh ? "FR trigger（3-1 / 2FB）" : "FR trigger (3-1 / 2FB)"}
                    </Text>
                    <TutorialCubeStep
                      title={zh ? "起始态" : "Start"}
                      body={
                        zh
                          ? "最常见的组合 case，大部分情况可转化到此。"
                          : "The most common combined case; most positions reduce to this."
                      }
                      scramble={FR_TRIGGER_SETUP}
                      emphasis="axis"
                    />
                    <TutorialCubeStep
                      title={zh ? "执行 R2" : "After R2"}
                      body={zh ? "侧面半转调整棱块分布。" : "Side half-turn rearranges the edges."}
                      scramble={FR_TRIGGER_SETUP}
                      solution={["R2"]}
                      emphasis="axis"
                      algLabel="R2"
                    />
                    <TutorialCubeStep
                      title={zh ? "执行 R2 U2 完成" : "R2 U2 — done"}
                      body={
                        zh
                          ? "再 U2 同时完成角柱与棱块，到达 FR 形态。"
                          : "U2 finishes both corner pillars and edges—FR shape reached."
                      }
                      scramble={FR_TRIGGER_SETUP}
                      solution={["R2", "U2"]}
                      emphasis="axis"
                  ...
                  <Box>
                    <Text fontSize="xs" fontWeight="semibold" color="fg.muted" mb="2">
                      {zh ? "简单 case 参考（UD 轴）" : "Simple cases (UD axis)"}
                    </Text>
                    ...
                    <Text fontSize="xs" color="fg.muted" mt="2">
                      {zh
                        ? "较难 case 及完整公式表请查看「说明」。"
                        : "For harder cases and the full alg table, see Reference."}
                    </Text>
                  </Box>
                </Box>
