import WCAResultChart from '@/views/Wca/PlayerComponents/WCAResultChart';
import './ResultDetailWithEvent.css';
import {
  getRecordColor,
  roundColorMap,
  roundNameMap,
  roundSortOrder,
} from '@/views/Wca/utils/events';
import { formatAttempts, resultsTimeFormat } from '@/views/Wca/utils/wca_results';
...
import ResultDetailWithRankingTimers from '@/views/Wca/PlayerComponents/ResultWIthEventRankingTimers';
  ...
  const columns: ColumnsType<(typeof dataSource)[0]> = [
    ...
    },
    {
      ...
      render: (best, record, index) => {
        ...
        return (
          <Space orientation="horizontal" size={4} style={{ display: 'flex', whiteSpace: 'nowrap' }}>
    ...
    <>
      ...
      <Table
        ...
        rowKey={(record) =>
          record.id != null
            ? String(record.id)
            : `${record.competition_id}-${record.event_id}-${record.round_type_id}-${record.round_id}`
        }
        pagination={false}
        size="small"
        bordered={false}
