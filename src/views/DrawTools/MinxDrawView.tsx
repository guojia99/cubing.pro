...
const minxCenterShapes: Array<{ d?: string; points?: string; transform?: string }> = [
  {
...
function MinxHasCenterDraw() {
  const [minxPointsM, setMinxPointsM] = useState<PathSvg[]>([]);

  useEffect(() => {
    setMinxPointsM(
      minxCenterShapes.map((shape, i) => {
        if (shape.points) {
          return { key: `MinxHasCenterDraw${i}`, points: shape.points };
        }
        return {
          key: `simple_sk${i}`,
          d: shape.d,
      ...
      items={[
        {
          ...
          content: <SimpleMinxDraw />,
        },
        {
          key: "minx_center",
          label: t("draws.expanded_view"),
          content: <MinxHasCenterDraw />,
        },
      ]}
    />
  );
