...
type LineDef = {
  ...
  labelKey:
    ...
    | "draws.sk.top.right"
    | "draws.sk.right"
    | "draws.sk.bottom.right"
    | "draws.sk.bottom.left"
    | "draws.sk.left";
  suffix: string;
};
...
const lineDefs: LineDef[] = [
  ...
  },
  {
    ...
    transform: "",
    labelKey: "draws.sk.bottom.right",
    suffix: " 1",
    key: 7,
  },
  {
...
function SimpleSkDraw() {
  const [skPointsM, setSkPointsM] = useState<PathSvg[]>([]);

  useEffect(() => {
    setSkPointsM(
      skPoints.map((point, i) => ({
        key: `simple_sk${i}`,
        d: point.d,
        transformStr: point.transform,
      })),
    );
  }, []);
  ...
  return (
    <DrawNavTabs
      ...
      items={[
        {
          key: "simple_sk",
          label: t("draws.expanded_view"),
          content: <SimpleSkDraw />,
        },
        {
          key: "3d_2_sk",
          label: t("draws.stereogram"),
          content: <SK3DDraw />,
        },
      ]}
    />
