export default {
    ...
    'wca.litCities.chinaLocked': '中国での大会参加がないため、中国地図は未解放です（台湾・香港・マカオ等は中国地図に含まれます）。',
    ...
    'wca.milestone.timelinePinchHint': '下のエリア内で二本指のピンチイン・ピンチアウトでマイルストーンのタイムラインを拡大・縮小できます。PC では同じエリア内で Ctrl（Mac は ⌘）を押しながらマウスホイールを回しても拡大・縮小できます。',
    ...
    'wca.milestone.helpIntro': '各タイプの意味と生成条件の概要です。表示はデータと本ページのルールに依存します。',
    ...
    'wca.milestone.help.first_overseas_competition': '成績の時系列で、WCA 登録国・地域以外で初めて参加した大会。初戦から海外の場合は本項目は出しません。',
    'wca.milestone.help.competing_anniversary': '初戦から 10 / 15 / 20… 年の各記念日（暦）より後の最初の大会。次の段と同じ大会なら重複しません。',
    ...
    'wca.milestone.help.grand_slam': '本ページの公式種目順に基づき、多くの種目は初めての有効シングルと平均が必要。足BLD（333ft）は対象外、3x3x3 複数BLD（333mbf）はシングルのみ。すべて達成後に表示。日数は最も早い有効記録から最後の条件達成まで（両端含む）。',
    'wca.milestone.help.record_breaker': 'NR / AsR / CR / WR などの記録フラグが付いた成績。',
    'wca.milestone.help.first_podium': '決勝系ラウンドで、種目ごとに初の表彰台または初の金メダル（本ページのラウンド条件に準拠）。',
    'wca.milestone.help.first_blindfolded_success': '各 BLD 種目で初めて有効なシングルまたは平均。',
    'wca.milestone.help.significant_improvement': '自己ベスト比でシングルまたは平均が、設定した改善閾値（既定 33%）以上一気に上がったとき。',
    'wca.milestone.help.first_333_average_sub10': '大会日とラウンド順で、3x3 平均が初めて 10.00 秒未満になった記録。それ以前の各 3x3 ラウンドを列挙（有効平均がないラウンドも含む）。',
    'wca.milestone.help.first_triple_podium_competition': '参加順で、決勝系ラウンドで少なくとも 3 種目で表彰台に入った最初の大会。',
    'wca.milestone.help.busy_competition_year': '1 暦年内の WCA 大会が 15 を超える年（16 回以上）。その年の最後の大会（開始日）に 1 件生成し、年間回数を表示。',
    'wca.milestone.help.multi_gold_single_comp': '同じ大会の決勝系ラウンドで少なくとも 2 種目で金メダル。該当する大会ごとに 1 件。',
    ...
    'wca.proportion.inferNote': 'このカード内のタイム入力は下の表のみに反映されます。参照曲線・セグメント表・中央値パラメータには影響しません。アンカー種目：{anchor}。比率はサーバー側の参照順で補間され、範囲外の外挿は不正確になることがあります。',
