...
const LanguageSelect = () => {
    ...
    const menuItems = useMemo(() => {
        return locales.map((lang) => ({
            key: lang.key,
            label: (<Space>
          <img src={getFlagUrl(lang.flag)} alt={lang.label} style={{
                    width: 16,
                    height: 12,
                    objectFit: 'cover',
                    borderRadius: 2,
    ...
    return (<Dropdown menu={{ items: menuItems, onClick: handleMenuClick }} placement="bottomRight">
      <a onClick={(e) => e.preventDefault()} style={{ padding: '0 12px' }} title="切换语言">
        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <GlobalOutlined style={{ fontSize: 16 }}/>
          <img src={getFlagUrl(currentLang?.flag || 'cn')} alt="当前语言" style={{
            width: 16,
            height: 12,
            objectFit: 'cover',
            borderRadius: 2,
