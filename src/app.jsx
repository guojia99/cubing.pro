...
import { applyWebsiteUiToDocument, layoutPatchFromWebsiteUi, readWebsiteUiFromStorage, writeWebsiteUiToStorage, } from '@/utils/websiteUiConfig';
/**
...
export async function getInitialState() {
    ...
    if (cu?.data?.id) {
        ...
        for (const kvKey of tryKeys) {
            try {
                const r = await Request.get(`/user/kv/${encodeURIComponent(kvKey)}`, { headers: AuthHeader() });
                const inner = r.data?.data;
                const raw = inner?.value ?? inner?.Value;
                if (raw) {
                    const cfg = JSON.parse(raw);
                    writeWebsiteUiToStorage(cfg);
                    Object.assign(mergedSettings, layoutPatchFromWebsiteUi(cfg));
...
export const layout = ({ initialState }) => {
    ...
    return {
        ...
        menuHeaderRender: (logoDom, titleDom, props) => props?.isMobile ? (<div style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%' }}>
        ...
        </div>) : (<div style={{ display: 'flex', alignItems: 'center', height: '100%', minHeight: 22, gap: 6 }}>
          {logoDom}
          {titleDom}
        </div>),
        contentStyle: {
        ...
        childrenRender: (children) => {
          ...
          <TokenCallbackHandler>
            <div className="app-content-wrapper" data-ant-content-theme={contentDark ? 'dark' : 'light'} style={{ marginTop: 32 }}>
              {children}
            </div>
          </TokenCallbackHandler>
        </ConfigProvider>);
