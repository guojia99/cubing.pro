import { Link } from '@@/exports';
import React from 'react';
export const PlayerLink = (userId, name, color) => {
    const linkColor = color !== '' ? color : 'var(--ant-color-link)';
    return (<strong>
      <Link to={'/player/' + userId} style={{ color: linkColor }}>
        {name}
      </Link>
    </strong>);
};
export const CompetitionLink = (compsId, name) => {
    return (<td>
      <strong>
        <Link to={'/competition/' + compsId}>{name}</Link>
      </strong>
    </td>);
};
export const WCALink = (wcaId, name = null) => {
    if (wcaId === null || wcaId === undefined) {
        return <>-</>;
    }
    let wca = wcaId.toUpperCase();
    let dataName = wca;
    if (name !== null) {
        dataName = name;
    }
    if (wca !== undefined && wca !== '' && wca !== "-") {
        return (<strong>
        <Link to={'/wca/player/' + wca}>{dataName}</Link>
      </strong>);
    }
    return <>-</>;
};
/**
 * 判断字符串是否包含中文，并提取所有中文字符
 * @param text 输入字符串
 * @returns { hasChinese: boolean; chinese: string } 包含中文标志和提取出的中文字符串
 */
function extractChinese(text) {
    // 匹配所有中文字符的正则（基本汉字区间）
    const regex = /[\u4e00-\u9fff]/g;
    const matches = text.match(regex);
    if (matches) {
        return matches.join('');
    }
    return text;
}
export const WCALinkWithCnName = (wcaId, name = null) => {
    if (name === null || name === '') {
        return WCALink(wcaId, name);
    }
    return WCALink(wcaId, extractChinese(name));
};
//# sourceMappingURL=Links.jsx.map