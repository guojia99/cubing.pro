import { CubesCn } from '@/components/CubeIcon/cube';
import { CubeIcon } from '@/components/CubeIcon/cube_icon';
import { eventRouteM } from '@/components/Data/cube_result/event_route';
import { resultStringPro, resultTimeString } from '@/components/Data/types/result';
import { Table } from 'antd';
import { RecordTagWithResult } from '@/components/Data/cube_record/record_tag';
import { generateRecordMap } from '@/components/Data/cube_record/record_utils';
import { CompetitionLink, PlayerLink } from '@/components/Link/Links';
import '@/components/Data/table_fixed_column.css';
import './result_tables.css';
export const playerResultKeys = ['EventNameOnlyOne', 'Round', 'Best', 'Average', 'Result'];
export const ResultsTable = (dataSource, keys, records, other_columns = undefined) => {
    let recordsMap = generateRecordMap(records); // map[resultsID]Record
    const columnsMap = new Map([
        [
            'Index',
            {
                title: '序号',
                dataIndex: 'Index',
                key: 'Index',
                width: 100,
            },
        ],
        [
            'Round',
            {
                title: '轮次',
                dataIndex: 'Round',
                key: 'Round',
                width: 80,
            },
        ],
        [
            'PersonName',
            {
                title: '玩家名',
                dataIndex: 'PersonName',
                key: 'PersonName',
                width: 150,
                // eslint-disable-next-line @typescript-eslint/no-unused-vars
                onCell: (result, rowIndex) => ({
                    className: 'custom-cell',
                }),
                render: (value, result) => {
                    return <>{PlayerLink(result.CubeID, value, '')}</>;
                },
            },
        ],
        [
            'CubeID',
            {
                title: 'CubeID',
                dataIndex: 'CubeID',
                key: 'CubeID',
                width: 100,
            },
        ],
        [
            'Best',
            {
                title: '单次',
                dataIndex: 'Best',
                key: 'Best',
                // eslint-disable-next-line @typescript-eslint/no-unused-vars
                onCell: (result, rowIndex) => {
                    const m = eventRouteM(result.EventRoute);
                    const isRepeatedly = m.repeatedly;
                    return {
                        className: 'cube_result_Best_col',
                        colSpan: isRepeatedly ? 2 : 1, // 如果是 repeated 模式，占两列
                    };
                },
                width: 135,
                render: (results, result) => {
                    const m = eventRouteM(result.EventRoute);
                    let inter = m.integer ? m.integer : false;
                    if (m.repeatedly) {
                        inter = true;
                        return (<>
                {RecordTagWithResult(
                            // resultTimeString(results !== undefined ? results: 0, inter),
                            [result.BestRepeatedlyReduction, '/', result.BestRepeatedlyTry, ' ', resultTimeString(result.BestRepeatedlyTime, false)].join(''), result.id + '_repeatedly', true, false, recordsMap)}
              </>);
                    }
                    return (<span className={'cube_result_Best_col'}>
              {RecordTagWithResult(resultTimeString(results, inter), result.id + '_best', true, false, recordsMap)}
            </span>);
                },
            },
        ],
        [
            'Average',
            {
                title: '平均',
                dataIndex: 'Average',
                key: 'Average',
                width: 135,
                // eslint-disable-next-line @typescript-eslint/no-unused-vars
                onCell: (result, rowIndex) => {
                    const m = eventRouteM(result.EventRoute);
                    // 当是 repeatedly 模式时，该列不占格子
                    return {
                        className: 'cube_result_Average_col',
                        colSpan: m.repeatedly ? 0 : 1,
                    };
                },
                render: (results, result) => {
                    const m = eventRouteM(result.EventRoute);
                    if (m.repeatedly) {
                        // todo 多轮的
                        return <></>;
                    }
                    // todo 需要pb
                    return (<span className={'cube_result_Average_col'}>
              {RecordTagWithResult(resultTimeString(results, false, false, m.integer), result.id + '_average', false, false, recordsMap)}
            </span>);
                },
            },
        ],
        [
            'Result',
            {
                title: '成绩',
                dataIndex: 'Result',
                key: 'Result',
                render: (results, result) => {
                    let body = [];
                    // eslint-disable-next-line array-callback-return
                    const data = resultStringPro(results, result.EventRoute);
                    // eslint-disable-next-line array-callback-return
                    data.map((value, idx) => {
                        body.push(<span key={idx} className="cube_result_result_cell" style={{ minWidth: '80px', display: 'inline-block' }}>
                {value}
              </span>);
                    });
                    return <div className={'cube_result_results_col'}>{body}</div>;
                },
            },
        ],
        [
            'Result_with_repeatedly',
            {
                title: '成绩',
                dataIndex: 'Result',
                key: 'Result',
                render: (results, result) => {
                    let body = [];
                    // eslint-disable-next-line array-callback-return
                    const data = resultStringPro(results, result.EventRoute);
                    // eslint-disable-next-line array-callback-return
                    data.map((value, idx) => {
                        body.push(<span key={idx} className="cube_result_result_cell" style={{ minWidth: '80px', display: 'inline-block' }}>
                {RecordTagWithResult(value, result.id + '_repeatedly', true, false, recordsMap)}
              </span>);
                    });
                    return <div className={'cube_result_results_col'}>{body}</div>;
                },
            },
        ],
        [
            'EventName',
            {
                title: '项目',
                dataIndex: 'EventID',
                key: 'EventID',
                width: 80,
                // eslint-disable-next-line @typescript-eslint/no-unused-vars
                onCell: (result, rowIndex) => ({
                    className: 'custom-cell',
                }),
                render: (value, result) => {
                    return (<span style={{ minWidth: '80px', width: '80px', display: 'inline-block' }}>
              {CubeIcon(result.EventID, result.EventID, {})} {CubesCn(value)}
            </span>);
                },
            },
        ],
        [
            'EventNameOnlyOne',
            {
                title: '项目',
                dataIndex: 'EventID',
                key: 'EventID',
                width: 80,
                // eslint-disable-next-line @typescript-eslint/no-unused-vars
                onCell: (result, rowIndex) => ({
                    className: 'custom-cell',
                }),
                render: (value, result) => {
                    if (result.RoundNumber !== 1) {
                        return null;
                    }
                    // todo 中英文？
                    return (<span style={{ minWidth: '80px', width: '80px', display: 'inline-block' }}>
              {CubeIcon(result.EventID, result.EventID, {})} {CubesCn(value)}
            </span>);
                },
            },
        ],
        [
            'Rank',
            {
                title: '排名',
                dataIndex: 'Rank',
                key: 'Rank',
                width: 65,
                // eslint-disable-next-line @typescript-eslint/no-unused-vars
                onCell: (result, rowIndex) => ({
                    className: 'custom-cell cube_result_rank_col',
                }),
                render: (value) => {
                    return <span>{value + 1}</span>;
                },
            },
        ],
        [
            'CompetitionName',
            {
                title: '比赛',
                dataIndex: 'CompetitionName',
                key: 'CompetitionName',
                render: (value, record) => {
                    return <>{CompetitionLink(record.CompetitionID, value)}</>;
                },
                width: 260,
            },
        ],
    ]);
    let columns = [];
    if (other_columns) {
        for (let c of other_columns) {
            columns.push(c);
        }
    }
    for (let key of keys) {
        let column = columnsMap.get(key);
        if (column === undefined) {
            continue;
        }
        columns.push(column);
    }
    if (columns.length > 0) {
        columns[0] = { ...columns[0], fixed: 'left' };
    }
    return (<Table dataSource={dataSource} 
    // @ts-ignore
    columns={columns} pagination={false} size="small" className="cube-results-table" scroll={{ x: 'max-content' }}/>);
};
//# sourceMappingURL=result_tables.jsx.map