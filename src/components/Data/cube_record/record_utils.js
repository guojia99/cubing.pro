export function generateRecordMap(records) {
    if (!records) {
        return new Map();
    }
...
export function RecordMapWithRecordID(records) {
    if (!records) {
        return new Map();
    }
    return records.reduce((map, record) => {
...
export function MergeRecords(records) {
    const recordMap = {}; // 用 ResultId 作为 key
    const mergedRecords = [];
...
//# sourceMappingURL=record_utils.js.map