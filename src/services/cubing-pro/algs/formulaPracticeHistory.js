    ...
    try {
        ...
        if (Array.isArray(parsed)) {
            ...
            records = parsed.map((r, i) => ({
                id: `${r.createdAt ?? Date.now()}-${i}`,
                createdAt: r.createdAt ?? Date.now(),
                timeMs: r.time ?? r.timeMs ?? 0,
                formulaKey: r.formulaKey ?? `${r.setName}:${r.groupName}:${r.formulaName}`,
                formulaName: r.formulaName ?? '',
                setName: r.setName ?? '',
                groupName: r.groupName ?? '',
                scramble: r.scramble ?? '',
                scrambleIndex: r.scrambleIndex ?? 0,
                image: r.image ?? '',
...
function loadStorageData(cube, classId) {
    try {
        ...
        if (raw) {
            ...
            if (migrated) {
                ...
                const isOldFormat = fromLegacyKey ||
                    raw.startsWith('[') ||
...
function saveStorageData(cube, classId, data) {
    if (!cube || !classId) {
    ...
    let current = { version: 2, sessions: data.sessions.map((s) => ({
            ...s,
...
export function getFormulaPracticeHistory(cube, classId) {
 ...
 * 创建新的练习会话，在「开始练习」时调用
 */
export function createPracticeSession(cube, classId, params) {
    if (!cube || !classId) {
        if (process.env.NODE_ENV === 'development') {
 ...
 * 分页获取练习历史
 */
export function getFormulaPracticeHistoryPage(cube, classId, page, pageSize = PAGE_SIZE) {
 ...
 */
export function appendFormulaPracticeRecord(cube, classId, sessionId, input) {
    if (!cube || !classId) {
        if (process.env.NODE_ENV === 'development') {
 ...
 * 删除单条记录
 */
export function deleteFormulaPracticeRecord(cube, classId, recordId) {
    if (!cube || !classId)
        return getFormulaPracticeHistory(cube, classId);
    const data = loadStorageData(cube, classId);
    const sessions = data.sessions
 ...
 */
export function computeTrimmedMean(times, trimRatio) {
    if (times.length === 0)
        return null;
 ...
 */
export function computeAverages(records, trimRatio) {
...
export function clearFormulaPracticeHistory(cube, classId) {
    try {
...
//# sourceMappingURL=formulaPracticeHistory.js.map