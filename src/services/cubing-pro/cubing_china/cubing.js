...
// 写入缓存到 localStorage
function setCacheToStorage(data) {
    try {
        const payload = {
            data,
            timestamp: Date.now(),
...
// === 原有异步接口（保留 + 支持持久化缓存）===
export async function getCubingChinaComps() {
    const now = Date.now();
    let cachedData = null;
    // 尝试从 localStorage 读取
    ...
    try {
        const response = await Request.get("/wca/comps/china", {});
...
export function findCubingCompetitionByIdentifier(identifier) {
    const storageCache = getCacheFromStorage();
    const now = Date.now();
