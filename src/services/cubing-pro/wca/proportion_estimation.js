 ...
 */
export async function getResultProportionEstimation(estimationType, wrN) {
    const res = await Request.get('/wca/extend/resultProportionEstimation', {
        params: {
            estimationType,
            wrN,
        },
...
export function interpolateRatioAt(anchorCs, segments, event, globalRatio) {
 ...
 */
export function solveBackendAnchorCs(targetEventCs, targetEvent, backendAnchor, segments, globalRatio) {
    if (targetEvent === backendAnchor) {
        return Math.max(0, targetEventCs);
    }
    const gE = globalRatio[targetEvent] ?? 1;
    const implied = (ta) => ta * interpolateRatioAt(ta, segments, targetEvent, gE);
    if (targetEventCs <= 0) {
        return 0;
    }
    let lo = 1;
 ...
 */
export function impliedCentisAllEvents(backendAnchorCs, events, backendAnchor, segments, globalRatio) {
 ...
 */
export function segmentRefRangeAsAnchorCs(refMinCs, refMaxCs, anchorEvent, backendAnchor, segments, globalRatio) {
    if (anchorEvent === backendAnchor) {
        return [refMinCs, refMaxCs];
    }
    const g = globalRatio[anchorEvent] ?? 1;
    const low = refMinCs * interpolateRatioAt(refMinCs, segments, anchorEvent, g);
    const high = refMaxCs * interpolateRatioAt(refMaxCs, segments, anchorEvent, g);
    return [low, high];
}
//# sourceMappingURL=proportion_estimation.js.map