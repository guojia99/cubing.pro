...
| 常量 / Token | 用途 |
|--------------|------|
| `ALGS_COLORS.cardBg` → `algs.card.bg` | 列表小卡片背景（深浅色自动切换） |
| `ALGS_COLORS.cardBorder` → `algs.card.border` | 列表小卡片边框 |
| `ALGS_COLORS.cardDiagramBg` → `algs.card.diagram` | 列表卡片内魔方图底色 |
| `ALGS_COLORS.sectionBg` → `algs.section.bg` | 分类大卡片背景 |
| `SET_CARD_COLORS` | 公式集分组卡片（rgba，详情页） |
| `PRACTICE_COLORS` | 练习工具面板色块（rgba，详情页） |

深色模式下 `algs.card.*` 使用 `brand.900` / `brand.700` 海蓝系，避免浅色 rgba 叠在 `brand.950` 上发灰。
